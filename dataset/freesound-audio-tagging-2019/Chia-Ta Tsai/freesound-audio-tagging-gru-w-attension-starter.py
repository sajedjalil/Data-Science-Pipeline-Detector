...
from keras.layers import Embedding, Input, Dense, CuDNNGRU, Bidirectional, SpatialDropout1D, Conv1D, GlobalAveragePooling1D, GlobalMaxPooling1D, Dropout, concatenate 
...
def labelize(series):
    return series.apply(lambda x: pd.Series(index=x.split(',')).fillna(1)).fillna(0).astype(np.int8)


def split_index(series):
    train_split_index = series.apply(hash)
    tmp = train_split_index.value_counts()
    minor_split_index = tmp.loc[tmp <= 5]
...
def main():
    ...
    train_curated = pd.read_csv('../input/train_curated.csv').set_index('fname')
    train_curated_df = labelize(train_curated['labels'])

    # train_noisy_path = '../input/train_noisy/'
    # train_noisy = pd.read_csv('../input/train_noisy.csv').set_index('fname')
    # train_labels = labelize(train_noisy['labels'])

    #
    label_columns = sorted(train_curated_df.columns.tolist())

    df_split_index = split_index(train_curated['labels'])
    train_x = transform(train_curated.index.tolist(), train_curated_path, conf)
    train_y = train_curated_df[label_columns].values
    ...
    model = get_model(input_shape=(636, 128), output_shape=len(label_columns))
