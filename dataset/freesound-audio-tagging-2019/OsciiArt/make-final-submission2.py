...
MAX_PAD = 32000
MAX_BATCHSIZE = 512
NUMBATCH_PER_NUMDATA = 1/30 # TODO search best value
MAX_PATIENCE = 0.2
...
def main():
    ...
    # df_test_sort.to_csv("df_test_sort.csv", index=None)
    ...
    preds_test_avr = (
        ...
        + preds_test_mel[:,1, :4].mean(axis=(0,1,2)) * 3/13 
        + preds_test_mel[:,2, :3].mean(axis=(0,1,2)) * 3/13 
        + preds_test_wav[:,0].mean(axis=(0,1,2)) * 1/13 
        + preds_test_wav[:,1,:2].mean(axis=(0,1,2)) * 1/13 
        + preds_test_wav[:,2,:2].mean(axis=(0,1,2)) * 1/13 )
