...
def bb_sort(ll):
    ...
    while not optimal:
        ...
        for i in range(1,len(ll) - 3):
            lcopy=[[] for _ in range(6)]
            b=list(permutations(([0,1,2])))
            for k in range(1,6):
              lcopy[k]=ll[:]
              lcopy[k][i], lcopy[1][i+1], lcopy[1][i+2] = ll[i+b[k][0]][:], ll[i+b[k][1]][:],ll[i+b[k][2]][:]
