...
for n in [1.263]:
    ...
    for i in range(90,-90,int(-180/n)):
        ...
        for j in range(180,-180,int(-360/n)):
            ...
            " AND  " + str(j) + "));")
            c.commit()
    ts = pd.read_sql("SELECT i,j,min(Latitude) mnLa,min(Longitude) mnLo,max(Latitude) mxLa,max(Longitude) mxLo FROM gifts group by i,j;",c)
    print(ts)
    for limit_ in [67]:
