...
for ngs_file in NGS_csv_files:
    ...
    ngs['primary_partner_gsisid'] = ngs['primary_partner_gsisid'].replace('Unclear',0).fillna(0).astype('int')
    ...
    injury_plays = ngs.loc[~ngs['gsisid_injured'].isna()]
    ngs_injury_plays = pd.concat([injury_plays, ngs_injury_plays])
    # Remove data from memory
    del ngs
    gc.collect()
ngs_injury_plays.to_parquet('NGS-injury-plays.parquet')