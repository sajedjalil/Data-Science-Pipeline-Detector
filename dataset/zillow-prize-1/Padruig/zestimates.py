...
# based entirely on the suggestions (and taunting) of Zeeshan-ul-hassan Usmani
...
# We need to load the datasets that will be needed to train our machine
# learning algorithms, handle our data and make predictions.
...
# We are now label encoding our datasets. All of the machine learning
# algorithms employed in scikit learn assume that the data being fed
# to them is in numerical form. LabelEncoding ensures that all of our
# categorical variables are in numerical representation. Also note
# that we are filling the missing values in our dataset with a zero
# before label encoding them. This is to ensure that label encoder
# function does not experience any problems while carrying out its operation
...
# We will now drop the features that serve no useful purpose. We will
# also split our data and divide it into the representation to make it
# clear which features are to be treated as determinants in predicting
# the outcome for our target feature. Make sure to include the same features
# in the test set as were included in the training set #
...
# that we could monitor and the test the progress of our machine learning
# algorithm. This would let us know when our model might be over or under
# fitting on the dataset that we have employed. #
...
# We can now select the parameters for Xgboost and monitor the progress
# of results on our validation set. The explanation of the xgboost parameters
# and what they do can be found on the following link
...
# Let us now predict the target variable for our test dataset. All we have to
# do now is just fit the already trained model on the test set that we had
# made merging the sample file with properties dataset
