...
prop =prop.drop(['storytypeid', 'basementsqft', 'yardbuildingsqft26', 'fireplaceflag',
       'architecturalstyletypeid'], axis = 1)
...
def kinetic_energy(column_df):
    probs = (np.unique(column_df, 
                       return_counts=True)[1]).astype(float)/column_df.shape[0]
...
for col  in   prop.columns :
    if prop[col].isnull().values.any()==True:
        prop[col]=replace_nan(prop[col])
           
prop.head()

print(" sentences: prop is  containing nans is :",prop.isnull().values.any())
