## modified to get Out of fold predictions using these features. to be combined with other sources
...
def kfold_lightgbm(df, num_folds, stratified = False, debug= False):
    ...
    ## changed 0get OOF for train as well, and always - DAN
    # if not debug:
    train_df["oof_preds_Agg_1"] = oof_preds  # check right indices acc. to correlation
    print("train Correlation: \n ", train_df[["oof_preds_Agg_1",'TARGET']].corr())
    
    test_df["oof_preds_Agg_1"] = sub_preds
    df_oof = pd.concat([train_df.loc[:,['SK_ID_CURR',"oof_preds_Agg_1"]],test_df.loc[:,['SK_ID_CURR',"oof_preds_Agg_1"]]])

    df_oof[['SK_ID_CURR', 'oof_preds_Agg_1']].to_csv("credit_oof_preds_Agg1.csv", index= False)
...
def main(debug = False):
    ...
    with timer("Run LightGBM with kfold"):
        feat_importance = kfold_lightgbm(df, num_folds= 6, stratified= True, debug= debug)
