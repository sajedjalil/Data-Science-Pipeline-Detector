...
def application_train_test(num_rows = None, nan_as_category = False):
    ...
    df['le_income_ratio'] = df['AMT_INCOME_TOTAL'] / df['NEW_INC_BY_ORG']
    del df['NEW_INC_BY_ORG']
    ...
    df['credit_div_income_total'] = df['AMT_CREDIT'] / df['AMT_INCOME_TOTAL']
    df['annuity_div_income_total'] = df['AMT_ANNUITY'] / df['AMT_INCOME_TOTAL']
    df['credit_div_annuity'] = df['AMT_CREDIT'] / df['AMT_ANNUITY']
    df['birth div employed'] = df['DAYS_BIRTH'] / df['DAYS_EMPLOYED']
...
def display_importances(feature_importance_df_, cv):
    ...
    plt.tight_layout()
    plt.savefig('lgbm_importances01_%.6f.png' % cv)
