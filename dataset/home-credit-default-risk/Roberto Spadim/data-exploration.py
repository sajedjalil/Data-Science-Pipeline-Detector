...
def train_model(data_, test_, y_, folds_):
    ...
    for n_fold, (trn_idx, val_idx) in enumerate(folds_.split(data_)):
        ...
        ret = clf.fit(trn_x, trn_y,
                eval_set=[(trn_x, trn_y), (val_x, val_y)],
                eval_metric='auc', verbose=100, early_stopping_rounds=100  # 30
                )

        print(ret)
        plt.plot(ret.evals_result_['training']['auc'], label='Fold '+str(n_fold+1)+', Treinamento', color=colors[n_fold], linestyle='-')
        plt.plot(ret.evals_result_['valid_1']['auc'], label='Fold '+str(n_fold+1)+', Validação', color=colors[n_fold], linestyle='-.')
        plt.axvline(ret.best_iteration_, label='Fold '+str(n_fold+1)+', Early Stop ['+str(ret.best_iteration_)+']', color=colors[n_fold], linestyle=':')
...
def display_importances(feature_importance_df_):
    ...
    plt.title('LightGBM Variáveis Média entre Folds')
    plt.tight_layout()
...
def display_roc_curve(y_, oof_preds_, folds_idx_):
    ...
    for n_fold, (_, val_idx) in enumerate(folds_idx_):
        ...
        scores.append(score)
        plt.plot(fpr, tpr, lw=1, alpha=0.3, label='Fold %d (AUC = %0.4f)' % (n_fold + 1, score))

    plt.plot([0, 1], [0, 1], linestyle='--', lw=2, color='r', label='Sorte', alpha=.8)
    fpr, tpr, thresholds = roc_curve(y_, oof_preds_)
    score = roc_auc_score(y_, oof_preds_)
    #plt.plot(fpr, tpr, color='b',
    #         label='ROC Médio (AUC = %0.4f $\pm$ %0.4f)' % (score, np.std(scores)),
    #         lw=2, alpha=.8)
...
def display_precision_recall(y_, oof_preds_, folds_idx_):
    ...
    #         label='ROC Média (AUC = %0.4f $\pm$ %0.4f)' % (score, np.std(scores)),
    #         lw=2, alpha=.8)
    ...
    plt.ylabel('Precisão')
    plt.title('LightGBM Revocação / Precisão')
    plt.legend(loc="best")
    #plt.tight_layout()
