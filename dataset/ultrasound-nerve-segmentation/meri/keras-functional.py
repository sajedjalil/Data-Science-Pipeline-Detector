...
from keras.layers import Input, Dense, Flatten, merge, UpSampling2D
...
def create_model(img_rows, img_cols):
    ...
    f1 = Flatten()(pool2)
    d1 = Dense(2, activation='softmax')(f1)
...
def run_cross_validation(nfolds=5):
    ...
    for train_index, test_index in kf:
        ...
        callbacks = [
            ...
            ModelCheckpoint("-cache.h5-", monitor='val_loss', verbose=1, save_best_only=True),
        ]
        model.fit(X_train, Y_train, batch_size=batch_size, nb_epoch=nb_epoch,
              shuffle=True, verbose=2, validation_data=(X_valid, Y_valid),
              callbacks=callbacks)
              
        model.load_weights("-cache.h5-")
