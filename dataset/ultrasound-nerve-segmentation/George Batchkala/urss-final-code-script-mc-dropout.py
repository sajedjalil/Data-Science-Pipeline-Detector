...
def get_dropout(p=0.5, mc=pred_PARS['mc']):
    def f(_input):
        return Dropout(p)(_input, training=mc)
    return f
...
def train_and_predict():
    ...
    # choosing method of prediction: with or without mc dropout
    if pred_PARS['mc']:
        n = pred_PARS['mc_sample_size']
        
        # versions with 1 and 2 outputs
        if PARS['outputs'] == 1:    
            imgs_mask_test = model.predict(imgs_test) / n
            for i in tqdm.tqdm(range(n)):
                imgs_mask_test += model.predict(imgs_test) / n
        else:
            imgs_mask_test = [model.predict(imgs_test)[0] / n, model.predict(imgs_test)[1] / n]
            for i in tqdm.tqdm(range(n)):
                imgs_mask_test[0] += model.predict(imgs_test)[0] / n
                imgs_mask_test[1] += model.predict(imgs_test)[1] / n            
    else:
