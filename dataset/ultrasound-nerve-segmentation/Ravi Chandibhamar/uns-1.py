...
def create_model(img_rows, img_cols):
    model = Sequential()
    model.add(Convolution2D(8, 8, 4, border_mode='same', init='he_normal',
    ...
    sgd = SGD(lr=1.000e-3, decay=0.8e-6, momentum=0.90, nesterov=True)
    model.compile(optimizer=sgd, loss='categorical_crossentropy')
    return model


def run_single(nfolds=1):
    ...
    model.fit(X_train, Y_train, batch_size=batch_size, nb_epoch=nb_epoch,
          shuffle=True, verbose=2, validation_data=(X_train, Y_train),
          callbacks=callbacks)

    predictions_valid = model.predict(X_train, batch_size=batch_size, verbose=1)
