...
# у меня не получилось дождаться на ноутбуке когда построится модель, поэтому я просто сохранил ее в output 
# в кегле и после этого скачал и использую загружая) на сервере строится примерно 7 минут.
# возможно стоит использовать word2vec с выкинутыми стоп словами (сделано)
...
# собственно основная суть того, что я изменил находится в этой фиче. По распределеню важности признаков
# видно, что она имеет самый большой вес. для построения была использована библиотека gensim. 
# возможно есть более адекватный способ сравнить схожесть двух предложений по векторам их слов, но мне
# пришла в голову только L2 норма разности суммы векторов.


    x['sim'] = df['word_shares'].apply(lambda x: float(x.split(':')[5]))


# далее были использованы описаные здесь https://www.kaggle.com/gpboss/magic-features-0-03-gain
# фичи
    print ("Extracting hash features...")
...
# добавление к данным train дополнительно нвопросов, которые не являются дубликатами, что бы 
# отношение дубликатов к общему количеству вопросов стало таким, как в тестовой выборке
# этот способ не использует цикл, в отличие от оригинала и работает сильно быстрее
    ...
    if 1: # Now we oversample the negative class - on your own risk of overfitting!
        ...
        print("Oversampling done, new proportion: {}".format(len(x_train[np.array (y_train) == 1]) / float((len(x_train)))))
        del pos_train, neg_train, neg_train_add
    
# запуск алгоритма xgboost с параметрами указанными в начале. относительно версии из открытых кернелов 
# поменял количество проходов и максимальную глубину. так получается лучший результат
...
# оставил без изменений код по созданию картинки, отображающей важность параметров модели.
