...
def minimize_train_log_loss( W ):
    train["prediction"] = GLOBAL_MEAN * W[0] + train["R"] * W[1] + train["R"] * train["R"] * W[2]
    score = log_loss( train['is_duplicate'], train['prediction'] )
    print(  score , W )
    return( score )

res = minimize(minimize_train_log_loss, [0.2,  0.75, 0.05], method='Nelder-Mead', tol=1e-4, options={'maxiter': 400})
W = res.x
print( 'Best weights: ',W )


#APPLY TO TESTSET
test["is_duplicate"] = GLOBAL_MEAN * W[0] + test["R"] *  W[1] + test["R"] * test["R"] * W[2]
