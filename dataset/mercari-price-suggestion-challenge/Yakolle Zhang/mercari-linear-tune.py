...
def read_data(data_dir='../input'):
    def fillna(df):
        ...
        df['item_condition_id'] = df.item_condition_id.fillna(0).apply(
            lambda ele: 0 if isinstance(ele, str) and ele not in string.digits else ele).astype(np.int16)
        df['shipping'] = df.shipping.fillna(0).apply(
            lambda ele: 0 if isinstance(ele, str) and ele not in string.digits else ele).astype(np.int8)
...
def name_thread(tr_name_col, tr_brand_col, ts_name_col, ts_brand_col):
    col_name = tr_name_col.name
    rec_time('train %s process' % col_name)
    tr_name_col = tr_name_col.str.replace(r'\s\s+', ' ')
    rec_time('train %s vectorize' % col_name)
    cvr = CountVectorizer(token_pattern=r'(?u)\w+|[^\w\s]', ngram_range=(1, 3), vocabulary=name_terms, dtype=np.uint8)
    tvr = TfidfVectorizer(token_pattern=r'(?u)\w+|[^\w\s]', ngram_range=(1, 2), min_df=2, dtype=np.float32)
    tr_col = tr_brand_col + ' ' + tr_name_col
    tr_x_name = cvr.fit_transform(tr_col)
    print_info('train %s vectorize' % col_name, 'name vocabulary size is %d.' % len(name_terms))
    rec_time('train %s vectorize using tfidf' % col_name)
    tr_x_name_tfidf = tvr.fit_transform(tr_col)
    print_info('train %s vectorize using tfidf' % col_name, tr_x_name_tfidf.shape)
    ...
    rec_time('test %s vectorize' % col_name)
    ts_col = ts_brand_col + ' ' + ts_name_col
    ts_x_name = cvr.transform(ts_col)
    ts_x_name_tfidf = tvr.transform(ts_col)
    print_info('test %s vectorize' % col_name)
    ...
    return ((tr_x_name, tr_x_name_cnts, ts_x_name, ts_x_name_cnts, nn_tr_x_name, nn_ts_x_name, name_len,
             tr_x_brand_cnts, ts_x_brand_cnts, tr_x_brand, ts_x_brand, nn_tr_x_brand, nn_ts_x_brand, brand_len,
             tr_x_name_tfidf, ts_x_name_tfidf))


def nn_desc_thread(tr_desc_col, ts_desc_col):
    ...
    return nn_tr_x_desc, nn_ts_x_desc, len(tkr.word_index) + 1
...
def desc_vectorize_tfidf_thread(tr_desc_col, tr_brand_col, ts_desc_col, ts_brand_col, desc_queue):
    col_name = tr_desc_col.name
    rec_time('train %s process' % col_name)
    tr_desc_col = tr_desc_col.str.replace(r'\s\s+', ' ')
    rec_time('train %s vectorize' % col_name)
    vr = TfidfVectorizer(token_pattern=r'(?u)\w+|[^\w\s]', ngram_range=(1, 3), min_df=3, dtype=np.float32)
    tr_x_desc = vr.fit_transform(tr_brand_col + ' ' + tr_desc_col)
    print_info('train %s vectorize' % col_name, tr_x_desc.shape)
    print_info('train %s process' % col_name)
    ...
    ts_desc_queue = Queue()
    worker1 = get_worker(func=desc_vectorize_test_thread, name='desc_vectorize_tfidf_worker1',
                         args=(1, vr, ts_col[ts_part_data_len: 2 * ts_part_data_len], ts_desc_queue))
    worker1.start()
    worker2 = get_worker(func=desc_vectorize_test_thread, name='desc_vectorize_tfidf_worker2',
...
def desc_thread(tr_desc_col, tr_brand_col, ts_desc_col, ts_brand_col, desc_queue):
    col_name = tr_desc_col.name
    rec_time('train %s process' % col_name)
    tr_desc_col = tr_desc_col.str.replace(r'\s\s+', ' ')
    rec_time('train %s vectorize' % col_name)
    ...
    ts_desc_col = ts_desc_col.str.replace(r'\s\s+', ' ')
    rec_time('test %s vectorize' % col_name)
    ts_x_desc = vr.fit_transform(ts_brand_col + ' ' + ts_desc_col)
    print_info('test %s vectorize' % col_name)
    print_info('test %s process' % col_name)
    del ts_brand_col
    gc.collect()

    nn_tr_x_desc, nn_ts_x_desc, max_desc_len = nn_desc_thread(tr_desc_col, ts_desc_col)
    tr_desc_cnts, ts_desc_cnts = desc_cnt_thread(tr_desc_col, ts_desc_col)

    desc_queue.put((tr_x_desc, ts_x_desc, tr_desc_cnts, ts_desc_cnts, nn_tr_x_desc, nn_ts_x_desc, max_desc_len))


def cat_vectorize_thread(col_name, tr_cat_col, ts_cat_col):
...
def get_data():
    ...
    desc_queue_tfidf = Queue()
    desc_worker_tfidf = get_worker(desc_vectorize_tfidf_thread, name='desc_vectorize_tfidf_thread',
                                   args=(train_df.item_description, train_df.brand_name, test_df.item_description,
                                         test_df.brand_name, desc_queue_tfidf))
    desc_worker_tfidf.start()

    desc_queue = Queue()
    desc_worker = get_worker(desc_thread, name='desc_thread',
                             args=(train_df.item_description, train_df.brand_name, test_df.item_description,
    ...
    (tr_x_name, tr_x_name_cnts, ts_x_name, ts_x_name_cnts, nn_x['name_seq'], nn_ts_x['name_seq'], len_dic['name_seq'],
     ...
     len_dic['brand_name_ind'], tr_x_name_tfidf, ts_x_name_tfidf) = name_thread(train_df['name'], train_df.brand_name,
                                                                                test_df['name'], test_df.brand_name)
    ...
    (tr_x_desc, ts_x_desc, tr_desc_cnts, ts_desc_cnts, nn_x['item_description_seq'], nn_ts_x['item_description_seq'],
     len_dic['item_description_seq']) = desc_queue.get()
    desc_queue.close()
    desc_worker.join()
    del desc_worker, desc_queue
    gc.collect()

    tr_x_desc_tfidf, ts_x_desc_tfidf = desc_queue_tfidf.get()
    desc_queue_tfidf.close()
    desc_worker_tfidf.join()
    del desc_queue_tfidf, desc_worker_tfidf
    gc.collect()
    ...
    del tr_x_brand_embeds, tr_x_brand_cnts, tr_x_cat_embeds, tr_cat_cnts, tr_x_name_cnts, tr_desc_cnts
    gc.collect()
    ...
    del ts_x_brand_embeds, ts_x_brand_cnts, ts_x_cat_embeds, ts_cat_cnts, ts_x_name_cnts, ts_desc_cnts
    gc.collect()
    ...
    x = hstack([tr_x_ship, tr_x_cond, tr_x_brand, tr_x_cat1, tr_x_cat2, tr_x_cat3, tr_x_cat_n], dtype=np.float32)
    del tr_x_ship, tr_x_cond, tr_x_brand, tr_x_cat1, tr_x_cat2, tr_x_cat3, tr_x_cat_n
    gc.collect()
    ts_x = hstack([ts_x_ship, ts_x_cond, ts_x_brand, ts_x_cat1, ts_x_cat2, ts_x_cat3, ts_x_cat_n], dtype=np.float32)
    del ts_x_ship, ts_x_cond, ts_x_brand, ts_x_cat1, ts_x_cat2, ts_x_cat3, ts_x_cat_n
    gc.collect()
    print_info('combine features')

    return (submission, y, x, ts_x, nn_x, nn_ts_x, len_dic, x_cnts, ts_x_cnts, tr_x_name, tr_x_desc, ts_x_name,
            ts_x_desc, tr_x_name_tfidf, ts_x_name_tfidf, tr_x_desc_tfidf, ts_x_desc_tfidf)
...
param_cache = {'Ridge-alpha': {"{'max_iter'- 200, 'tol'- 0.01, 'fit_intercept'- True}": {0.04: (0.4417291898413521, 6.838492222682735e-05), 0.08: (0.4401265137913611, 0.00029337520969502373), 0.2: (0.4360903252787709, 0.00010995482966133507), 0.4: (0.43275402258088097, 0.00032414672158986857), 0.25: (0.4349912223831699, 0.0001503199988870385), 0.3: (0.43405348656039033, 0.0004064403173041631), 0.35: (0.4334000376019157, 0.00039935329490964344), 0.45: (0.4321961385692882, 0.0004170824432457032), 0.5: (0.43179073344719743, 0.00043673712248323837), 0.55: (0.43142157961336935, 0.00042961782373804325), 0.6: (0.4311712977497829, 0.0003850259614192886), 0.575: (0.431267617448025, 0.00038097058145217244), 0.625: (0.4309741376061297, 0.0003886577920253764), 0.65: (0.4308932455586409, 0.000392370984376722), 0.675: (0.4308164810953352, 0.000395972870488176), 0.7: (0.4307058744502164, 0.0003616143435064001), 0.725: (0.430600224097119, 0.00040700159078504994), 0.75: (0.43054033333266717, 0.0004100833022734063), 0.775: (0.4304571620787083, 0.0003863507330851834), 0.8: (0.4303781776914789, 0.00041963400043823484), 0.825: (0.43033273532162586, 0.000422350271644667), 0.85: (0.4302722803769451, 0.0004070077558085694), 0.875: (0.4302346133656225, 0.00041148962927853305), 0.9: (0.43019967354361943, 0.0004158156368826438), 0.925: (0.4301393437037825, 0.0004241976684517812), 0.93046875: (0.43013340941627176, 0.0004250932113450123), 0.9609375: (0.4301024330657136, 0.0004299589069471643)}}, 'Ridge-max_iter': {"{'alpha'- 0.925, 'tol'- 0.01, 'fit_intercept'- True}": {10: (0.43723123221400628, 0.0037859889968414884), 20: (0.4307653118019053, 0.0003280974652825619), 40: (0.43099790066885985, 0.00021617193836764859), 80: (0.43099790066885985, 0.00021617193836764859), 160: (0.43099790066885985, 0.00021617193836764859)}}}


def run():
    data_dir = '../input'
    (submission, y, x, test_x, nn_x, nn_test_x, len_dic, x_cnts, ts_x_cnts, tr_x_name, tr_x_desc, ts_x_name, ts_x_desc,
     tr_x_name_tfidf, ts_x_name_tfidf, tr_x_desc_tfidf, ts_x_desc_tfidf) = get_data()
    gc.collect()
    del submission, test_x, nn_test_x, len_dic, x_cnts, ts_x_cnts, tr_x_name, tr_x_desc, ts_x_name, ts_x_desc, \
     ts_x_name_tfidf, ts_x_desc_tfidf
    gc.collect()
    
    x = hstack([x, tr_x_name_tfidf, tr_x_desc_tfidf, nn_x['cnts']]).tocsr()

    ridge = Ridge(solver='auto', fit_intercept=True, alpha=0.4, max_iter=200, normalize=False, tol=0.01, random_state=0)
    init_param = [('alpha', 0.4), ('max_iter', 200), ('tol', 0.01), ('fit_intercept', True)]
    param_dic = {'alpha': [4e-2, 8e-2, .2],
                 'max_iter': ['grid', 10, 20, 40, 80, 160, 200],
                 'tol': [8e-3, .02],
                 'fit_intercept': [True, False]}

    model = ridge

    tune(model, x, y, init_param, param_dic, measure_func=measure_handler, detail=True, kc=(2, 1),
         random_state=4000, max_optimization=False, score_min_gain=1.5e-3, factor_cache=param_cache)
