...
def desc_cnt_thread(tr_desc_col, ts_desc_col, tr_desc_queue):
    ...
    tr_desc_queue.put((tr_x_desc_cnts, ts_x_desc_cnts))
...
param_cache = {'LgbTrainer-learning_rate': {"{'num_leaves'- 88, 'max_depth'- 13, 'bagging_fraction'- 0.3, 'feature_fraction'- 0.9}": {0.1: (0.4305195946496796, 1.1161512610297075e-05), 0.15: (0.42783412455663544, 0.00014218364555479424), 0.2: (0.4285634654365076, 0.00017920320370520826), 0.4: (0.4459609608345677, 0.0002622372786683924), 0.8: (0.5137656318735548, 0.0004233197260755106), 0.1125: (0.4294156113300218, 0.00011482578794064224), 0.125: (0.4285604980273288, 6.432826212221521e-05), 0.1375: (0.428226885584177, 0.00010654756438086), 0.1625: (0.42777160708484124, 0.00038095592578246684), 0.175: (0.427761353923944, 0.00018211201414700115), 0.1875: (0.42815339399017716, 0.00017721932475672775)}}, 'LgbTrainer-num_leaves': {"{'learning_rate'- 0.175, 'max_depth'- 13, 'bagging_fraction'- 0.3, 'feature_fraction'- 0.9}": {8: (0.45482574024486755, 0.00040968051358938884), 16: (0.4412598689468249, 0.0004236134206655473), 32: (0.43208735304650753, 0.00018825161934038492)}}}
...
def run():
    ...
    del test_x, submission, x_cnts, test_x_cnts
    gc.collect()

    num_boost_round = 2700
    origin_params = {'objective': 'regression', 'metric': 'rmse', 'verbose': -1, 'nthread': 4,
                     'learning_rate': 0.1, 'num_leaves': 31, 'max_depth': 0, 'bagging_fraction': 0.8,
                     'feature_fraction': 0.8}
    lgb_model = LgbTrainer(n_round=num_boost_round, params=origin_params)

    init_param = [('learning_rate', 0.15), ('num_leaves', 88), ('max_depth', 13), ('bagging_fraction', 0.3), ('feature_fraction', 0.9)]
    param_dic = {'learning_rate': [.1, .2, .4, .8],
                 'num_leaves': [8, 16, 32, 64],
                 'max_depth': ['grid', 3, 7, 8, 9, 10, 11, 12, 13],
                 'min_data': [16, 32, 64, 128],
                 'bagging_fraction': ['grid', .3, .4, .5, .6, .7, .8, .9, 1.0],
                 'bagging_freq': [1, 2, 4],
                 'feature_fraction': ['grid', .3, .4, .5, .6, .7, .8, .9, 1.0],
    ...
    tune(lgb_model, x, y, init_param, param_dic, measure_func=measure_handler, detail=True, kc=(2, 1),
         random_state=3000, data_dir=data_dir, max_optimization=False, score_min_gain=1e-3)
