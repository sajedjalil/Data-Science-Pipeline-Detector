...
def item_cond_thread(tr_cond_col, ts_cond_col):
    ...
    rec_time('train %s process' % col_name)
    tr_x_cond = pd.get_dummies(tr_cond_col.astype(np.str), sparse=True).to_coo()
    print_info('train %s process' % col_name)

    rec_time('test %s process' % col_name)
    ts_cond_col = ts_cond_col.astype(np.uint8).clip(1, 5)
    ts_x_cond = pd.get_dummies(ts_cond_col.astype(np.str), sparse=True).to_coo()
    print_info('test %s process' % col_name)
...
def ship_thread(tr_ship_col, ts_ship_col):
    col_name = 'shipping'
    rec_time('train %s process' % col_name)
    tr_x_ship = pd.get_dummies(tr_ship_col.astype(np.str), sparse=True).to_coo()
    print_info('train %s process' % col_name)

    rec_time('test %s process' % col_name)
    ts_x_ship = pd.get_dummies(ts_ship_col.astype(np.str), sparse=True).to_coo()
    print_info('test %s process' % col_name)
...
def get_lgb_data(train_df, test_df, share_queue):
    ...
    # x = hstack([tr_x_ship, tr_x_cond, tr_x_brand_embeds, tr_x_brand_cnts, tr_x_brand, tr_x_cat_embeds, tr_cat_cnts,
    #             tr_x_cats, tr_name_cnts, tr_x_name, tr_desc_cnts, tr_x_desc])
    # ts_x = hstack([ts_x_ship, ts_x_cond, ts_x_brand_embeds, ts_x_brand_cnts, ts_x_brand, ts_x_cat_embeds, ts_cat_cnts,
    #                ts_x_cats, ts_name_cnts, ts_x_name, ts_desc_cnts, ts_x_desc])
...
def run_ols(x, y, test_x, x_cnts, ts_x_cnts):
    ...
    iind, oind = ShuffleSplit(n_splits=1, train_size=0.8, test_size=None, random_state=3).split(y).__next__()
    ix = x[iind]
    iy = y[iind]
    ox = x[oind]
    oy = y[oind]
    ...
    rec_time('ols validation')
    h_rmsle = measure_handler(oy, ols.predict(ox))
    print_info('ols validation', 'holdout rmsle: %s' % h_rmsle)
    del ox, oy
    gc.collect()
...
def run_fm(x, y, test_x, x_cnts, ts_x_cnts):
    rec_time('fm combine features')
    x.data = np.ones(x.data.shape)
    x = hstack([x, x_cnts]).tocsr()
    test_x.data = np.ones(test_x.data.shape)
    test_x = hstack([test_x, ts_x_cnts]).tocsr()
    print_info('fm combine features')
    del x_cnts, ts_x_cnts
    gc.collect()

    rec_time('fm insample_outsample_split')
    iind, oind = ShuffleSplit(n_splits=1, train_size=0.8, test_size=None, random_state=53).split(y).__next__()
    ix = x[iind]
    iy = y[iind]
    ox= x[oind]
    oy = y[oind]
    print_info('fm insample_outsample_split', ix.shape)

    rec_time('fm train')
    fm = FM_FTRL(alpha=0.01, beta=0.01, L1=0.00001, L2=0.1, D=ix.shape[1], alpha_fm=0.01, L2_fm=0.0, init_fm=0.01,
    ...
    # rec_time('fm embedding predict')
    # fm_px = fm.predict(x)
    # print_info('fm embedding predict')
    # del x, y
    # gc.collect()

    # rec_time('fm predict')
    # fm_p = fm.predict(test_x)
    # print_info('fm predict')
    # del test_x, fm
    # gc.collect()

    rec_time('fm validation')
    h_rmsle = measure_handler(oy, fm.predict(ox))
    print_info('fm validation', 'holdout rmsle: %s' % h_rmsle)
    del ox, oy
    gc.collect()
...
def run_lgb(x, y, rnn_px, fm_px, ols_px):
    rec_time('lgb train embedding')
    x = hstack([x, np.round(rnn_px), np.round(fm_px).reshape(-1, 1), np.round(ols_px).reshape(-1, 1)]).tocsr()
    print_info('lgb train embedding')
    del ols_px, rnn_px, fm_px
    gc.collect()
    ...
    rec_time('lgb train')
    lgb_model = lgb.train(params, lgb.Dataset(tx, label=ty), 2700)
...
def run_rnn(x, y, test_x, len_dic):
    ...
    print_info('tv split', '%s %s' % (ty.shape, vy.shape))
    del iind, tind, vind
    gc.collect()
    ...
    def get_rnn_model():
        ...
        cat_seq = Input(shape=[tx['category_name_seq'].shape[1]], name='category_name_seq')
        ...
        flat = concatenate([rnn_name_seq, rnn_desc_seq, rnn_cat_seq, cluster, Flatten()(emb_brand), Flatten()(emb_cat),
                            cond, ship, cnts])

        flat = Dropout(0.25)(Dense(256, activation='relu')(flat))
        # flat = Dropout(0.2)(Dense(16, activation='relu')(flat))
...
def run():
    ...
    del nn_x, nn_y, nn_test_x, len_dic
    gc.collect()
    ...
    fm_px, fm_p = run_fm(x, y, test_x, scaled_x_cnts, scaled_ts_x_cnts)
    del scaled_x_cnts, scaled_ts_x_cnts
    gc.collect()
    
    exit(0)

    lgb_model = run_lgb(x, y, rnn_px, fm_px, ols_px)
    del x, y, rnn_px, fm_px, ols_px
    gc.collect()

    rec_time('lgb test embedding')
    test_x = hstack([test_x, np.round(rnn_p), np.round(fm_p).reshape(-1, 1), np.round(ols_p).reshape(-1, 1)]).tocsr()
    ...
    submission.to_csv('lgb_rnn_fm_ols_100_0_0_0.csv', index=False)

    submission['price'] = np.expm1(rnn_p)
    submission.to_csv('lgb_rnn_fm_ols_0_100_0_0.csv', index=False)

    submission['price'] = np.expm1(fm_p)
    submission.to_csv('lgb_rnn_fm_ols_0_0_100_0.csv', index=False)

    submission['price'] = np.expm1(ols_p)
    submission.to_csv('lgb_rnn_fm_ols_0_0_0_100.csv', index=False)

    submission['price'] = np.expm1(0.82 * lgb_p + 0.07 * rnn_p + 0.1 * fm_p + 0.01 * ols_p)
    submission.to_csv('lgb_rnn_fm_ols_82_7_10_1.csv', index=False)

    submission['price'] = np.expm1(0.87 * lgb_p + 0.05 * rnn_p + 0.07 * fm_p + 0.01 * ols_p)
    submission.to_csv('lgb_rnn_fm_ols_87_5_7_1.csv', index=False)

    submission['price'] = np.expm1(0.92 * lgb_p + 0.03 * rnn_p + 0.04 * fm_p + 0.01 * ols_p)
    submission.to_csv('lgb_rnn_fm_ols_92_3_4_1.csv', index=False)
    print_info('save')
