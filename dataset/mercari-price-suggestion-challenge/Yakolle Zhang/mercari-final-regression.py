...
def read_data(data_dir='../input'):
    ...
    # test_df = test_df.append(test_df).append(test_df).append(test_df).append(test_df).append(test_df.iloc[:300000],
    #                                                                                          ignore_index=True)
...
def desc_vectorize_test_thread(data_id, vr, ts_desc_col, desc_queue=None):
    ...
    if desc_queue is not None:
        desc_queue.put((data_id, ts_x_desc))
    else:
        return data_id, ts_x_desc
...
def get_data():
    ...
    tr_cx = [scaled_x_cnts, tr_x_ship, tr_x_cond, tr_x_brand]
    del x_cnts, tr_x_name, tr_x_desc
    gc.collect()
    ...
    ts_cx = [scaled_ts_x_cnts, ts_x_ship, ts_x_cond, ts_x_brand]
    del ts_x_cnts, ts_x_name, ts_x_desc
    gc.collect()
    ...
    tr_cx += [tr_x_cat1, tr_x_cat2, tr_x_cat3, tr_x_cat_n]
    gc.collect()
    ...
    ts_cx += [ts_x_cat1, ts_x_cat2, ts_x_cat3, ts_x_cat_n]
    gc.collect()
    print_info('combine features 1', '%s %s' % (x.shape, ts_x.shape))

    return submission, y, x, ts_x, nn_x, nn_ts_x, len_dic, tr_cx, ts_cx


def get_tfidf_name_thread(tr_name_col, tr_brand_col, ts_name_col, ts_brand_col, name_queue):
    col_name = tr_name_col.name
    rec_time('train %s process' % col_name)
    tr_name_col = tr_name_col.str.replace(r'\s\s+', ' ')
    rec_time('train %s vectorize' % col_name)
    ...
    tr_x_name = tvr.fit_transform(tr_col)
    print_info('train %s vectorize' % col_name, tr_x_name.shape)
    print_info('train %s process' % col_name)
    del tr_col, tr_name_col, tr_brand_col
    gc.collect()
    ...
    ts_x_name = tvr.transform(ts_col)
    print_info('test %s vectorize' % col_name)
    print_info('test %s process' % col_name)
    del ts_col, ts_name_col, ts_brand_col, tvr
    gc.collect()

    name_queue.put((tr_x_name, ts_x_name))


def get_tfidf_desc_test_thread(vr, ts_col):
    col_name = 'item_description'
    rec_time('test %s vectorize' % col_name)
    ts_part_data_len = int(0.32 * ts_col.shape[0])
    rec_time('test %s vectorize transform' % col_name)

    if ts_col.shape[0] < max_data_size:
        ts_desc_queue = Queue()
        ...
        worker1.join()
        worker2.join()
        del worker1, worker2, ts_desc_queue
        gc.collect()
    else:
        rec_time('test part%d %s vectorize transform' % (0, col_name))
        ts_x_desc0 = vr.transform(ts_col[:ts_part_data_len])
        print_info('test part%d %s vectorize transform' % (0, col_name))
        data_id_12, ts_x_desc_12 = desc_vectorize_test_thread(1, vr, ts_col[ts_part_data_len: 2 * ts_part_data_len])
        data_id_21, ts_x_desc_21 = desc_vectorize_test_thread(2, vr, ts_col[2 * ts_part_data_len:])
    if 1 == data_id_12:
        ts_x_desc = vstack((ts_x_desc0, ts_x_desc_12, ts_x_desc_21))
    else:
        ts_x_desc = vstack((ts_x_desc0, ts_x_desc_21, ts_x_desc_12))
    print_info('test %s vectorize transform' % col_name)
    del ts_x_desc_12, ts_x_desc_21
    gc.collect()

    return ts_x_desc


def get_tfidf_desc_thread(tr_desc_col, tr_brand_col):
    col_name = tr_desc_col.name
    rec_time('train %s vectorize' % col_name)
...
def get_ridge_data(tr_cx):
    ...
    name_queue = Queue()
    name_worker = get_worker(get_tfidf_name_thread, name='get_tfidf_name_thread',
                             args=(train_df['name'], train_df.brand_name, test_df['name'],
                                   test_df.brand_name, name_queue))
    name_worker.start()
    del train_df['name'], test_df['name']
    gc.collect()

    vr, tr_x_desc = get_tfidf_desc_thread(train_df.item_description, train_df.brand_name)
    ts_desc_col = test_df.brand_name + ' ' + test_df.item_description
    del train_df, test_df
    gc.collect()

    tr_x_name, ts_x_name = name_queue.get()
    name_queue.close()
    name_worker.join()
    del name_queue, name_worker
    gc.collect()

    rec_time('combine train features')
    x = hstack(tr_cx + [tr_x_name, tr_x_desc], dtype=np.float32).tocsr()
    print_info('combine train features')
    del tr_cx, tr_x_name, tr_x_desc
    gc.collect()

    return x, ts_x_name, vr, ts_desc_col
...
def run_lgb(x, y, rnn_px):
    rec_time('lgb train embedding')
    x = hstack([x, rnn_px], dtype=np.float32).tocsr()
    ...
    params = {'objective': 'regression', 'metric': 'rmse', 'verbose': -1, 'nthread': 4,
              'learning_rate': 0.175, 'num_leaves': 64, 'max_depth': 13, 'bagging_fraction': 0.8, 'bagging_freq': 1,
              'feature_fraction': 0.9, 'lambda_l1': 1.6, 'lambda_l2': 0.05}
    rec_time('lgb train')
    lgb_model = lgb.train(params, lgb.Dataset(tx, label=ty), 3400)
...
def run_rnn(x, y, test_x, len_dic):
    ...
    iind, _ = ShuffleSplit(n_splits=1, train_size=0.8, test_size=None, random_state=85).split(y).__next__()
    ...
    print_info('rnn tv split', '%s %s' % (ty.shape, vy.shape))
    del iind, tind, vind
    gc.collect()
    ...
    def get_rnn_model():
        ...
        cluster = Dense(64, activation='relu')(cluster)

        flat = concatenate([rnn_name_seq, rnn_desc_seq, cluster, Flatten()(emb_brand), Flatten()(emb_cat),
                            cond, ship, cnts])

        flat = Dense(256, activation='relu')(flat)
        output = Dense(1, activation='relu')(flat)

        rnn = Model([name_seq, desc_seq, brand, cat, cond, ship, cnts], output)
        rnn.compile(loss='mse', optimizer=Adam())
        return rnn

    epochs = 2
    batch_size = 4500
    steps = (ty.shape[0] // batch_size) * epochs
    lr_init, lr_fin = 9e-3, 4e-4
    ...
    rec_time('rnn train')
    rnn_model.fit(tx, ty, epochs=epochs, batch_size=batch_size, verbose=0)
...
def run_cnn(x, y, test_x, len_dic):
    ...
    def get_cnn_model():
        ...
        cluster = concatenate([Flatten()(emb_brand), Flatten()(emb_cat), cond, ship])
        cluster = Dense(64, activation='relu', kernel_initializer=glorot_normal(seed=540))(cluster)

        flat = concatenate([cnn_name_seq, cnn_desc_seq, cluster, Flatten()(emb_brand), Flatten()(emb_cat),
                            cond, ship, cnts])

        flat = Dense(256, activation='relu', kernel_initializer=glorot_normal(seed=541))(flat)
        output = Dense(1, activation='relu')(flat)
    ...
    backend.set_value(cnn_model.optimizer.lr, lr_init)
    backend.set_value(cnn_model.optimizer.decay, lr_decay)

    rec_time('cnn train')
    cnn_model.fit(tx, ty, epochs=epochs, batch_size=batch_size, verbose=0)
...
def run():
    submission, y, x, test_x, nn_x, nn_test_x, len_dic, tr_cx, ts_cx = get_data()
    gc.collect()

    rnn_px, rnn_p = run_rnn(nn_x, y, nn_test_x, len_dic)
    cnn_p = run_cnn(nn_x, y, nn_test_x, len_dic)
    del nn_x['cnts'], nn_test_x['cnts'], nn_x, nn_test_x, len_dic
    gc.collect()

    lgb_model = run_lgb(x, y, rnn_px)
    del x, rnn_px
    gc.collect()

    rec_time('lgb test embedding')
    if test_x.shape[0] < max_data_size:
        test_x = hstack([test_x, rnn_p], dtype=np.float32).tocsr()
    else:
        test_x = combine_features([test_x, rnn_p])
    gc.collect()
    ...
    x, ts_x_name, vr, ts_desc_col = get_ridge_data(tr_cx)
    del tr_cx
    gc.collect()

    rec_time('ridge train')
    ridge = Ridge(alpha=0.925, max_iter=20, tol=0.01, random_state=0)
    ridge.fit(x, y)
    print_info('ridge train')
    del x, y
    gc.collect()

    ts_x_desc = get_tfidf_desc_test_thread(vr, ts_desc_col)
    del vr, ts_desc_col
    gc.collect()

    rec_time('combine test features')
    if ts_x_name.shape[0] < max_data_size:
        test_x = hstack(ts_cx + [ts_x_name, ts_x_desc], dtype=np.float32).tocsr()
    else:
        test_x = combine_features(ts_cx + [ts_x_name, ts_x_desc])
    print_info('combine test features')
    del ts_cx, ts_x_name, ts_x_desc
    gc.collect()

    ridge_p = ridge.predict(test_x)
    del ridge, test_x
    gc.collect()

    rec_time('save')
    submission['price'] = np.expm1(lgb_p)
    submission.to_csv('lgb_rnn_cnn_ridge_100_0_0_0.csv', index=False)

    submission['price'] = np.expm1(rnn_p)
    submission.to_csv('lgb_rnn_cnn_ridge_0_100_0_0.csv', index=False)

    submission['price'] = np.expm1(cnn_p)
    submission.to_csv('lgb_rnn_cnn_ridge_0_0_100_0.csv', index=False)

    submission['price'] = np.expm1(ridge_p)
    submission.to_csv('lgb_rnn_cnn_ridge_0_0_0_100.csv', index=False)

    submission['price'] = np.expm1(0.55 * lgb_p + 0.01 * rnn_p + 0.12 * cnn_p + 0.32 * ridge_p)
    submission.to_csv('lgb_rnn_cnn_ridge_55_1_12_32.csv', index=False)

    submission['price'] = np.expm1(0.49 * lgb_p + 0.01 * rnn_p + 0.1 * cnn_p + 0.4 * ridge_p)
    submission.to_csv('lgb_rnn_cnn_ridge_49_1_10_40.csv', index=False)

    submission['price'] = np.expm1(0.52 * lgb_p + 0.01 * rnn_p + 0.11 * cnn_p + 0.36 * ridge_p)
    submission.to_csv('lgb_rnn_cnn_ridge_52_1_11_36.csv', index=False)
    print_info('save')
