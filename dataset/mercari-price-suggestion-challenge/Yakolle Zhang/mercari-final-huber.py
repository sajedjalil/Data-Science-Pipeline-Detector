...
def run_ols(x, y, test_x, x_cnts, ts_x_cnts):
    ...
    x = hstack([x, x_cnts], dtype=np.float32).tocsr()
    test_x = hstack([test_x, ts_x_cnts], dtype=np.float32).tocsr()
...
def run_lgb(x, y, rnn_px):
    ...
    params = {'objective': 'huber', 'metric': 'rmse', 'verbose': -1, 'nthread': 5,
              'learning_rate': 0.1703125, 'alpha': 0.9, 'num_leaves': 64, 'max_depth': 13, 'bagging_fraction': 0.9,
              'bagging_freq': 1, 'feature_fraction': 0.8, 'lambda_l1': 0.8, 'lambda_l2': 0.01}
    ...
    rec_time('lgb train')
    lgb_model = lgb.train(params, lgb.Dataset(tx, label=ty), 4600)
...
def run():
    ...
    submission.to_csv('lgb_rnn_cnn_100_0_0.csv', index=False)

    submission['price'] = np.expm1(rnn_p)
    submission.to_csv('lgb_rnn_cnn_0_100_0.csv', index=False)

    submission['price'] = np.expm1(cnn_p)
    submission.to_csv('lgb_rnn_cnn_0_0_100.csv', index=False)

    submission['price'] = np.expm1(0.84 * lgb_p + 0.01 * rnn_p + 0.15 * cnn_p)
    submission.to_csv('lgb_rnn_cnn_84_1_15.csv', index=False)

    submission['price'] = np.expm1(0.82 * lgb_p + 0.01 * rnn_p + 0.17 * cnn_p)
    submission.to_csv('lgb_rnn_cnn_82_1_17.csv', index=False)

    submission['price'] = np.expm1(0.8 * lgb_p + 0.01 * rnn_p + 0.19 * cnn_p)
    submission.to_csv('lgb_rnn_cnn_80_1_19.csv', index=False)
    print_info('save')
