...
def print_info(title, message=None):
    if mode:
        ...
        cur_time = int(time())
        if message is None:
            print('【%ds】【%s】' % (cur_time - last_time, title))
        else:
            print('【%ds】【%s】【%s】' % (cur_time - last_time, title, message))
...
def embed_target_agg(col, statistical_size=30, k=5, random_state=0):
    if col.shape[0] < statistical_size:
        ...
        return np.mean(means), np.std(means), np.mean(stds), np.std(stds), np.mean(mins), np.std(mins), np.mean(
            q1s), np.std(q1s), np.mean(medians), np.std(medians), np.mean(q3s), np.std(q3s), np.mean(maxes), np.std(
            maxes)
...
def desc_vectorize_thread(tr_desc_col, ts_desc_col, tr_brand_col, ts_brand_col, desc_queue):
    col_name = 'item_description'
    rec_time('train %s vectorize fit_transform' % col_name)
    ...
    print_info('train %s vectorize fit_transform' % col_name, 'desc vocabulary size is %d.' % len(desc_terms))
...
def name_cnt_thread(tr_name_col, ts_name_col):
    col_name = 'name'
    rec_time('encode_text %s' % col_name)
    tr_x_name_cnts = encode_text(tr_name_col)
    ts_x_name_cnts = encode_text(ts_name_col)
    print_info('encode_text %s' % col_name)
    return tr_x_name_cnts, ts_x_name_cnts


def name_thread(tr_name_col, ts_name_col, tr_brand_col, ts_brand_col, name_queue):
    col_name = 'name'
    ...
    col = tr_brand_col + ' ' + tr_col
    tr_x_name = vr.fit_transform(col)
    ts_x_name = vr.transform(ts_brand_col + ' ' + ts_col)
    print_info('%s vectorize' % col_name, 'name vocabulary size is %d.' % len(name_terms))
    print_info('%s process' % col_name)
    name_queue.put((tr_x_name, ts_x_name))


def desc_cnt_thread(tr_desc_col, ts_desc_col, desc_queue):
    rec_time('encode_text desc')
    tr_x_desc_cnts = encode_text(tr_desc_col)
    ts_x_desc_cnts = encode_text(ts_desc_col)
    print_info('encode_text desc')
    desc_queue.put((tr_x_desc_cnts, ts_x_desc_cnts))


def brand_thread(tr_brand_col, ts_brand_col):
    col_name = 'brand_name'
    rec_time('%s vectorize' % col_name)
    vr = CountVectorizer(token_pattern='.+', min_df=10)
...
def cat_thread(tr_cat_col, ts_cat_col):
    ...
    vr = CountVectorizer(token_pattern='.+', min_df=30)
    col_name = 'cat1'
    rec_time('%s vectorize' % col_name)
    ...
    return (np.vstack([tr_cat_len, tr_cat_entity_cnt]).T, np.vstack([ts_cat_len, ts_cat_entity_cnt]).T), (
        tr_x_cats, ts_x_cats), (tr_cat_entity, ts_cat_entity)
...
def item_cond_thread(tr_cond_col, ts_cond_col):
    ...
    rec_time('%s process' % col_name)
    tr_col = tr_cond_col.astype(str)
    ts_col = ts_cond_col.astype(str)
    rec_time('%s vectorize' % col_name)
    vr = CountVectorizer(token_pattern='\d+')
    tr_x_cond = vr.fit_transform(tr_col)
    ts_x_cond = vr.transform(ts_col)
    print_info('%s vectorize' % col_name)
    print_info('%s process' % col_name)
...
def ship_thread(tr_ship_col, ts_ship_col):
    col_name = 'shipping'
    rec_time('%s process' % col_name)
    tr_col = tr_ship_col.astype(str)
    ts_col = ts_ship_col.astype(str)
    rec_time('%s vectorize' % col_name)
    vr = CountVectorizer(token_pattern='\d+')
    tr_x_ship = vr.fit_transform(tr_col)
    ts_x_ship = vr.transform(ts_col)
    print_info('%s vectorize' % col_name)
    print_info('%s process' % col_name)
...
def get_lgb_data(train_df, test_df):
    col_name = 'brand_name'
    rec_time('%s process' % col_name)
    ...
    other_queue = Queue()
    other_worker = get_worker(name_thread, name='name_thread', args=(
        train_df['name'], test_df['name'], train_df.brand_name, test_df.brand_name, other_queue))
    other_worker.start()
    ...
    train_df[col_name] = train_df[col_name].str.replace(r'\s\s+', ' ')
    test_df[col_name] = test_df[col_name].str.replace(r'\s\s+', ' ')
    print_info('%s normalize' % col_name)

    desc_queue = Queue()
    desc_worker = get_worker(desc_vectorize_thread, name='desc_vectorize_thread', args=(
        train_df.item_description, test_df.item_description, train_df.brand_name, test_df.brand_name, desc_queue))
    desc_worker.start()

    tr_x_name_cnts, ts_x_name_cnts = name_cnt_thread(train_df.name, test_df.name)
    tr_x_ship, ts_x_ship = ship_thread(train_df.shipping, test_df.shipping)

    tr_x_name, ts_x_name = other_queue.get()
    other_worker.join()
    ...
    (tr_x_cat_cnts, ts_x_cat_cnts), (tr_x_cats, ts_x_cats), (tr_cat_entity, ts_cat_entity) = cat_thread(
        train_df.category_name, test_df.category_name)
    train_df['cat_entity'] = tr_cat_entity
    test_df['cat_entity'] = ts_cat_entity
    ...
    tr_x_cat_embeds, ts_x_cat_embeds = embed_thread(train_df, test_df, 'cat_entity', ['item_condition_id', 'shipping'],
                                                    random_state=5000)
    tr_x_cat_embeds = tr_x_cat_embeds.astype(np.float16)
    ts_x_cat_embeds = ts_x_cat_embeds.astype(np.float16)
    tr_x_brand_embeds, ts_x_brand_embeds = embed_thread(train_df, test_df, 'brand_name',
                                                        ['item_condition_id', 'shipping'], random_state=5001)
    tr_x_brand_embeds = tr_x_brand_embeds.astype(np.float16)
    ts_x_brand_embeds = ts_x_brand_embeds.astype(np.float16)

    tr_x_desc_cnts, ts_x_desc_cnts = other_queue.get()
    other_queue.close()
    other_worker.join()
    ...
    x_cnts = {'brand_embeds': tr_x_brand_embeds, 'brand_cnts': tr_x_brand_cnts, 'cat_embeds': tr_x_cat_embeds,
              'cat_cnts': tr_x_cat_cnts, 'name_cnts': tr_x_name_cnts, 'desc_cnts': tr_x_desc_cnts}
    ts_x_cnts = {'brand_embeds': ts_x_brand_embeds, 'brand_cnts': ts_x_brand_cnts, 'cat_embeds': ts_x_cat_embeds,
                 'cat_cnts': ts_x_cat_cnts, 'name_cnts': ts_x_name_cnts, 'desc_cnts': ts_x_desc_cnts}
    x = hstack([tr_x_brand_embeds, tr_x_brand_cnts, tr_x_cat_embeds, tr_x_cat_cnts, tr_x_name_cnts, tr_x_desc_cnts,
                tr_x_ship, tr_x_cond, tr_x_cats, tr_x_brand, tr_x_name, tr_x_desc])
    ts_x = hstack([ts_x_brand_embeds, ts_x_brand_cnts, ts_x_cat_embeds, ts_x_cat_cnts, ts_x_name_cnts, ts_x_desc_cnts,
                   ts_x_ship, ts_x_cond, ts_x_cats, ts_x_brand, ts_x_name, ts_x_desc])
    print_info('combine features')

    rec_time('tocsr')
    x, target, ts_x, sub = x.tocsr(), train_df.price.copy().values, ts_x.tocsr(), test_df[['test_id']]
    print_info('tocsr', '%s %s' % (x.shape, ts_x.shape))
    return x, target, ts_x, sub, x_cnts, ts_x_cnts
...
def batch_predict(model, data, batch_num=1):
    p = []
...
def ols_blend_train(x, y, test_x, ols_queue):
    rec_time('blending ols insample_outsample_split')
    ix, iy, holdout_data_set = insample_outsample_split(x, y, train_size=0.8, random_state=83)
    print_info('blending ols insample_outsample_split', ix.shape)

    rec_time('blending ols train')
    ols = LinearRegression(copy_X=False)
    ols.fit(ix, iy)
    print_info('blending ols train')

    rec_time('blending ols holdout data score')
    h_scores = []
    for hx, hy in holdout_data_set:
        h_scores.append(measure_handler(hy, ols.predict(hx)))
    print_info('blending ols holdout data score', h_scores)
    print('blending ols holdout_mean=%s, holdout_std=%s' % (np.mean(h_scores), np.std(h_scores)))

    ols_p = ols.predict(test_x)
    del x, y, test_x, ols, ix, iy, holdout_data_set
    gc.collect()
    ols_queue.put(ols_p)


def run_lgb(train_df, test_df):
    x, y, test_x, submission, x_cnts, test_x_cnts = get_lgb_data(train_df, test_df)
    gc.collect()

    rec_time('lgb insample_outsample_split')
    iind, oind = ShuffleSplit(n_splits=1, train_size=0.9, test_size=None, random_state=53).split(y).__next__()
    ix = x[iind]
    iy = y[iind]
    # ox = x[oind]
    # oy = y[oind]
    # print_info('lgb insample_outsample_split', '%s %s' % (ix.shape, ox.shape))
    ...
    # vx = ix[tsize:]
    # vy = iy[tsize:]
    # print_info('lgb train valid split', '%s %s' % (tx.shape, vx.shape))
    ...
    params = {'objective': 'regression', 'metric': 'rmse', 'verbose': -1, 'nthread': 4,
              'learning_rate': 0.15, 'num_leaves': 72, 'max_depth': 13, 'bagging_fraction': 0.3,
              'feature_fraction': 0.9}
    rec_time('lgb train')
    lgb_model = lgb.train(params, lgb.Dataset(tx, label=ty), 2800)
    print_info('lgb train')
    # lgb_model = lgb.train(params, lgb.Dataset(tx, label=ty), 3000, valid_sets=[lgb.Dataset(vx, label=vy)],
    #                       early_stopping_rounds=1000, verbose_eval=1000)
    # print_info('lgb train', 'Best iteration: %d' % lgb_model.best_iteration)
    # del tx, ty, vx, vy
    del tx, ty
    gc.collect()

    # rec_time('lgb score holdout data')
    # holdout_num = 5
    # holdout_frac = 0.7
    # holdout_scores = []
    # for i in range(holdout_num):
    #     _, hind = ShuffleSplit(n_splits=1, train_size=None, test_size=holdout_frac, random_state=53 + i).split(
    #         oy).__next__()
    #     hx = ox[hind]
    #     hy = oy[hind]
    #     holdout_scores.append(measure_handler(hy, lgb_model.predict(hx)))
    # print_info('lgb score holdout data', holdout_scores)
    # print('lgb holdout data scores',
    #       'holdout_mean=%s, holdout_std=%s' % (np.mean(holdout_scores), np.std(holdout_scores)))
    ...
    del lgb_model
    gc.collect()
    return lgb_p, submission, x, y, test_x, x_cnts, test_x_cnts


def get_rnn_data(train_df, test_df, rnn_queue):
    len_dic = {}
    ler = LabelEncoder()
    cols = ['category_name', 'brand_name']
    for col_name in cols:
        rec_time('%s label encode' % col_name)
        ler.fit(np.hstack([train_df[col_name], test_df[col_name]]))
        ind_col_name = col_name + '_ind'
        train_df[ind_col_name] = ler.transform(train_df[col_name])
        test_df[ind_col_name] = ler.transform(test_df[col_name])
        len_dic[ind_col_name] = ler.classes_.shape[0]
        print_info('%s label encode' % col_name)
    del ler

    tkr = Tokenizer()
    cols = ['category_name', 'item_description', 'name']
    for col_name in cols:
        rec_time('%s fit tokenizer' % col_name)
        tkr.fit_on_texts(train_df[col_name])
        seq_col_name = col_name + '_seq'
        train_df[seq_col_name] = tkr.texts_to_sequences(train_df[col_name])
        test_df[seq_col_name] = tkr.texts_to_sequences(test_df[col_name])
        len_dic[seq_col_name] = len(tkr.word_index) + 1
        print_info('%s fit tokenizer' % col_name)
    del tkr

    def to_keras_data(df):
        return {
            'name_seq': pad_sequences(df.name_seq, maxlen=20, truncating='post', padding='post'),
            'desc_seq': pad_sequences(df.item_description_seq, maxlen=65, truncating='post', padding='post'),
            'cat_seq': pad_sequences(df.category_name_seq, maxlen=10, truncating='post', padding='post'),
            'brand': df.brand_name_ind.copy().values,
            'cat': df.category_name_ind.copy().values,
            'cond': df.item_condition_id.astype(np.uint8).values,
            'ship': df.shipping.astype(np.uint8).values
        }

    rec_time('split & to keras data')
    in_ind, out_ind = ShuffleSplit(n_splits=1, train_size=0.9, test_size=None, random_state=253).split(
        train_df).__next__()
    y = train_df.price.copy().values
    ix = train_df.take(in_ind)
    iy = y[in_ind]

    train_size = int(0.99 * ix.shape[0])
    tx = to_keras_data(ix.iloc[:train_size])
    ty = iy[:train_size]
    vx = to_keras_data(ix.iloc[train_size:])
    vy = iy[train_size:]

    # ox = to_keras_data(train_df.take(out_ind))
    # oy = y[out_ind]
    ox = vx
    oy = vy

    test_x = to_keras_data(test_df)
    print_info('split & to keras data', '%s %s %s %s' % (ix.shape, oy.shape, ty.shape, vy.shape))

    rec_time('calc embed input lens')
    len_dic['cond'] = np.max([train_df.item_condition_id.max(), test_df.item_condition_id.max()]) + 1
    print_info('calc embed input lens', len_dic)
    rnn_queue.put(((tx, ty, vx, vy, ox, oy, test_x), len_dic, (in_ind, out_ind)))


def combine_cnts(x_cnts, test_x_cnts):
    rec_time('combine cnts')
    x_cnt_list = []
    ts_x_cnt_list = []
    for col_name in ['brand_embeds', 'brand_cnts', 'cat_embeds', 'cat_cnts', 'name_cnts', 'desc_cnts']:
        x_cnt_list.append(x_cnts[col_name])
        ts_x_cnt_list.append(test_x_cnts[col_name])
    x_cnts = np.hstack(x_cnt_list)
    test_x_cnts = np.hstack(ts_x_cnt_list)
    print_info('combine cnts')
    rec_time('scale cnts')
    scaler = MinMaxScaler()
    x_cnts = scaler.fit_transform(x_cnts)
    test_x_cnts = scaler.transform(test_x_cnts)
    print_info('scale cnts')
    return x_cnts, test_x_cnts


def rnn_merge_data(tx, vx, ox, test_x, x_cnts, test_x_cnts, inds):
    in_ind, out_ind = inds
    ix_cnts = x_cnts[in_ind]
    train_size = tx['ship'].shape[0]
    tx['cnts'] = ix_cnts[:train_size]
    vx['cnts'] = ix_cnts[train_size:]
    ox['cnts'] = x_cnts[out_ind]
    test_x['cnts'] = test_x_cnts
    return tx, vx, ox, test_x


def run_rnn(data, len_dic, inds, x_cnts, test_x_cnts):
    tx, ty, vx, vy, ox, oy, test_x = data
    x_cnts, test_x_cnts = combine_cnts(x_cnts, test_x_cnts)
    tx, vx, ox, test_x = rnn_merge_data(tx, vx, ox, test_x, x_cnts, test_x_cnts, inds)
    del x_cnts, test_x_cnts, inds
    gc.collect()

    def get_rnn_model():
        name_seq = Input(shape=[tx['name_seq'].shape[1]], name='name_seq')
        desc_seq = Input(shape=[tx['desc_seq'].shape[1]], name='desc_seq')
        cat_seq = Input(shape=[tx['cat_seq'].shape[1]], name='cat_seq')
        ...
        emb_desc_seq = Embedding(len_dic['item_description_seq'], 60)(desc_seq)
        emb_cat_seq = Embedding(len_dic['category_name_seq'], 20)(cat_seq)
        emb_brand = Embedding(len_dic['brand_name_ind'], 10)(brand)
        emb_cat = Embedding(len_dic['category_name_ind'], 10)(cat)
        emb_cond = Embedding(len_dic['cond'], 5)(cond)

        rnn_name_seq = GRU(8)(emb_name_seq)
        rnn_desc_seq = GRU(16)(emb_desc_seq)
        rnn_cat_seq = GRU(8)(emb_cat_seq)

        flat = concatenate([rnn_name_seq, rnn_desc_seq, rnn_cat_seq, Flatten()(emb_brand), Flatten()(emb_cat),
                            Flatten()(emb_cond), ship, cnts])

        flat = Dropout(0.25)(Dense(512, activation='relu')(flat))
        flat = Dropout(0.2)(Dense(64, activation='relu')(flat))
        output = Dense(1, activation='linear')(flat)

        rnn = Model([name_seq, desc_seq, cat_seq, brand, cat, cond, ship, cnts], output)
        rnn.compile(loss='mse', optimizer=Adam())
        return rnn

    epochs = 2
    batch_size = 5000
    steps = (tx['ship'].shape[0] // batch_size) * epochs
    lr_init, lr_fin = 9e-3, 4.5e-4
    lr_decay = (lr_init / lr_fin) ** (1 / (steps - 1))

    rnn_model = get_rnn_model()
    backend.set_value(rnn_model.optimizer.lr, lr_init)
    backend.set_value(rnn_model.optimizer.decay, lr_decay)

    class Heartbeating(Callback):
        def on_batch_end(self, batch, logs={}):
            if not np.random.randint(10):
                print('rnn(%s), memory(%s), cpu(%s)' % (
                    batch, psutil.virtual_memory().percent, psutil.cpu_percent(0)))

    hb = Heartbeating()

    rec_time('rnn train')
    rnn_model.fit(tx, ty, epochs=epochs, batch_size=batch_size, verbose=0)  # 0, callbacks=[hb])
    ...
    # v_rmsle = measure_handler(oy, rnn_model.predict(ox))
    # print_info('rnn holdout score', 'holdout rmsle: %s' % v_rmsle)
...
def run():
    ...
    rnn_queue = Queue()
    rnn_worker = get_worker(func=get_rnn_data, args=(train_df, test_df, rnn_queue), name='get_rnn_data')
    rnn_worker.start()

    lgb_p, submission, x, y, test_x, x_cnts, test_x_cnts = run_lgb(train_df, test_df)
    del train_df, test_df
    gc.collect()

    data, lens, inds = rnn_queue.get()
    rnn_queue.close()
    rnn_worker.join()
    del rnn_worker, rnn_queue
    gc.collect()

    ols_queue = Queue()
    ols_worker = get_worker(func=ols_blend_train, args=(x, y, test_x, ols_queue), name='ols_blend_train')
    ols_worker.start()

    rnn_p = run_rnn(data, lens, inds, x_cnts, test_x_cnts).reshape(-1)
    del data, lens, inds
    gc.collect()
    ...
    ols_queue.close()
    ols_worker.join()
    del ols_worker, ols_queue, x, y, test_x, x_cnts, test_x_cnts
    gc.collect()

    rec_time('save')
    submission['price'] = np.expm1(lgb_p)
    submission.to_csv('lgb_ols_rnn_100_0_0.csv', index=False)

    submission['price'] = np.expm1(ols_p)
    submission.to_csv('lgb_ols_rnn_0_100_0.csv', index=False)

    submission['price'] = np.expm1(rnn_p)
    submission.to_csv('lgb_ols_rnn_0_0_100.csv', index=False)

    submission['price'] = np.expm1(0.87 * lgb_p + 0.03 * ols_p + 0.1 * rnn_p)
    submission.to_csv('lgb_ols_rnn_87_3_10.csv', index=False)

    submission['price'] = np.expm1(0.72 * lgb_p + 0.03 * ols_p + 0.25 * rnn_p)
    submission.to_csv('lgb_ols_rnn_72_3_25.csv', index=False)

    submission['price'] = np.expm1(0.57 * lgb_p + 0.03 * ols_p + 0.4 * rnn_p)
    submission.to_csv('lgb_ols_rnn_57_3_40.csv', index=False)
    print_info('save')
