...
def item_cond_thread(tr_cond_col, ts_cond_col):
    ...
    rec_time('train %s vectorize' % col_name)
    vr = CountVectorizer(token_pattern='\d+', binary=True, dtype=np.uint8)
...
def run_ols(x, y, iind, oind, test_x):
    rec_time('ols train split')
    tind = iind[:int(0.67 * iind.shape[0])]
    tx = x[tind]
    ty = y[tind]
    ox = x[oind]
    print_info('ols train split', tx.shape)
...
def run_lgb(x, y, iind, oind, test_x):
    rec_time('lgb train split')
    tind = iind[int(0.33 * iind.shape[0]):]
    tx = x[tind]
    ty = y[tind]
    ox = x[oind]
    print_info('lgb train split', tx.shape)
    ...
    rec_time('lgb train')
    lgb_model = lgb.train(params, lgb.Dataset(tx, label=ty), 3300)
    print_info('lgb train')
    del tx, ty
    gc.collect()

    rec_time('lgb stacking predict')
    lgb_px = batch_predict(lgb_model, ox, batch_num=10)
    print_info('lgb stacking predict')
...
def run_rnn(x, y, iind, oind, test_x, len_dic):
    rec_time('rnn train split')
    tind = iind[:int(0.33 * iind.shape[0])]
    tind = np.append(tind, iind[int(0.67 * iind.shape[0]):])
    ty = y[tind]
    tx = {}
    ox = {}
    ...
    print_info('rnn train split', ty.shape)
    gc.collect()
    ...
    rnn = get_rnn_model()
    backend.set_value(rnn.optimizer.lr, lr_init)
    backend.set_value(rnn.optimizer.decay, lr_decay)

    rec_time('rnn train')
    rnn.fit(tx, ty, epochs=epochs, batch_size=batch_size, verbose=0)
    print_info('rnn train')
    del tx, ty
    gc.collect()

    batch_size = 70000
    rec_time('rnn stacking predict')
    rnn_px = rnn.predict(ox, batch_size)
    print_info('rnn stacking predict')

    rec_time('rnn predict')
    rnn_p = rnn.predict(test_x, batch_size)
...
def run():
    ...
    iind, oind = ShuffleSplit(n_splits=1, train_size=0.75, test_size=None, random_state=853).split(y).__next__()
    oy = y[oind]
    ...
    rnn_px, rnn_p = run_rnn(nn_x, nn_y, iind, oind, nn_test_x, len_dic)
    del nn_x, nn_y, nn_test_x, len_dic, scaled_x_cnts, scaled_ts_x_cnts
    gc.collect()

    rec_time('ols & lgb combine features')
    x = hstack([x, x_cnts]).tocsr()
    test_x = hstack([test_x, ts_x_cnts]).tocsr()
    print_info('ols & lgb combine features')
    del x_cnts, ts_x_cnts
    gc.collect()

    ols_px, ols_p = run_ols(x, y, iind, oind, test_x)
    lgb_px, lgb_p = run_lgb(x, y, iind, oind, test_x)
    # del x, y, test_x
    # gc.collect()

    rec_time('stacking prepare data')
    ox = hstack([x[oind], lgb_px.reshape(-1, 1), ols_px.reshape(-1, 1), rnn_px]).tocsr()
    train_size = int(0.9 * oy.shape[0])
    tx = ox[:train_size]
    ty = oy[:train_size]
    vx = ox[train_size:]
    vy = oy[train_size:]
    test_x = hstack([test_x, lgb_p.reshape(-1, 1), ols_p.reshape(-1, 1), rnn_p]).tocsr()
    print_info('stacking prepare data')
    del ox, oy
    gc.collect()

    rec_time('stacker train')
    params = {'objective': 'regression', 'metric': 'rmse', 'verbose': -1, 'nthread': 5,
              'learning_rate': 0.1, 'num_leaves': 64, 'max_depth': 8, 'bagging_fraction': 0.9, 'bagging_freq': 1,
              'feature_fraction': 0.9}
    stacker = lgb.train(params, lgb.Dataset(tx, label=ty), 3000, valid_sets=[lgb.Dataset(vx, label=vy)],
                        early_stopping_rounds=500, verbose_eval=500)
    print_info('stacker train', 'Best iteration: %d' % stacker.best_iteration)
    del tx, ty, vx, vy
    gc.collect()

    rec_time('stacker predict')
    p = stacker.predict(test_x)
    print_info('stacker predict')
    del test_x
    gc.collect()
    ...
    submission.to_csv('stacker_lgb_ols_rnn_100_0_0_0.csv', index=False)

    submission['price'] = np.expm1(lgb_p)
    submission.to_csv('stacker_lgb_ols_rnn_0_100_0_0_0.csv', index=False)

    submission['price'] = np.expm1(ols_p)
    submission.to_csv('stacker_lgb_ols_rnn_0_0_100_0.csv', index=False)

    submission['price'] = np.expm1(rnn_p)
    submission.to_csv('stacker_lgb_ols_rnn_0_0_0_100.csv', index=False)

    submission['price'] = np.expm1(0.87 * p + 0.1 * lgb_p + 0.01 * ols_p + 0.02 * rnn_p)
    submission.to_csv('stacker_lgb_ols_rnn_87_10_1_2.csv', index=False)

    submission['price'] = np.expm1(0.82 * p + 0.14 * lgb_p + 0.01 * ols_p + 0.03 * rnn_p)
    submission.to_csv('stacker_lgb_ols_rnn_82_14_1_3.csv', index=False)

    submission['price'] = np.expm1(0.92 * p + 0.05 * lgb_p + 0.01 * ols_p + 0.02 * rnn_p)
    submission.to_csv('stacker_lgb_ols_rnn_92_5_1_2.csv', index=False)
    print_info('save')
