...
NUM_BRANDS = (train.append(test).brand_name.value_counts().values==1).sum()
NUM_CATEGORIES = (train.append(test).category_name.value_counts().values==1).sum()

print("NUM_BRANDS:[{}]; NUM_CATEGORIES:[{}]".format(NUM_BRANDS,NUM_CATEGORIES))
NAME_MIN_DF = 10
MAX_FEATURES_ITEM_DESCRIPTION = 200000
...
def main():
    ...
    train_X, valid_X, train_y, valid_y = train_test_split(X, y, test_size = 0.12, random_state = 144) 
    ...
    train_X2, valid_X2, train_y2, valid_y2 = train_test_split(X, y, test_size = 0.12, random_state = 107) 
    d_train2 = lgb.Dataset(train_X2, label=train_y2, max_bin=8192)
    d_valid2 = lgb.Dataset(valid_X2, label=valid_y2, max_bin=8192)
    watchlist2 = [d_train2, d_valid2]
    del train_X2, valid_X2
    ...
    model = lgb.train(params3, train_set=d_train2, num_boost_round=3000, valid_sets=watchlist2, \
    early_stopping_rounds=300, verbose_eval=300) 
    predsL3 = model.predict(X_test)

    print('[{}] Predict lgb 3 completed.'.format(time.time() - start_time))
    

    preds = (predsR*0.15 + predsL*0.25 + predsL2*0.25 + predsL3*0.2 + predsR2*0.15)
