...
if develop:
    ...
    ## Ensemble
    preds = preds_rnn_ensemble.flatten()*0.5 + preds_fm * 0.5
    print("FM + RNN Ensemble RMSLE:", rmsle(np.expm1(dev_y), np.expm1(preds)))
else:
    test_submission = []
    batch_num = 0
    for test in pd.read_csv('../input/test.tsv', sep='\t', chunksize=700000):
    ## Pre-processing
        batch_num += 1
        ## Pre-processing
        ...
        rnn_epoch1 = rnn_model()
        rnn_epoch1.load_weights("/tmp/weights.01.hdf5")
        preds_epoch2 = rnn_epoch2.predict(test_X, batch_size = 70000)
        preds_epoch1 = rnn_epoch1.predict(test_X, batch_size = 70000)
        preds_rnn_ensemble = preds_rnn * 0.55 + preds_epoch2 * 0.25 + preds_epoch1 * 0.2
        ...
        test_submission.append(submission)
    if len(test_submission) > 1:
        test_submission = pd.concat(test_submission)
        test_submission.to_csv("submission.csv", index = False)
    else:
        test_submission[0].to_csv("submission.csv", index = False)