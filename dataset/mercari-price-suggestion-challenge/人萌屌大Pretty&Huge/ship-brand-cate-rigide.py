...
print("intercept encoders")
vect_brand = LabelBinarizer(sparse_output=True)
X_shipping = vect_brand.fit_transform(df["intercept"])

print("shipping encoders")
vect_brand = LabelBinarizer(sparse_output=True)
X_shipping = vect_brand.fit_transform(df["shipping"])
...
#model = MLPRegressor(alpha=0.005, hidden_layer_sizes = (10,20,5), max_iter = 10, 
...
df_test["price"] = [abs(x) for x in np.expm1(preds)]
