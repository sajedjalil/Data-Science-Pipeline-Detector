...
cols_to_keep = [k for k, v in n_docs.items() if v > .0005 * X.shape[0]]
...
def baseline_model():
    model = Sequential()
    model.add(Dense(1000, input_dim=X_filtered.shape[1], kernel_initializer="normal", activation="relu"))
...
submission_df.to_csv("006_bigrams_keras.csv", index=False)