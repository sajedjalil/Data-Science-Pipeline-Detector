...
def main():
    ...
    for ii in range(4):
        ...
        train_X, valid_X, train_y, valid_y = train_test_split(X, y, test_size = 0.15, random_state = 33*ii+112) 
        ...
        predsL0 = model.predict(X_test)
        predsL += 0.25*predsL0
    
        print('[{}] Predict lgb {} completed.'.format(time.time() - start_time, ii+1))


    submission['price'] = np.expm1(predsL)
    submission.to_csv("submission_lgbm_5.csv", index=False)
