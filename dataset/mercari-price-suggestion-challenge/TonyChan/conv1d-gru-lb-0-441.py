...
with graph.as_default():
    ...
    #name = conv1d(name, num_filters=12, filter_size=3)
    ...
    rnn_layer2 = GRU(8) (cat)
    rnn_layer3 = GRU(8) (name)
 
    main_l = concatenate([rnn_layer2, rnn_layer3])
    ...
    out = tf.concat([main_l, desc, brand, cat, ship, cond], axis=1)
    #out = tf.concat([desc, brand, cat, ship, cond], axis=1)
...
predsConv = y_pred * std + mean
preds = predsConv
submission['price'] = np.expm1(preds)
submission.to_csv("TCsubmission.csv", index=False)