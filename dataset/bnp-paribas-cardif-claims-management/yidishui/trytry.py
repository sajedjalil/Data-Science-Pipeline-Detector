    ...
    #  Return array of unique values in the object. Significantly faster than numpy.unique. Includes NA values.
        ...
        if (typ=='flo') or (typ=='int'):
            ...
            if (typ=='obj'):
                ...
                tempTable['weight']=tempTable.apply(lambda x: .5+.5*x['sum']/x['count'] if (x['sum']>0.5*x['count']) else .5+.5*(x['sum']-x['count'])/x['count'], axis=1)
...
X, Y, Xtarget, Ytarget=cross_validation.train_test_split(train, target, test_size=0.3,random_state=1)
from sklearn import tree 
clf=tree.DecisionTreeClassifier(criterion='entropy',min_samples_leaf=5,max_features='sqrt')
clf.fit(X,Xtarget)

print("决策树准确率：{:.16f}".format(clf.score(X,Xtarget)))
print("决策树训练集：{:.16f}".format(clf.score(Y,Ytarget)))
