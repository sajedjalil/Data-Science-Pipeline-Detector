...
train = pd.melt(train[list(train.columns[-100:])+['Page']], id_vars='Page', var_name='date', value_name='Visits')
...
#only recent dates (added by aloisio)

print("max", max( train.date), "min", min(train.date))

train_all = train.loc[train.date <=  np.datetime64('2016-10-31')]

train_all.loc[train_all.Visits.isnull(), 'Visits']=0

print("train: ", len(train_all) )
print(train_all.columns)

test = train.loc[train.date >  np.datetime64('2016-10-31')]

train_page_per_dow = train_all.groupby(['Page','ferie']).median().reset_index()


train_page_per_dow.columns = ['Page', 'ferie', 'Pred']
print("dow", len(train_page_per_dow))
print(train_page_per_dow.columns)
