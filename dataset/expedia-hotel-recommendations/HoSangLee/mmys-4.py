...
all = fread("./submission_2016-06-04-14-36.csv",header=TRUE)


#all$sub2 = fread("./submission_combo_merge.csv",header=TRUE)$hotel_cluster

#all = as.data.frame(all)

#submission <- data.frame(id=all$id)

#replaceLast<-function(x) {
#best5 = as.integer(strsplit(x[2]," ")[[1]])
#toReplace = as.integer(strsplit(x[3]," ")[[1]][1])
#found = FALSE
#for (i in 1:4) {
#if (best5[i]==toReplace) {
#found = TRUE
#break
#}
#}
...
#best5[5] = toReplace
#}
#paste(best5[1:5],collapse=" ")
#}

#submission$hotel_cluster = apply(all,1,replaceLast)
#cat("Total Update: ",totalUpdate,"\n")
#write.csv(submission, file='hs_stacking.csv', row.names=FALSE)