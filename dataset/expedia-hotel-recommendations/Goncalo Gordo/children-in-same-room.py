...
def prepare_arrays_match():
    ...
    while total < 38000000:
        ...
        user_id = arr[7]
        is_package = arr[9]
        kids_in_room = 1 if int(arr[14]) > 0 and int(arr[15]) < 2 else 0
        ...
        if srch_destination_id != '' and hotel_country != '' and hotel_market != '':
            s2 = (srch_destination_id,hotel_country,hotel_market,is_package, kids_in_room)
...
def gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster):
    ...
    while 1:
        ...
        user_id = arr[8]
        is_package = arr[10]
        kids_in_room = 1 if int(arr[15]) > 0 and int(arr[16]) < 2 else 0
