...
def prepare_arrays_match():
    ...
    while 1:
        ...
        if arr[11] != '' and arr[0] != '':
            in_month = int(arr[11][5:7])
            in_day = int(arr[11][8:9])
            b_month = int(arr[0][5:7])
            b_day = int(arr[0][8:9])
        with_time = 0 if (in_day - b_day) > 4 or (in_month - b_month) > 1 or [(in_month - b_month) > -11 and (in_month - b_month) < 0] or [[(in_month - b_month) == -11 or (in_month - b_month) == 1] and (in_day - (b_day - 31)) > 4] else 1
        ...
        if srch_destination_id != '' and hotel_country != '' and hotel_market != '':
            s2 = (srch_destination_id,hotel_country,hotel_market,is_package,with_time)
...
def gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster):
    ...
    while 1:
        ...
        if arr[12] != '' and arr[1] != '':
            in_month = int(arr[12][5:7])
            in_day = int(arr[12][8:9])
            b_month = int(arr[1][5:7])
            b_day = int(arr[1][8:9])
        with_time = 0 if (in_day - b_day) > 4 or (in_month - b_month) > 1 or [(in_month - b_month) > -11 and (in_month - b_month) < 0] or [[(in_month - b_month) == -11 or (in_month - b_month) == 1] and (in_day - (b_day - 31)) > 4] else 1
