# coding: utf-8
# forked originally from 'Ravi: https://kaggle.com/company'
...
def prepare_arrays_match():
    ...
    best_hotels_search_dest_kids = dict()
    best_hotels_search_dest_nokids = dict()
    ...
    while 1:
        ...
        if srch_destination_id != '' and hotel_country != '' and hotel_market != '' and kids == 1:
            s2 = (srch_destination_id,hotel_country,hotel_market)
            if s2 in best_hotels_search_dest_kids:
                if hotel_cluster in best_hotels_search_dest_kids[s2]:
                    best_hotels_search_dest_kids[s2][hotel_cluster] += append_1
                else:
                    best_hotels_search_dest_kids[s2][hotel_cluster] = append_1
            else:
                best_hotels_search_dest_kids[s2] = dict()
                best_hotels_search_dest_kids[s2][hotel_cluster] = append_1
       
        if srch_destination_id != '' and hotel_country != '' and hotel_market != '' and kids == 0:
            s5 = (srch_destination_id,hotel_country,hotel_market,is_package)
            if s5 in best_hotels_search_dest_nokids:
                if hotel_cluster in best_hotels_search_dest_nokids[s5]:
                    best_hotels_search_dest_nokids[s5][hotel_cluster] += append_1
                else:
                    best_hotels_search_dest_nokids[s5][hotel_cluster] = append_1
            else:
                best_hotels_search_dest_nokids[s5] = dict()
                best_hotels_search_dest_nokids[s5][hotel_cluster] = append_1
    ...
    f.close()
    return best_s00,best_s01, best_hotels_country, best_hotels_od_ulc, best_hotels_uid_miss, best_hotels_search_dest_kids, best_hotels_search_dest_nokids, popular_hotel_cluster


def gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest_kids, best_hotels_search_dest_nokids, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster):
    ...
    while 1:
        ...
        s2 = (srch_destination_id,hotel_country,hotel_market)
        if s2 in best_hotels_search_dest_kids:
            d = best_hotels_search_dest_kids[s2]
        ...
        s5 = (srch_destination_id,hotel_country,hotel_market,is_package)
        if s5 in best_hotels_search_dest_nokids:
            d = best_hotels_search_dest_nokids[s5]
            topitems = nlargest(5, d.items(), key=itemgetter(1))
            for i in range(len(topitems)):
                if topitems[i][0] in filled:
                    continue
...
best_s00,best_s01,best_hotels_country, best_hotels_od_ulc, best_hotels_uid_miss, best_hotels_search_dest_kids, best_hotels_search_dest_nokids, popular_hotel_cluster = prepare_arrays_match()
gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest_kids, best_hotels_search_dest_nokids, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster)
