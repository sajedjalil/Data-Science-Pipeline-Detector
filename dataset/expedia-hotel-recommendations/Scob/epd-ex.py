...
def prepare_arrays_match():
    ...
    while 1:
        ...
        if arr[11] != '' and arr[12] != '':
            ...
            book_day = int(arr[11][8:])
            co_year = int(arr[12][:4])
            co_month = int(arr[12][5:7])
            co_day = int(arr[12][8:])
            delta = (co_year - book_year)*365 + (co_month - book_month)*30 + (co_day - book_day)
            if delta < 2:
                los = 2
            elif delta < 4:
                los = 4
...
def gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster):
    ...
    while 1:
        ...
        if arr[12] != '' and arr[13] != '':
            book_year = int(arr[12][:4])
            book_month = int(arr[12][5:7])
            book_day = int(arr[12][8:])
            co_year = int(arr[13][:4])
            co_month = int(arr[13][5:7])
            co_day = int(arr[13][8:])
            delta = (co_year - book_year)*365 + (co_month - book_month)*30 + (co_day - book_day)
            if delta < 2:
                los = 2
