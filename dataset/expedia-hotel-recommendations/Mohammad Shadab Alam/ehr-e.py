...
def run_solution():
    ...
    best_hotel_uid = defaultdict(lambda: defaultdict(int))
    ...
    while 1:
        ...
        append_1 = append_0 *  (3 + 12*is_booking)
        ...
        if user_id != '':
            best_hotel_uid[user_id][hotel_cluster] += append_2
        if hotel_country != '':
    ...
    while 1:
        ...
        if user_id in best_hotel_uid:
            d = best_hotel_uid[hotel_country]
            topitems = nlargest(5, d.items(), key=itemgetter(1))
            for i in range(len(topitems)):
                if len(filled) == 5:
                    break
