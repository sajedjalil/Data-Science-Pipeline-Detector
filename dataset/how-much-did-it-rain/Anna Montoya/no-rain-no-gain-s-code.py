...
def plotFig(out_file_name, cumulative=True):
	...
	train = pd.read_csv("train_2013.csv", usecols=["RR1","Expected"])
	...
	RR1_perc_list = [-0.01,0] + np.percentile(RR1[np.where(RR1>0.00000)[0]], xrange(2,101,1) )
	...
	rain_perc_list = [-1000] + range(70) + [max(train["Expected"])]	
	...
	for i,val in enumerate(np.unique(dig_RR1)):
		...
		for j in xrange(1,len(rain_perc_list)):
