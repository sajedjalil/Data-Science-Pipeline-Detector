...
def print_memory_usage():
    print('this process is consuming {:.2f} GB RAM'.format(
...
def get_df(csv_path, n_rows=None, column_names=None):
    ...
    if column_names == ['ID'] or column_names == ['target']:
        date_columns = False
        dtype_map = {'ID': 'u32'} if column_names == ['ID'] else {'target': 'u8'}
    else:
...
def get_X_df(csv_file_path, n_rows_with_header=None):
    return get_df(csv_path=csv_file_path, n_rows=n_rows_with_header,
...
def get_train_y_values(train_csv_path='../input/train.csv', n_rows_with_caption=None):
    return get_df(csv_path=train_csv_path, n_rows=n_rows_with_caption,
                  column_names=['target']).values.ravel()
...
if __name__ == '__main__':
    ...
    train_X_df = get_train_X_df()
    print('train_X_df loaded in {:.4f} seconds'.format(time.perf_counter() - start_time))
    print_memory_usage()
    print('\n=== train_X_df.info() ===', flush=True)
    print(train_X_df.info(verbose=False, memory_usage = True, null_counts=True), flush=True)
    ...
    start_time = time.perf_counter()
    test_X_df = get_test_X_df()
    print('test_X_df loaded in {:.4f} seconds'.format(time.perf_counter() - start_time))
    print_memory_usage()
    print('\n=== test_X_df.info() ===', flush=True)
    print(test_X_df.info(verbose=False, memory_usage = True, null_counts=True), flush=True)
