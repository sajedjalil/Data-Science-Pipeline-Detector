...
print('original gc threshold: {}'.format(gc.get_threshold()))
gc.set_threshold(1, 1, 1)
print('modified gc threshold: {}'.format(gc.get_threshold()), flush=True)

import resource
import sys

rusage_maxrss_denominator = 1073741824 if sys.platform == 'darwin' else 1048576

def print_memory_usage():
    print('{:.2f}GB -- approximate memory usage so far'.format(
        (resource.getrusage(
            resource.RUSAGE_SELF).ru_maxrss +
         resource.getrusage(
             resource.RUSAGE_CHILDREN).ru_maxrss) /
        rusage_maxrss_denominator), flush=True)
...
from sklearn.feature_selection import SelectFpr, SelectPercentile, f_classif
...
def get_df(csv_path, n_rows=None, column_names=None):
    ...
    if column_names == ['ID'] or column_names == ['target']:
        ...
        date_columns = ['VAR_0073', 'VAR_0075',
...
def get_X_df(csv_file_path, n_rows_with_header=None):
    return get_df(csv_path=csv_file_path, n_rows=n_rows_with_header, column_names=[
...
def predict(batch_size=30000, temp_test_row_count_limit=None):
    ...
    for offset in range(0, total_row_count, slice_row_count):
        ...
        print('{} encoded'.format(X_test.shape))
    ...
    def transform(self, data_frame):
        if self.key == 'object':
            ...
            return data_frame.select_dtypes(
                include=['object']).fillna(-9.22337204e+18).values.astype(str)
        ...
        if self.key == 'number':
            ...
            return data_frame.select_dtypes(
                include=['number'],
                exclude=['datetime64']).fillna(-9.22337204e+18).values.astype('float32')
...
if __name__ == '__main__':
    ...
    feature_encoder = FeatureUnion(transformer_list=[
        ('datetime', Pipeline([
            ('separator', DTypeSelector(key='datetime')),
            ('filter', SelectPercentile(f_classif, percentile=14)),
        ])),
        ('object', Pipeline([
            ('separator', DTypeSelector(key='object')),
            ('encoder', FeatureHasher(input_type='string')),
            ('filter', VarianceThreshold(threshold=2)),
        ])),
        ('number', Pipeline([
            ('separator', DTypeSelector(key='number')),
            ('filter', SelectFpr(f_classif, alpha=0.02)),
        ])),
    ])
    ...
    clf = xgb.XGBClassifier(silent=1, nthread=-1,
                            objective='binary:logistic', n_estimators=300,
                            learning_rate=0.05, max_depth=7,
                            min_child_weight=0,
                            gamma=0.2, max_delta_step=0,
                            subsample=0.75, colsample_bytree=0.75,
                            # missing=-9.22337204e+18
                            )
    print('fitting', flush=True)
    clf.fit(X_train, y_train,
            eval_set=[(X_dev, y_dev)], eval_metric='logloss', early_stopping_rounds=1,
            verbose=True)
    sys.stderr.flush()
    print_memory_usage()
