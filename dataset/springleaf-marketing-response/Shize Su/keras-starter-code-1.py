...
def build_model(input_dim, output_dim):
    model = Sequential()
    model.add(Dense(input_dim, 64, init='lecun_uniform'))
    model.add(Activation('relu'))
    model.add(Dropout(0.5))

    model.add(Dense(64, 64, init='lecun_uniform'))
    model.add(Activation('relu'))
    model.add(Dropout(0.5))

    model.add(Dense(64, output_dim, init='lecun_uniform'))
...
if __name__ == "__main__":
    ...
    for train, valid in kfolds:
        ...
        model.fit(X_train, Y_train, nb_epoch=20, batch_size=16, validation_data=(X_valid, Y_valid), verbose=0)
    ...
    submission.to_csv('Keras_BTB_ssz.csv')    
