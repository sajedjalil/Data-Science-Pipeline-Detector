# Prediction of survival in the Titanic wreck using only names
# LSTM encoder of character sequences in names for Titanic survival prediction
import sys
import os
import copy
import math
...
#Read data
random_init(**args)
train_text = np.array([normalise(v) for v in pd.read_csv('../input/tabular-playground-series-apr-2021/train.csv').Name.values])
train_targets = np.array(pd.read_csv('../input/tabular-playground-series-apr-2021/train.csv').Survived.values)
test_text = [normalise(v) for v in pd.read_csv('../input/tabular-playground-series-apr-2021/test.csv').Name.values]

#Split in training and validation
idx = list(range(len(train_text)))
random.shuffle(idx)
valid_text = train_text[-int(len(train_text)*args['cv_percentage']):]
train_text = train_text[:-int(len(train_text)*args['cv_percentage'])]
valid_targets = train_targets[-int(len(train_targets)*args['cv_percentage']):]
train_targets = train_targets[:-int(len(train_targets)*args['cv_percentage'])]
...
#Write results
df_out = pd.DataFrame({'PassengerId': pd.read_csv('../input/tabular-playground-series-apr-2021/test.csv')['PassengerId'], 'Survived': (test_predictions >= best_th).astype(int)})       
