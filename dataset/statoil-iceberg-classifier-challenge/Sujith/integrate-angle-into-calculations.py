...
# frost filter
#   arr_band1[i] = frost_filter(arr_band1[i], damping_factor=k_value1, win_size=winsize)
    arr_band2[i] = np.reshape(np.array(train.iloc[i, 1]), (75, 75)) # green band
#   arr_band2[i] = frost_filter(arr_band2[i], damping_factor=k_value1, win_size=winsize)
...
model.fit(x_train, y_train, epochs=30,
          validation_data=(x_test, y_test), batch_size=64, callbacks=callbacks)
