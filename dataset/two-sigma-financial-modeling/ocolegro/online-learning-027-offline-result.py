...
def fit_derived_coeffs(reward,preds,a1,a2,b1,b2):
    fit_alpha,fit_beta,residual = 1e6,1e6,1e6
    if (reward == 0):
        reward = .000001
    for beta in [-1.,0,1.]:
        t1 = a1 + beta*b1
...
def convert(actions,ids,all_ids):
    d_ = {}
...
train_fit = train[train['timestamp'] < np.min(train['timestamp'])+250]
...
'''
for i in train['timestamp'].unique()[0:len(train['timestamp'].unique())-2]:
    subset = train[train['timestamp']== i]
    if (len(subset) == 0): continue

    subset_future = train[train['timestamp'] == i+1]
    if (len(subset_future) == 0): continue
    ...
    if (len(subset_future) == 0):
        continue

    for col in ['actions','derived_feat_better']:
        subset_future   = subset_future.loc[np.isfinite(subset_future[col]),:]

    if ( (len(subset_future['actions']) == len(subset_future['derived_feat_better_shifted_2']))
         and (len(subset_future['derived_feat_better']) == len(subset_future['derived_feat_better_shifted_2']))):
...
actions,prev_feats,prev_feats_derived_feat,derived_features_copy = [],[],[],[]
...
while True:
    ...
    if (len(features) > 0):
        if ('preds' in features.columns):
            fit_tuple = fit_derived_coeffs(reward,features['preds'],
            ...
            coefs,residual = update_derived_coeffs(coefs,fit_tuple)
        ...
        observation.target.y = convert(actions,features[['id']].as_matrix(),observation.target.id)
    ...
    if (len(derived_features_copy) != 0):
        prev_feats_derived_feat         = prev_feats.merge(derived_features_copy,on='id',suffixes=('','_shifted'))
