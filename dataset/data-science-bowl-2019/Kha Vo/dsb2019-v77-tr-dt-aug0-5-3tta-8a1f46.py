...
class TransfomerModel(nn.Module):
    ...
    def forward(self, cate_x, cont_x, mask):        
        ...
        print('\n\n', 'cate_x', cate_x.shape, 'cont_x', cont_x.shape, 'mask', mask.shape)
        
        cate_emb = self.cate_emb(cate_x).view(batch_size, self.cfg.seq_len, -1)
        print('cate_emb_1', cate_emb.shape)
        cate_emb = self.cate_proj(cate_emb)     
        print('cate_emb_2', cate_emb.shape)
        ...
        print('extended_attention_mask_1', extended_attention_mask.shape)
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        print('extended_attention_mask_2', extended_attention_mask.shape)
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        print('extended_attention_mask_3', extended_attention_mask.shape)
        head_mask = [None] * self.config.num_hidden_layers
        
        encoded_layers = self.encoder(seq_emb, extended_attention_mask, head_mask=head_mask)
        print('encoded_layers', [len(item) for item in encoded_layers])
        sequence_output = encoded_layers[-1]
        print('sequence_output_1', sequence_output.shape)
        sequence_output = sequence_output[:, -1]
        print('sequence_output_2', sequence_output.shape)
