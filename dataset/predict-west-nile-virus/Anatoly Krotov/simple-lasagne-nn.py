...
#from lasagne.utils import EarlyStopper, LinearAdjustVariable, TrainRatioStopper, BestIteratorSaver
...
def assemble_X(base, weather):
    ...
    for b in base:
        ...
        for days_ago in [0,1,2,3,5,8,12]:
            day = date - datetime.timedelta(days=days_ago)
            for obs in ["Tmax","Tmin","Tavg","DewPoint","WetBulb","PrecipTotal","Depart","Sunrise","Sunset","Heat","Cool","ResultSpeed"]:
...
def submit(net, mean, std):
    ...
    #
    out = csv.writer(open("west_nile_v694.csv", "w"))
