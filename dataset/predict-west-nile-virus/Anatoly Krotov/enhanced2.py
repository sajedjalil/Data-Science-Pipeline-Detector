...
def load_weather():
    ...
    for line in csv.DictReader(open("../input/weather.csv")):
        for name, converter in {"Date" : date,
                                ...
                                "PrecipTotal" : precip,"Sunrise" : precip,"Heat" : precip,"Cool" : precip,
                                "Depart" : float, 
...
def assemble_X(base, weather):
    ...
    for b in base:
        ...
        for days_ago in [0,1,2,3]:
            ...
            for obs in ["Tmax","Tmin","Tavg","DewPoint","WetBulb","PrecipTotal","Depart","Sunrise","Heat","Cool","ResultSpeed"]:
...
def submit(net, mean, std):
    ...
    #
    out = csv.writer(open("west_nile_v82.csv", "w"))
