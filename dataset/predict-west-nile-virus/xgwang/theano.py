...
def assemble_X(base, weather):
    ...
    for b in base:
        ...
        for days_ago in [1,5,9,14]:
            day = date - datetime.timedelta(days=days_ago)
            for obs in ["Tmax","Tmin","Tavg","DewPoint","Heat","Cool","WetBulb","PrecipTotal","Depart","AvgSpeed"]:
...
def submit(net, mean, std):
    ...
    #
    out = csv.writer(open("LB_Lasagne1.2.csv", "w"))
