...
def assemble_X(base, weather):
    ...
    for b in base:
        ...
        date5 = np.cos((2*3.1459265359*date.month)/365)
        date6 = np.sin((2*3.1459265359*date.weekday())/365)
        date7 = np.cos((2*3.1459265359*date.weekday())/365)
        lat, longi = b["Latitude"], b["Longitude"]
        case = [date.year, date.month,date4,date5,date6,date7, date.day,date.weekday(),date2,date3, lat, longi]
        ...
        for days_ago in [1,2,3,5,8,12]:
            ...
            for obs in ["Tmax","Tmin","Tavg","DewPoint","WetBulb","PrecipTotal","Depart","Sunrise","Sunset","Heat","Cool","ResultSpeed","ResultDir"]:
                ...
                #case.append(np.sin(2*3.14*weather[day][station][obs]))
                case.append(np.cos(2*3.14*weather[day][station][obs]))
...
def train():
    ...
    on_epoch_finished=[
            AdjustVariable(learning_rate, target=0, half_life=10),
            ],
