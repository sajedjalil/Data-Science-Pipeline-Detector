...
def gen_moisq(line, head=train_head):
    ...
    for line in train_dic[key]:
        dis = math.sqrt((float(line[train_idx3]) - lati) ** 2 + (float(line[train_idx4]) - logi) ** 2)
        res = int(line[idx])
        temp.append((dis, res))
...
fo = csv.writer(open("submission6.csv", "w"), lineterminator="\n")
fo.writerow(["Id","WnvPresent"])
i = 0
for item in ids:
    fo.writerow([ids[i], sigmoid(5*ypred[i])])
    #fo.writerow([ids[i], ypred[i]])
    i += 1
