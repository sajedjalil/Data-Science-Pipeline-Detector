...
def load_spray():
    spray = {}
    for l in csv.DictReader(open("../input/spray.csv")):
        for name, converter in {"Date": date}.items():
            spray[converter(l[name])] = 1
    return spray
...
def assemble_X(base, weather, spray):
    X = []
    for b in base:
        date = b["Date"]
        ...
        for days_ago in [0,-1,-2,-3]:
            day = date - datetime.timedelta(days=days_ago)
            if day in spray:
                case.append(float(1.0))
            else: case.append(float(0.0))
...
def train():
    ...
    spray = load_spray()
    X = assemble_X(training, weather, spray)
    ...
    hidden1_num_units=352, 
    dropout1_p=0.4,
    hidden2_num_units=352, 
    dropout2_p=0.4,
...
def submit(net, mean, std):
    ...
    spray = load_spray()
    X = assemble_X(testing, weather, spray)
    
    normalize(X, mean, std)
    predictions = net.predict_proba(X)[:,0]    
    #
    out = csv.writer(open("west_nile_v6.csv", "w"))
