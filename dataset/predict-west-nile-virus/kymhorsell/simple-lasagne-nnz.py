...
def assemble_X(base, weather):
    ...
    for b in base:
        ...
        for days_ago in [1,2,3,5,6,7,8,9,10,11,12]:
            day = date - datetime.timedelta(days=days_ago)
            for obs in ["Tmin","PrecipTotal","ResultSpeed","ResultDir"]:
...
def submit(net, mean, std):
    ...
    #
    out = csv.writer(open("west_nile_rkh.csv", "w"))
