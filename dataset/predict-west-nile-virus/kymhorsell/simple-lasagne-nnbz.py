...
def load_testing():
    ...
    for line in csv.DictReader(open("../input/test.csv")):
        for name, converter in {"Date" : date, 
                                "Latitude" : ll, "Longitude" : ll}.items(): #hack
            line[name] = converter(line[name])
        training.append(line)
    return training
...
def assemble_X(base, weather):
    ...
    for b in base:
        ...
        for days_ago in [0,1,3,5,7,9,11,13,15]: # hack
            day = date - datetime.timedelta(days=days_ago)
            for obs in ["Tmax","Tmin","Tavg","DewPoint","WetBulb","PrecipTotal","Depart","Cool","ResultSpeed","ResultDir"]:
