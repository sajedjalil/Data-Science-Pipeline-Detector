...
def gen_water(line, head=train_head):
    ...
    idx2 = head.index("Date")
    return tofloat(weather_dic[line[idx2]][idx1])
...
def gen_resultspeed(line, head=train_head):
    idx1 = weather_head.index("ResultSpeed")
    idx2 = head.index("Date")
    return tofloat(weather_dic[line[idx2]][idx1])
    
def gen_resultdir(line, head=train_head):
    idx1 = weather_head.index("ResultDir")
    idx2 = head.index("Date")
    return tofloat(weather_dic[line[idx2]][idx1])
...
for line in fi:
    ...
    Xtrain[i][11] = mos[2]
    Xtrain[i][12] = mos[3]
    Xtrain[i][13] = gen_resultspeed(line)
    Xtrain[i][14] = gen_resultdir(line)
...
for line in fi:
    ...
    Xtest[i][11] = mos[2]
    Xtest[i][12] = mos[3]
    Xtest[i][13] = gen_resultspeed(line, test_head)
    Xtest[i][14] = gen_resultdir(line, test_head)
    i += 1
