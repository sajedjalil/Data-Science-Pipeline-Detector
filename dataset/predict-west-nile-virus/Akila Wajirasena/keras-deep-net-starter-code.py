...
def process_line(line, indexes, weather_dic, weather_indexes):
    ...
    tmax = tofloat(weather_dic[date][weather_indexes["Tmax"]])
    tmin = tofloat(weather_dic[date][weather_indexes["Tmin"]])
    tavg = tofloat(weather_dic[date][weather_indexes["Tavg"]])
    dewpoint = tofloat(weather_dic[date][weather_indexes["DewPoint"]])
    wetbulb = tofloat(weather_dic[date][weather_indexes["WetBulb"]])
    pressure = tofloat(weather_dic[date][weather_indexes["StnPressure"]])
    depart = tofloat(weather_dic[date][weather_indexes["Depart"]])
    resultSpeed = tofloat(weather_dic[date][weather_indexes["ResultSpeed"]])
    resultDir = tofloat(weather_dic[date][weather_indexes["ResultDir"]])

    return [month, week, latitude, longitude, tmax, tmin, tavg, dewpoint, wetbulb, pressure, depart, resultSpeed, resultDir]
