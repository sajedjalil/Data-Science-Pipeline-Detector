...
y = np.array(get_fields(raw_train, label_fields), dtype=np.int32)
del raw_train
...
print('Input dimensions: {}'.format(in_dim))


def build_model(in_dim, output_dim, hn=32, dp=0.5, layers=2):
    model = Sequential()
    # model.add(Dense(input_dim, hn, init='glorot_uniform'))
    # model.add(Dense(hn, input_dim=in_dim))
    model.add(Dense(hn, input_shape=(in_dim,)))
    model.add(PReLU(input_shape=(hn,)))
    model.add(Dropout(dp))

    for i in range(layers):
    #   model.add(Dense(hn, hn, init='glorot_uniform'))
      model.add(Dense(hn, input_shape=(hn,)))
      model.add(PReLU(input_shape=(hn,)))
...
if RUN_FOLDS:
  for train, valid in kfolds:
      ...
      model = build_model(in_dim, output_dim, HN)
