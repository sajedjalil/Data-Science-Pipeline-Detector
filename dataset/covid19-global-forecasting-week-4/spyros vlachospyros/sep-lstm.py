...
W_C=data[['Province/State','Country/Region','Date','Confirmed','New Cases','New Confirmed pct','BreakOut']]
W_D=data[['Province/State','Country/Region','Date','Deaths','Death Rate','New Deaths','New Deaths pct','BreakOut']] #'New Cases','New Deaths'
W_C = np.array(W_C)
W_D = np.array(W_D)
T_C = []
T_D=[]
...
#CNN model 
model_C = Sequential()
#model_C.add(Conv1D(64,2,strides=1,padding='same',activation='softplus',input_shape=(t_window,7)))
model_C.add(Conv1D(32,3,strides=1,padding='same',activation='softplus',input_shape=(t_window,7)))
...
#LSTM model
#model.add(Flatten())
model_C.add(LSTM(64, activation='softplus',kernel_regularizer=regularizers.l2(0.0001)))
model_C.add(Dense(32,activation='softplus'))
#model_C.add(Dense(16,activation='softplus'))
...
history_C = model_C.fit(X_C,Y_C, epochs=400, verbose=1 , batch_size = 72,shuffle=False)
plot_loss(history_C)
#************************************************************************************
...
#CNN model 
model_D = Sequential()
#model_D.add(Conv1D(64,2,strides=1,activation='softplus',input_shape=(t_window,8)))
#model_D.add(Conv1D(32,3,strides=1,padding='same',activation='softplus',input_shape=(t_window,8)))
model_D.add(Conv1D(16,3,strides=1,padding='same',activation='softplus',input_shape=(t_window,8)))
#model.add(BatchNormalization())
...
#LSTM model
#model.add(Flatten())
model_D.add(LSTM(64, activation='softplus',kernel_regularizer=regularizers.l2(0.001)))#,recurrent_regularizer=regularizers.l2(0.001)))
...
#compile and train the model
history_D = model_D.fit(X_D,Y_D, epochs=350, verbose=1 , batch_size =168,shuffle=False)
plot_loss(history_D)
...
for i in range(1,country+1):
    ...
    kC_test['New Cases'][pos_k:pos_k+overlap] = data['New Cases'][pos_d:pos_d+overlap]
    kD_test['New Deaths'][pos_k:pos_k+overlap] = data['New Deaths'][pos_d:pos_d+overlap]
...
kC_test = kC_test[['Province_State','Country_Region','Date','Confirmed','New Confirmed pct','BreakOut','New Cases']]  
kD_test = kD_test[['Province_State','Country_Region','Date','Deaths','Death Rate','New Deaths pct','BreakOut','New Deaths']]   
...
for i in range(0,country):
    for k in range(overlap-t_window,43-t_window):
        ...
        kC_test[z+t_window,5] = kC_test[z+t_window,3] - kC_test[z+t_window-1,3] #new case confirmed
        ...
        kD_test[z+t_window,6] = kD_test[z+t_window,3] - kD_test[z+t_window-1,3] #new case dead
