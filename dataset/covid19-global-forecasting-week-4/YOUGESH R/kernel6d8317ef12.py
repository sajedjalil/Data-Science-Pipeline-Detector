...
for i in country:
    ...
    fatalitymodel=LinearRegression().fit(polynomial_features.fit_transform(pd.DataFrame(d["Date"])),d["Fatalities"])
...
for i in range(testx): #testx
    ...
    temp=test.iloc[i,1]
    print(temp)
    temp=polynomial_features.fit_transform([[temp]])
    casepred.append(int(mod1.predict(temp)))
    fatpred.append(int(mod2.predict(temp)))
...
"""model1=casepredmodel[78]
model2=fatalitypredmodel[78]
scaler=PolynomialFeatures(degree=3)
x=[i for i in range(85,150)]
x=pd.DataFrame(x)
x=scaler.fit_transform(x)
predc=model1.predict(x)
predf=model2.predict(x)
x=[i for i in range(85,150)]
plt.plot(x,predc)
plt.show()
plt.plot(x,predf)
...
for i in range(22000):
    if(data.iloc[i,0]=="India"):
        print(i)
        
m=data.iloc[10703:10780]
print(m)
plt.plot(x,predc)
plt.plot(m["Date"],m["ConfirmedCases"])
plt.show()"""
