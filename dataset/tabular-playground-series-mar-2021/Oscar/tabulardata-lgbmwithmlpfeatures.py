...
def evaluate_model(testset,predictor,return_embedding=False,**kwargs):
    testlen = testset.shape[0]
    nbatches = math.ceil(testlen/kwargs['batch_size'])
    if return_embedding:
        predictions = np.zeros((testlen,predictor.in_dim))
    else:
    ...
    with torch.no_grad():
        for b in range(nbatches):
            ...
            estimated = predictor(X,return_embedding)
...
lgb1 = LGBMClassifier(num_leaves=args_lgbm['num_leaves'],
                      max_depth=args_lgbm['max_depth'],
                      learning_rate=args_lgbm['learning_rate'],
                      n_estimators=args_lgbm['n_estimators'],
                      objective="binary")
lgb1.fit(trainset,traintargets)
val_pred_lgb1 = lgb1.predict_proba(validset)
test_pred_lgb1 = lgb1.predict_proba(testset)
auc = compute_roc_auc(val_pred_lgb1,validtargets)
print('LGBM original features AUC: {0:.5f}'.format(auc))

# MLP to extract features
random_init(**args)
...
trainset_emb = evaluate_model(trainset,predictor,return_embedding=True,**args)
validset_emb = evaluate_model(validset,predictor,return_embedding=True,**args)
testset_emb = evaluate_model(testset,predictor,return_embedding=True,**args)

# Second LGBM
random_init(**args)

lgb2 = LGBMClassifier(num_leaves=args_lgbm['num_leaves'],
                      max_depth=args_lgbm['max_depth'],
                      learning_rate=args_lgbm['learning_rate'],
                      n_estimators=args_lgbm['n_estimators'],
                      objective="binary")
lgb2.fit(trainset_emb,traintargets)
val_pred_lgb2 = lgb2.predict_proba(validset_emb)
test_pred_lgb2 = lgb2.predict_proba(testset_emb)
auc = compute_roc_auc(val_pred_lgb2,validtargets)
print('LGBM MLP features AUC: {0:.5f}'.format(auc))

# Combination
val_pred = (val_pred_lgb1 + val_pred_lgb2) / 2
test_pred = (test_pred_lgb1 + test_pred_lgb2) / 2
auc = compute_roc_auc(val_pred,validtargets)
print('Combination AUC: {0:.5f}'.format(auc))

out_df = pd.DataFrame(data={'id':test_data['id'],'target':test_pred[:,1]}).set_index('id',drop=True)
