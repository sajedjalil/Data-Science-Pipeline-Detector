...
def get_im_cv2(path):
    img = cv2.imread(path)
    resized = cv2.resize(img, (30, 30), cv2.INTER_LINEAR)
    return resized
...
def create_model():
    model = Sequential()
    model.add(ZeroPadding2D((1, 1), input_shape=(3, 30, 30), dim_ordering='th'))
...
def run_cross_validation_create_models(nfolds=15):
