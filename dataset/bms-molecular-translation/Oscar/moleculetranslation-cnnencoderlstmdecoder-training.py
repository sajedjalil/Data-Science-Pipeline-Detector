#Train encoder and decoder from scratch in PyTorch
import sys
import io
import os
import re
...
train_data['formula'] = [v.split('/')[1] for v in train_data['InChI'].values]
train_data['connections'] = [v.split('/')[2] for v in train_data['InChI'].values]

#Get target vocabulary
args['vocab'] = read_characters(train_data[args['targets']].values,**args)
args['inv_vocab'] = {args['vocab'][v]:v for v in args['vocab']}

#Randomize train and validation sets
train_data = train_data.sample(frac=1)
cv_data = train_data.iloc[-args['cv_samples']:]
train_data = train_data.iloc[:args['train_samples']]

#Build encoder, decoder, optimizer and criterion
encoder = CNNEncoder(**args).to(args['device'])
decoder = LSTMDecoder(**args).to(args['device'])
...
for i in range(100):
    st = int(i*args['train_samples']/100)
    en = min(int((i+1)*args['train_samples']/100),args['train_samples'])
    loss = train_model(train_data.iloc[st:en],encoder,decoder,optimizer,criterion,**args)
    samples_idx = sample_model(cv_data,encoder,decoder,**args)
    lev = [Levenshtein.distance(indexes_to_characters(samples_idx[:,j],**args),cv_data.iloc[j][args['targets']]) for j in range(len(cv_data))]
    print('Completed {0:d}%, training loss: {1:.2f}, Levenshtein distance: {2:.1f}'.format(i+1,loss,np.mean(lev)))

#Save encoder and decoder
encoder = encoder.cpu()
decoder = decoder.cpu()
torch.save(encoder,'cnn_encoder.pytorch')
torch.save(decoder,'lstm_decoder.pytorch')
