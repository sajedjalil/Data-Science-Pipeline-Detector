...
def GPIndividual1(data):
    predictions = (np.sinh(((((np.sinh(data["team1Seed"]) - data["team2Seed"]) + ((np.tanh(data["team2Wmax"]) + ((data["team1Lstd"] + np.minimum( (data["team1losses"]),  (data["year"])))/2.0))/2.0)) + ((data["team2Seed"] == (1.0/(1.0 + np.exp(- data["team2wins"])))).astype(float)))/2.0)) +
                    ...
                    ((np.floor(np.minimum( (((1.751 == data["team1Lmax"]).astype(float))),  (np.cos(data["team1WAverage"])))) == ((np.round(((((-(data["team2LAverage"])) <= data["team2losses"]).astype(float)) - 2.211120)) <= (data["team2Wmin"] * data["team2losses"])).astype(float))).astype(float)) +
                    ...
                    np.sin(np.sinh(((((-((((-(0.3879)) <= data["team1Lmax"]).astype(float)))) + ((data["team1Wmin"] >= np.floor(data["team1Seed"])).astype(float)))/2.0) - ((np.sin(data["team2Wstd"]) > (data["team2Wmax"] + np.abs(data["team2"]))).astype(float))))) +
                    ...
                    ((-(((data["team2Wmin"] >= ((-((data["team2Wstd"] + 0.39310))) * (1.0/(1.0 + np.exp(- (-((1.0/(1.0 + np.exp(- 0.317310)))))))))).astype(float)))) * (((1.0/(1.0 + np.exp(- data["year"]))) * data["team1Lmin"]) * data["team2Lstd"])) +
                    ...
                    ((((data["team2"] <= ((((data["team1Lmin"] + 5.916570)/2.0) + (np.cos(np.maximum( (data["team1Lmin"]),  (np.maximum( ((1.0/(1.0 + np.exp(- data["team1Wmin"])))),  (data["team2LAverage"]))))) / 2.0))/2.0)).astype(float)) <= ((((data["team2"] + 5.428570)/2.0) <= data["team1Wmedian"]).astype(float))).astype(float)) +
                    ...
                    np.minimum( (np.cos(data["team1"])),  (((((((((((data["team1"] / 2.0) / 2.0) * 9.908604) <= 0.058823).astype(float)) <= data["team1Wstd"]).astype(float)) == np.ceil(((data["team1"] / 2.0) * 9.869604))).astype(float)) - 0.094340))) +
                    ...
                    ((math.tanh((-(1.909430))) > np.sin(np.maximum( (data["team2Wmin"]),  (np.minimum( (np.minimum( (data["team2Seed"]),  (((data["team1LAverage"] + data["team1Wstd"])/2.0)))),  ((((data["team2Seed"] <= data["team2Wmin"]).astype(float)) - data["team2Lstd"]))))))).astype(float)) +
                    ...
                    ((data["team2Wmin"] > ((0.900310 + (((1.0/(1.0 + np.exp(- (((data["team2Seed"] * np.maximum( (data["team2"]),  (data["team1Lmax"]))) <= ((data["team2losses"] < data["team1Lmax"]).astype(float))).astype(float))))) < data["team2Wmin"]).astype(float))) * 2.0)).astype(float)) +
...
if __name__ == "__main__":
    ...
    print(log_loss(trainlabels, np.clip(predictions.values*1.0080035, 1e-6, 1-1e-6)))
    test[test.columns] = np.round(ss.transform(test), 4)
    predictions = GPIndividual1(test)
    predictions.fillna(1, inplace=True)
    submission = pd.DataFrame({'Id': testids,
                               'Pred': np.clip(predictions.values*1.0088, 1e-6, 1-1e-6)})
