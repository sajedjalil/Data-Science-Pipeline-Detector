...
if __name__ == "__main__":
    ...
    target = trainlabels
    print('Training...')
    extc = ExtraTreesClassifier(n_estimators=500,max_features=27,criterion= 'entropy',min_samples_split=3,
                                max_depth=4, min_samples_leaf=3, n_jobs = -1)      
    
    extc.fit(X_train,target) 
    x_pred = extc.predict_proba(X_train)
    print(log_loss(trainlabels, np.clip(x_pred[:,1]*1.0695, 1e-6, 1-1e-6)))
    print('Predict...')
    y_pred = extc.predict_proba(X_test)
    e_pred1=y_pred[:,1]
    
    extc = ExtraTreesClassifier(n_estimators=500,max_features=27,criterion= 'entropy',min_samples_split=3,
                                max_depth=5, min_samples_leaf=3, n_jobs = -1)      
    ...
    e_pred3=y_pred[:,1]
    #print y_pred
    #submission = pd.DataFrame({'Id': testids,'Pred': np.clip(y_pred[:,1]*1.0695, 1e-6, 1-1e-6)})
    ...
    for index, row in preds.iterrows():
        ...
        try:
            ...
            prediction[i] = prediction[i]*0.4+predictions.values[i]*0.0+e_pred1[i]*0.2+e_pred2[i]*0.2+e_pred3[i]*0.2
            print("Elo Good ",p[1]," ",p[2])
        except:
            print("Elo Fail ",p[1]," ",p[2])
            prediction[i] = predictions.values[i]*0.0+e_pred1[i]*0.3333+e_pred2[i]*0.3333+e_pred3[i]*0.3334
        i += 1
    
    preds['Pred'] = np.clip(prediction, 0.07, 0.91)
