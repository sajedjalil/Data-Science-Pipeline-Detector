...
def GPIndividual1(data):
    predictions = (np.sinh(((((np.sinh(data["team1Seed"]) - data["team2Seed"]) + ((np.tanh(data["team2Wmax"]) + ((data["team1Lstd"] + np.minimum( (data["team1losses"]),  (data["year"])))/2.0))/2.0)) + ((data["team2Seed"] == (1.0/(1.0 + np.exp(- data["team2wins"])))).astype(float)))/2.0)) +
                    ...
                    ((-(((data["team2Wmin"] >= ((-((data["team2Wstd"] + 0.518310))) * (1.0/(1.0 + np.exp(- (-((1.0/(1.0 + np.exp(- 0.318310)))))))))).astype(float)))) * (((1.0/(1.0 + np.exp(- data["year"]))) * data["team1Lmin"]) * data["team2Lstd"])) +
                    ...
                    np.sin(np.floor((0.367879 - (((((np.minimum( (data["team1LAverage"]),  (np.floor(data["team2WAverage"]))) == ((2.405090 < -3.0))).astype(float)) + (data["team2wins"] * np.sin(np.minimum( (data["team1Lmax"]),  (data["team2WAverage"])))))/2.0) / 2.0)))) +
...
if __name__ == "__main__":
    ...
    #for treshold in np.linspace(1e-6, 1e-5, 20):
    treshold = 1e-5
    print(treshold, log_loss(trainlabels, np.clip(predictions.values, treshold, 1-treshold)))
    test[test.columns] = np.round(ss.transform(test), 4)
    predictions = GPIndividual1(test)
    predictions.fillna(1, inplace=True)
    submission = pd.DataFrame({'Id': testids,
                              'Pred': np.clip(predictions.values, treshold, 1-treshold)})
