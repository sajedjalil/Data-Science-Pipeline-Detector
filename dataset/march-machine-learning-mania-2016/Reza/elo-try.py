...
train = pd.read_csv('../input/RegularSeasonCompactResults.csv')
...
prediction = np.zeros((preds.shape[0], 3))

for index, row in preds.iterrows():
    p = list(map(int, str.split(str(row['Id']), '_')))
    prediction[index, 0] = team[p[1]]
    prediction[index, 1] = team[p[2]]
    prediction[index, 2] = 0.5 + (team[p[1]] - team[p[2]]) / 550.0

preds['Pred'] = np.clip(prediction[:, 2], 0.01, 0.99)
preds.to_csv('Prediction.csv', index=False)

preds['T1'] = prediction[:, 0]
preds['T2'] = prediction[:, 1]
