...
def GPIndividual1(data):
    predictions = (np.sinh(((((np.sinh(data["team1Seed"]) - data["team2Seed"]) + ((np.tanh(data["team2Wmax"]) + ((data["team1Lstd"] + np.minimum( (data["team1losses"]),  (data["year"])))/2.0))/2.0)) + ((data["team2Seed"] == (1.0/(1.0 + np.exp(- data["team2wins"])))).astype(float)))/2.0)) +
                    ...
                    ((np.floor(np.minimum( (((1.732 == data["team1Lmax"]).astype(float))),  (np.cos(data["team1WAverage"])))) == ((np.round(((((-(data["team2LAverage"])) <= data["team2losses"]).astype(float)) - 2.212120)) <= (data["team2Wmin"] * data["team2losses"])).astype(float))).astype(float)) +
                    ...
                    np.maximum( (np.minimum( (data["team1Wmin"]),  (np.ceil(np.minimum( (np.minimum( (0.148462),  (((data["team1Seed"] >= data["team2Lmedian"]).astype(float))))),  (data["team2losses"])))))),  ((((-(np.maximum( (data["team2"]),  (data["team2Seed"])))) > ((data["team1losses"] < 1.414214).astype(float))).astype(float)))) +
                    ...
                    np.minimum( (np.cos(data["team1"])),  (((((((((((data["team1"] / 2.0) / 2.0) * 8.869604) <= 0.058823).astype(float)) <= data["team1Wstd"]).astype(float)) == np.ceil(((data["team1"] / 2.0) * 9.869604))).astype(float)) - 0.094340))) +
