...
#df_all.to_csv("df_all.csv")  #no need to keep reprocessing for further grid searches
...
param_grid = {'rfr__n_estimators' : list(range(40,45,1)), 'rfr__max_depth': list(range(5,8,1))}
model = grid_search.GridSearchCV(estimator = clf, param_grid = param_grid, n_jobs = -1, cv = 10, verbose = 100)
