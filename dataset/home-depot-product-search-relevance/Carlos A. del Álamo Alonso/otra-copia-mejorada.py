...
for i in range(len(df_attr)):
    ...
    #if (last == int(df_attr.product_uid[i])):
     #   d[last][1] += " " +str(df_attr.value[i]).replace('\t'," ")
    #else:
    #    d[last]=[int(df_attr.product_uid[i]),str(df_attr.value[i]).replace('\t'," ")]
    #last=int(df_attr.product_uid[i])

df_attr = pd.DataFrame.from_dict(d,orient='index')
df_attr.columns = ['product_uid','value_attr']
print(df_attr.head())
...
df_all['value_attr'] = df_all['value_attr'].map(lambda x:str_stem(x))
df_all['len_of_query'] = df_all['search_term'].map(lambda x:len(x.split())).astype(np.int64)
#df_all['len_of_title'] = df_all['product_title'].map(lambda x:len(x.split())).astype(np.int64)
#df_all['len_of_description'] = df_all['product_description'].map(lambda x:len(x.split())).astype(np.int64)
df_all['product_info'] = df_all['search_term']+"\t"+df_all['product_title'] +"\t"+df_all['product_description']+"\t"+df_all['value_attr']
...
df_all = df_all.drop(['search_term','product_title','product_description','product_info','value_attr'],axis=1)
df_all.head()
...
param_grid = {'rfr__n_estimators' : list(range(23,26,1)), 'rfr__max_depth': list(range(6,9,1))}
