...
param_grid = {'rfr__n_estimators' : list(range(250,350,25)),# 300 top
              'rfr__max_depth': list(range(7,9,1)), #list(range(7,8,1))
              'rfr__max_features' : [.65, .85]
            }
model = grid_search.GridSearchCV(estimator = clf, param_grid = param_grid, n_jobs = -1, cv = 3, verbose = 150, scoring=RMSE)
