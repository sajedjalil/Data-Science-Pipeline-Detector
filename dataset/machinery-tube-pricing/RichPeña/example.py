...
train['date'] = (train.quote_date.dt.year * 10000 + train.quote_date.dt.month * 100 + train.quote_date.dt.day)/20170101
test['date'] = (test.quote_date.dt.year * 10000 + test.quote_date.dt.month * 100 + test.quote_date.dt.day)/20170101
...
imp = preprocessing.Imputer(missing_values=0, strategy='mean', axis=0).fit(train)
...
preds = pd.DataFrame({"id": idx, "cost": np.expm1(preds1)})
preds = pd.DataFrame({"id": idx, "cost": preds1})
