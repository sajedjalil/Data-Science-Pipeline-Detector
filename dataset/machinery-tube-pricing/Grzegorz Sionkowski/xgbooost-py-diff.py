...
## Global CV
if CV==True :
    predsAvgCV = (preds1CV+preds2CV)/2
    predsAvgTR = (preds1TR+preds2TR)/2
    ...
    predsAvgCV = np.expm1(predsAvgCV)
    predsAvgTR = np.expm1(predsAvgTR)
    crossvallog = np.expm1(crossvallog)
    label_log = np.expm1(label_log)
    CVScore=np.sqrt(sum(np.power(predsAvgCV-crossvallog,2))/crossrowsNB)
    TrainScore=np.sqrt(sum(np.power(predsAvgTR-label_log,2))/trainrowsNB)
    print('Real CV Score : '+str(np.round(CVScore,4))+ ' Train score : '+str(np.round(TrainScore,4)) + ' in '+str(np.round(time.time() - start_time, 1))+' seconds')
