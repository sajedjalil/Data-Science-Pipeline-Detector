...
train['target'] = label_log
test['target'] = 0

train.to_csv('caterpillar-train.csv', index=False)
test.to_csv('caterpillar-test.csv', index=False)
...
# preds = 0.49*np.expm1( (preds1+preds2+preds4)/3)+0.51*np.power(preds3,16)
