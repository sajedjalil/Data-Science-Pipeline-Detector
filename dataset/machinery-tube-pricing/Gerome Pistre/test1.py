...
quantTr = np.array(train[:,6].astype(float))
quantTst = np.array(test[:,6].astype(float))
label_log = np.log1p(labels * 3.59446 * quantTr)
...
preds1 = np.expm1((preds1) * (1 / 3.59446) * (1 / quantTst))
preds2 = np.expm1((preds2) * (1 / 3.59446) * (1 / quantTst))
preds4 = np.expm1((preds4) * (1 / 3.59446) * (1 / quantTst))

preds = 0.4*preds4+.1*preds1+0.1*preds2+0.4*np.power(preds3,16)
