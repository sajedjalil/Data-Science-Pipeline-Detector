...
def run_solution():
    ...
    avg_target //= total
    avg_target -= 2
    print('Adjusted average target: ', avg_target)
    gc.collect()
    ...
    while 1:
        ...
        if (cliente_id, producto_id) in client_product_arr:
            val = max(0,client_product_arr[(cliente_id, producto_id)]//client_product_arr_count[(cliente_id, producto_id)]-1)
            out.write(str(val))
            index_both += 1
        elif cliente_id in client_arr:
            val = max(0,client_arr[cliente_id]//client_arr_count[cliente_id]-1)
            out.write(str(val))
            index_client += 1
        elif producto_id in product_arr:
            val = max(0,product_arr[producto_id]//product_arr_count[producto_id]-1)
            out.write(str(val))
            index_product += 1
        else:
