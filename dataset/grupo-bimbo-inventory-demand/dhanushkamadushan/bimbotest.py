...
def process_product(product_id, sub):
    ...
    demand_prod_ruta = (df_train_subset.groupby(['Producto_ID', 'Ruta_SAK']))['log_demand'].median().to_dict()
    ...
    for z in zip(df_test_subset.id, df_test_p, df_test_l_pca):

   
        if z[2] in demand_prod_cli_age:
            o = demand_prod_cli_age[z[2]]
            if(o>100):
                if z[1] in demand_prod_ruta:
                    o = demand_prod_ruta[z[1]]
        elif z[1] in demand_prod_ruta:
            o = demand_prod_ruta[z[1]]
        else:
            o = 0
