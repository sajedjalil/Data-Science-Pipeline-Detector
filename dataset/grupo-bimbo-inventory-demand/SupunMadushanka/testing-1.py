...
train['Demanda_uni_equil'] = train['Demanda_uni_equil'].apply(lambda x: 1.004*np.log1p(x + 0.01) - 0.005)
...
gc.collect()
print (test.shape)
test.loc[:, 'Demanda_uni_equil'] = test.loc[:, 'MeanPCA'].apply(np.expm1)* 0.7179 +\
                            test.loc[:, 'MeanPR'].apply(np.expm1)*0.1868 + 0.119
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanPR'].apply(np.expm1)* 0.745 + 0.191
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanC'].apply(np.expm1)* 0.821 + 0.854
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanPA'].apply(np.expm1)* 0.53 + 0.96
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanP'].apply(np.expm1)* 0.5 + 1
indeks = test['Demanda_uni_equil'].isnull()
