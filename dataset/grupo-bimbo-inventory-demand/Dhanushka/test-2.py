...
gc.collect()
print (test.shape)
test.loc[:, 'Demanda_uni_equil'] = test.loc[:, 'MeanPCA'].apply(np.expm1)* 0.7172 +\
                            test.loc[:, 'MeanPR'].apply(np.expm1)*0.1848 + 0.125
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanPR'].apply(np.expm1)* 0.740 + 0.191
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanC'].apply(np.expm1)* 0.821 + 0.854
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanPA'].apply(np.expm1)* 0.53 + 0.94
indeks = test['Demanda_uni_equil'].isnull()
