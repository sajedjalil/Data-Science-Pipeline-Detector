...
def validation_get_optimal_thr(class_id):
    print('Go for class: {}'.format(class_id))
    train_files = glob.glob(INPUT_PATH + 'train_masks/*_{}_mask.gif'.format(class_id))
    train_masks = []
    div_factor = 1
    ...
    cv2.imwrite('avg_mask_{}.jpg'.format(class_id), (255*avg_mask_thr).astype(np.uint8))

    return best_score, avg_mask_thr


def create_submission(avg_score, avg_mask, classes):
    print('Create submission...')
    t = pd.read_csv(INPUT_PATH + 'sample_submission.csv')
    for class_id in classes:
        str = rle(avg_mask[class_id])
        t.loc[t['img'].str.contains('_' + class_id), 'rle_mask'] = str
    t.to_csv('subm_{}.gz'.format(avg_score), index=False, compression='gzip')
...
if __name__ == '__main__':
    ...
    for class_id in classes:
        best_score, avg_mask[class_id] = validation_get_optimal_thr(class_id)
        avg_score += best_score
    avg_score /= len(classes)
    create_submission(avg_score, avg_mask, classes)