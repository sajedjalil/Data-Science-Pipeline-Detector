...
if run_de:
    ...
    train_for_decomposition_no_y = train.drop('y', axis=1).copy().values
    test_for_decomposition = test.copy()

    # PCA
    pca = PCA(iterated_power='auto', n_components=None, random_state=None, svd_solver='auto')
    pca.fit(train_for_decomposition_no_y)
    ...
    pca.n_components = 5
    pca_results_train = pca.fit_transform(train_for_decomposition_no_y)
    pca_results_test = pca.fit_transform(test_for_decomposition)
...
plt.show()
fig_loss.savefig("model_avg.png")