...
trainfile = "../input/train_ver2.csv"
testfile = "../input/test_ver2.csv"

#feature_cols = ["ind_empleado","pais_residencia","sexo","age", "ind_nuevo", "antiguedad", "nomprov", "segmento"]
feature_cols = ['conyuemp', 'segmento', 'indfall', 'indrel', 'sexo', 'tipodom', 'ind_empleado', 'canal_entrada', 'indext', 'ind_actividad_cliente', 'indresi', 'pais_residencia', 'ind_nuevo', 'tiprel_1mes']
...
start_index = train_size - nrows  # 取训练集的后半部分？
for ind, col in enumerate(feature_cols):
    print(col)
    train = pd.read_csv(trainfile, usecols=[col]) # 每次只读取一列数据，只对改列进行处理，包括转换非数值特征
    test = pd.read_csv(testfile, usecols=[col])
    train.fillna(-99, inplace=True)
    test.fillna(-99, inplace=True)
    if train[col].dtype == "object":  # 如果是非数值型特征
        le = preprocessing.LabelEncoder()
        le.fit(list(train[col].values) + list(test[col].values))  # 统一将训练集和测试集的离散性特征转换为数值型离散型特征
    ...
    if ind == 0:
        ...
        train_X = np.hstack([train_X, temp_train_X])  # 横向组合
        test_X = np.hstack([test_X, temp_test_X])
    print(train_X.shape, test_X.shape)
del train
del test

train_y = pd.read_csv(trainfile, usecols=['ncodpers'] + target_cols, dtype=dtype_list)
last_instance_df = train_y.drop_duplicates('ncodpers', keep='last') # 去重，只保留最近的记录
...
# test_id = np.array(pd.read_csv(testfile, usecols=['ncodpers'])['ncodpers'])
# final_preds = []
...
# out_df.to_csv('./data/sub_rf.csv', index=False)
