...
if __name__ == "__main__":
	...
	fit= model.fit_generator(
			generator = batch_generator(train, 200, False), 
			nb_epoch = 1,
			samples_per_epoch = train_size
		)
	preds = model.predict_generator(generator=batch_generator(test, 1000, False, False), val_samples=test_size)
	...
	out_df.to_csv('amla_submission_v2.csv', index=False)
