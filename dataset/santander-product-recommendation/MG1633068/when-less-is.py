...
def processTrainData(in_file_name):
	x_vars_list = []
	y_vars_list = []
	cust_dict_2015 = {}
	cust_dict_2016 = {}
	for row in csv.DictReader(in_file_name):
		...
		if row['fecha_dato'] in ['2015-05-28']:	
			target_list = getTarget(row)
			cust_dict_2015[cust_id] =  target_list[:]
			continue
			
		if row['fecha_dato'] in ['2016-05-28']:	
			target_list = getTarget(row)
			cust_dict_2016[cust_id] =  target_list[:]
			continue
		...
		if row['fecha_dato'] == '2015-06-28':
			prev_target_list = cust_dict_2015.get(cust_id, [0]*22)
			target_list = getTarget(row)
			new_products = [max(x1 - x2,0) for (x1, x2) in zip(target_list, prev_target_list)]
			if sum(new_products) > 0:
				for ind, prod in enumerate(new_products):
	...
	return x_vars_list, y_vars_list, cust_dict_2016
	
def processTestData(in_file_name, cust_dict):
	x_vars_list = []
	for row in csv.DictReader(in_file_name):
		# use only the four months as specified by breakfastpirate #
...
if __name__ == "__main__":
	...
	x_vars_list, y_vars_list, cust_dict = processTrainData(train_file)
	...
	x_vars_list = processTestData(test_file, cust_dict)
	...
	out_df.to_csv('xgb_result.csv', index=False)
	print(datetime.datetime.now()-start_time)