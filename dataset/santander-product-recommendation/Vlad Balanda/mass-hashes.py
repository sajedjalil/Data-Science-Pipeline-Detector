...
def get_hashes(arr):
    ...
    f = (lambda x: -1 if 'NA' in str(x) else x)
    antiguedad = f(antiguedad)
    antiguedad = int(antiguedad)
    
    f1 = (lambda x: -1 if 'NA' in str(x) else x)
    indrel = f1(indrel)
    indrel = int(indrel)
    
    f2 = (lambda x: 2 if x == 99 else x)
    indrel = int(f2(indrel))
    
    f3 = (lambda x: -1 if 'NA' in str(x) else x)
    age = int(f3(age))
    
    f4 = (lambda x: 1 if x < 39 else 2)
    age_1 = f4(age)
    ...
    if 1:
        sub = [
            ...
            (5, pais_residencia, sexo, renta1, age, age_1, segmento),
