"""
Code based on BreakfastPirate Forum post and forked from SRK and taking 3 month lag and 6 month lag 
__author__ : Santanu
"""
import csv
import datetime
...
def processDataMK(in_file_name, cust_dict, lag_cust_dict,lag_cust_dict_1):
    x_vars_list = []
    y_vars_list = []
    
    for row in csv.DictReader(in_file_name):
        #Use only 6 months
        if row['fecha_dato'] not in ['2015-01-28','2015-03-28', '2015-05-28', '2015-06-28', '2015-01-28','2016-03-28', '2016-05-28', '2016-06-28']:
            continue

        cust_id = int(row['ncodpers'])
        #print(row['fecha_dato'])
        # lag ==3 
        if (row['fecha_dato'] in ['2015-03-28', '2016-03-28'] ) or (row['fecha_dato'] in ['2015-01-28', '2016-01-28'] ):
            target_list = getTarget(row)
            if (row['fecha_dato'] in ['2015-03-28', '2016-03-28'] ) :
                lag_cust_dict[cust_id] =  target_list[:]
            else:
                lag_cust_dict_1[cust_id] =  target_list[:]
            continue
        ...
        if row['fecha_dato'] == '2016-06-28':
            ...
            lag_target_list_1 = lag_cust_dict_1.get(cust_id, [0]*22)
            x_vars_list.append(x_vars + prev_target_list + lag_target_list + lag_target_list_1 )
    ...
    return x_vars_list, y_vars_list, cust_dict, lag_cust_dict,lag_cust_dict_1
...
if __name__ == "__main__":
	...
	x_vars_list, y_vars_list, cust_dict, lag_cust_dict,lag_cust_dict_1 = processDataMK(train_file, {}, {},{})
	...
	x_vars_list, y_vars_list, cust_dict, lag_cust_dict,lag_cust_dict_1 = processDataMK(test_file, cust_dict, lag_cust_dict,lag_cust_dict_1)
	...
	out_df.to_csv('sub_xgb_new_3.csv', index=False)
	print(datetime.datetime.now()-start_time)