...
#3.12后的39天，3.5后的39天... #step width =7(作为训！练！集！也就是label集，concat起来是带重叠区域（39/7倍=5倍）。相同日期、相同的y会有不同的特征，这是因为统计区域不同
#比如在3.12的特征，会在训练集的【窗口3.7-4.30】内，则是从3.7开始往前看；同时3.12也会在训练窗【窗口3.12-+39d】内，特征就从一定是3.12往前看)；以此类推，也会在训练窗【2.28-+39d】、【2.21-+39d】内，分别从2.28往前看、2.21往前看
#(数据集overlap)这样做的好处  1.增加训练集数量(More Data) ；2.同一个数据、增加不同统计窗的观察(Better Data)； 缺点（次要）：step越小，训练时间越长，训练数据越多、速度越慢。

#另外，更进一步，在单个【训练窗】时，例如【3.7-4.30】，统计3.7之前的统计量，再用不同的n_days作为窗口，比如1000是历史所有，30是往前看一个月，14是两周。
#用timedelta做日期加减，用【data_temp】来按需切割【特定统计日期的】统计区域的data
