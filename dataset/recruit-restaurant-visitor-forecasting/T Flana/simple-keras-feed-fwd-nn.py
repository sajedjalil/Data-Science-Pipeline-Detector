...
Example of a basic keras feed fwd ann.
My first kernel
...
train, valid, y_train, y_valid = train_test_split(train, targets, test_size=0.15, random_state=2018)
...
model = Sequential()
model.add(Dense(250,activation='relu',input_shape=(train.shape[1],)))
...
# fit network
model.fit(np.array(train), np.array(y_train), epochs=10000, batch_size=200, validation_data=(np.array(valid), np.array(y_valid)),
            verbose=2, callbacks=callbacks, shuffle=False)

model.load_weights('best_wt_recruit_new.hdf5')
    
# get validation score
pred = np.expm1(model.predict(np.array(valid)))
score = rmsle(np.expm1(np.array(y_valid)), pred)

print('score:',score)

# get predictions
prediction = np.expm1(model.predict(np.array(test)))
