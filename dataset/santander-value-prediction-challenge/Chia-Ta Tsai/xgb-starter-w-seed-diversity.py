...
def main():
    ...
    # lda
    #tf = LatentDirichletAllocation(n_components=10, n_jobs=-1, random_state=42)
    #trans_Xs.append(transform(X[X.columns], test_X[X.columns], tf, col_name='lda'))
    ...
    test_preds = [do_model_predict(X, y, test_X, params=params, nr_splits=3, random_state=s).rename('{}'.format(s)) for s in [0, 42, 13, 2018,]]
    test_preds = pd.concat(test_preds, axis=1).mean(axis=1).squeeze().to_frame('target')
    print(test_preds.head())
    test_preds.to_csv("subm_xgb.csv", index=True)
