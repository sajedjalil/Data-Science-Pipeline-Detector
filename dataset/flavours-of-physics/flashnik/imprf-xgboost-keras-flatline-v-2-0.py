...
nb_train_sample = int(len(y) * 0.75) # used to be 0.97, 0.78 is better, 0.83 possible
...
# train model
model.fit(X_train, y_train, batch_size=128, nb_epoch=170, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
...
rf = RandomForestClassifier(n_estimators=450, n_jobs=-1, criterion="entropy", random_state=1)
...
test_probs = (0.15*rf.predict_proba(test[features])[:,1]) + (0.4*gbm.predict(xgb.DMatrix(test[features])))+(0.3*predskeras) + (0.15*fb_preds) #is better
...
submission.to_csv("rf_xgboost_keras_flatness_v6.csv", index=False)