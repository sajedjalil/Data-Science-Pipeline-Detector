...
#agreement_probs= 0.333*rf.predict_proba(check_agreement[features])[:,1] + 0.333* gbm.predict(xgb.DMatrix(check_agreement[features])) + 0.333*rf1.predict_proba(check_agreement[features])[:,1]
#print('Checking agreement...')
#ks = evaluation.compute_ks(
#	agreement_probs[check_agreement['signal'].values == 0],
#	agreement_probs[check_agreement['signal'].values == 1],
#	check_agreement[check_agreement['signal'] == 0]['weight'].values,
#	check_agreement[check_agreement['signal'] == 1]['weight'].values)
#print ('KS metric 0.333/0.333/0.333:', ks, ks < 0.09)
...
#train_eval_probs = 0.333*rf.predict_proba(train_eval[features])[:,1] + 0.333*gbm.predict(xgb.DMatrix(train_eval[features])) + 0.333*rf1.predict_proba(train_eval[features])[:,1]
#print ('Calculating AUC...')
#AUC = evaluation.roc_auc_truncated(train_eval['signal'], train_eval_probs)
#print ('AUC 3x 0.333', AUC)
...
rf1pred = rf1.predict_proba(test[features])[:,1]
...
#test_probs = 0.333 * rfpred + 0.333 * gbmpred + 0.333 *rf1pred
...
print("Make predictions on the test set rf")
test_probs = rf1pred
#submission = pd.DataFrame({"id": test["id"], "prediction": test_probs})
