...
clf = UGradientBoostingClassifier(loss=loss, n_estimators=100, subsample=0.1, 
...
test_probs = (0.4*rf.predict_proba(test[features])[:,1]) + (0.4*gbm.predict(xgb.DMatrix(test[features])))+(0.19*predskeras) + (0.01*fb_preds)
