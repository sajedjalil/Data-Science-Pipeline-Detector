...
clf = UGradientBoostingClassifier(loss=loss, n_estimators=80, subsample=0.1, 
...
test_probs = (0.25*rf.predict_proba(test[features])[:,1]) + (0.125*gbm.predict(xgb.DMatrix(test[features])))+(0.25*predskeras) + (0.25*fb_preds)+(0.125*gbm1.predict(xgb.DMatrix(test[features])))
