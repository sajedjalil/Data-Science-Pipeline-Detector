...
# train model
model.fit(X_train, y_train, batch_size=256, nb_epoch=75, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
...
test_probs = (0.4*rf.predict_proba(test[features])[:,1]) + (0.4*gbm.predict(xgb.DMatrix(test[features])))+(0.1*predskeras) + (0.1*fb_preds)
