...
# train model
model.fit(X_train, y_train, batch_size=125, nb_epoch=77, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
...
test_probs = (0.3*rf.predict_proba(test[features])[:,1]) + (0.37*gbm.predict(xgb.DMatrix(test[features])))+(0.19*predskeras) + (0.01*fb_preds)
