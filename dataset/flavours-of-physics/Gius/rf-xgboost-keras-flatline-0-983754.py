...
# shuffle the data
np.random.seed(520) # used to be 369
np.random.shuffle(X)
np.random.seed(520)
np.random.shuffle(y)
...
# train model
model.fit(X_train, y_train, batch_size=128, nb_epoch=125, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
...
clf = UGradientBoostingClassifier(loss=loss, n_estimators=125, subsample=0.1, # n_estimators = 75
