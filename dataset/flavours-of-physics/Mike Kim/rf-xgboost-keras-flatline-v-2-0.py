...
# shuffle the data
np.random.seed(672) # used to be 369
np.random.shuffle(X)
np.random.seed(672)
np.random.shuffle(y)
...
for j in range(2):
  np.random.seed(j)
  model.fit(X_train, y_train, batch_size=128, nb_epoch=50+j, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
  predskeras = predskeras + model.predict(X, batch_size=256)[:, 1]

predskeras = predskeras/j
