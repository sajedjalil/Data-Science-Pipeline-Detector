...
# shuffle the data
np.random.seed(962) # used to be 369, 671 not bad
np.random.shuffle(X)
np.random.seed(962)
np.random.shuffle(y)
...
nb_train_sample = int(len(y) * 0.72) # used to be 0.97, 0.78 is not bad, 0.83 possible
...
# train model
model.fit(X_train, y_train, batch_size=128, nb_epoch=140, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
# nb_epoch = 50, 100 fine, 150 not bad
...
clf = UGradientBoostingClassifier(loss=loss, n_estimators=140, subsample=0.1, # n_estimators = 75, 150 not bad
...
rf = RandomForestClassifier(n_estimators=240, n_jobs=-1, criterion="entropy", random_state=1)
rf.fit(train[features], train["signal"]) # used to be n_estimators=300, 375 is better, 250 not bad

print("Train a XGBoost model")
params = {"objective": "binary:logistic",
          "eta": 0.2,# used to be 0.2 or 0.1
          "max_depth": 7, # used to be 5 or 6, 7 not bad
          "min_child_weight": 1,
          "silent": 1,
          "colsample_bytree": 0.7,
          "seed": 1}
num_trees=460 #used to be 300, 375 is better, 450 not bad
gbm = xgb.train(params, xgb.DMatrix(train[features], train["signal"]), num_trees)

print("Make predictions on the test set")
test_probs = (0.2*rf.predict_proba(test[features])[:,1]) + (0.2*gbm.predict(xgb.DMatrix(test[features])))+(0.3*predskeras) + (0.3*fb_preds) 
#test_probs = (0.3*rf.predict_proba(test[features])[:,1]) + (0.3*gbm.predict(xgb.DMatrix(test[features])))+(0.2*predskeras) + (0.2*fb_preds) # is not bad
