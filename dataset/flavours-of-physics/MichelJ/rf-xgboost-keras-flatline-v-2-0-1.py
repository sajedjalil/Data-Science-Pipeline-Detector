...
# shuffle the data
np.random.seed(4321) # used to be 671
np.random.shuffle(X)
np.random.seed(4321)
np.random.shuffle(y)
...
nb_train_sample = int(len(y) * 0.83) # used to be 0.97, 0.78 is better, 0.83 possible
...
params = {"objective": "binary:logistic",
          ...
          "seed": 1}
num_trees=375 #used to be 300, 375 is better
...
test_probs = (0.24*rf.predict_proba(test[features])[:,1]) + (0.28*gbm.predict(xgb.DMatrix(test[features])))+(0.30*predskeras) + (0.18*fb_preds) #is better
