...
# train model
model.fit(X_train, y_train, batch_size=128, nb_epoch=20, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
...
test_probs = (0.29*rf.predict_proba(test[features])[:,1]) + (0.31*gbm.predict(xgb.DMatrix(test[features])))+(0.2*predskeras) + (0.2*fb_preds) #is better
