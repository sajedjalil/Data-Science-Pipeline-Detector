...
model = Sequential()
model = Sequential()
model.add(Dense(X_train.shape[1], 16, init='lecun_uniform'))
model.add(Activation('tanh'))
model.add(Dropout(0.4))
model.add(Dense(16, 8, init='lecun_uniform'))
model.add(Activation('tanh'))
model.add(Dropout(0.4))
model.add(Dense(8, 2, init='lecun_uniform'))
model.add(Activation('softmax'))

sgd = SGD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True)
#model.compile(loss='mean_squared_error', optimizer="adadelta")
model.compile(loss='mean_squared_error', optimizer=sgd)
model.fit(X_train, y_train, batch_size=32, nb_epoch=50, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
