...
# train model
model.fit(X_train, y_train, batch_size=128, nb_epoch=85, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
...
clf = UGradientBoostingClassifier(loss=loss, n_estimators=75, subsample=0.1, 
...
rf = RandomForestClassifier(n_estimators=300, n_jobs=-1, criterion="entropy", random_state=1)
...
et = GradientBoostingClassifier(n_estimators=100, random_state=369)
et.fit(train[features], train["signal"])

print("Make predictions on the test set")
test_probs = (0.20*et.predict_proba(test[features])[:,1])+(0.20*rf.predict_proba(test[features])[:,1]) + (0.20*gbm.predict(xgb.DMatrix(test[features])))+(0.20*predskeras) + (0.20*fb_preds)
submission = pd.DataFrame({"id": test["id"], "prediction": test_probs})
submission.to_csv("rf_xgboost_keras_fn_gbm.csv", index=False)