...
clf = UGradientBoostingClassifier(loss=loss, n_estimators=168, subsample=0.1, # n_estimators = 75
                                  max_depth=8, min_samples_leaf=10,
                                  learning_rate=0.05, train_features=features, random_state=8)
clf.fit(train[features + ['mass']], train['signal'])
fb_preds = clf.predict_proba(test[features])[:,1]
print("Train a Random Forest model")
rf = RandomForestClassifier(n_estimators=198, n_jobs=-1, criterion="entropy", random_state=4)
rf.fit(train[features], train["signal"]) # used to be n_estimators=300, 375 is better, 250 could be fine

print("Train a XGBoost model")
params = {"objective": "binary:logistic",
          "eta": 0.05,# used to be 0.2 or 0.1
          "max_depth": 8, # used to be 5 or 6
          "min_child_weight": 1,
          "silent": 1,
          "colsample_bytree": 0.7,
          "seed": 1}
num_trees=278 #used to be 300, 375 is better
