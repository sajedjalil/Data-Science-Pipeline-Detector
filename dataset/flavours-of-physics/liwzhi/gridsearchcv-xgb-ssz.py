...
def add_features(df):
    ...
    df['flight_dist_sig_1'] = df['FlightDistance']**2/(df['FlightDistanceError']**2)
...
train_eval = train[train['min_ANNmuon'] > 0.3]
...
rf1 = RandomForestClassifier(n_estimators=300, n_jobs=-1, criterion="entropy", max_depth=10, max_features=10, min_samples_leaf=2, random_state=1)
...
rf = UGradientBoostingClassifier(loss=loss, n_estimators=250,  
                                  max_depth=6,
                                  learning_rate=0.15, train_features=features, subsample=0.8, random_state=248)
...
agreement_probs= 0.3*rf.predict_proba(check_agreement[features])[:,1] + 0.4* gbm.predict(xgb.DMatrix(check_agreement[features])) + 0.3*rf1.predict_proba(check_agreement[features])[:,1]
...
correlation_probs = 0.3*rf.predict_proba(check_correlation[features])[:,1] + 0.4*gbm.predict(xgb.DMatrix(check_correlation[features])) + 0.3*rf1.predict_proba(check_correlation[features])[:,1]
...
train_eval_probs = 0.3*rf.predict_proba(train_eval[features])[:,1] + 0.4*gbm.predict(xgb.DMatrix(train_eval[features])) + 0.3*rf1.predict_proba(train_eval[features])[:,1]
...
test_probs = 0.3*rf.predict_proba(test[features])[:,1] + 0.4*gbm.predict(xgb.DMatrix(test[features])) + 0.3*rf1.predict_proba(test[features])[:,1] #+ 0.10*rf1.predict_proba(test[features])[:,1] + 0.25*gbm.predict(xgb.DMatrix(test[features]))
