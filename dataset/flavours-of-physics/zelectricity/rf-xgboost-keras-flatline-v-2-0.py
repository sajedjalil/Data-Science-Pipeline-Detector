...
params = {"objective": "binary:logistic",
          ...
          "seed": 1}
num_trees=400 #used to be 300, 375 is better
gbm = xgb.train(params, xgb.DMatrix(train[features], train["signal"]), num_trees)


from sklearn.ensemble import AdaBoostClassifier
clfA=AdaBoostClassifier()
clfA.fit(train[features],train["signal"])
...
#test_probs = (0.35*rf.predict_proba(test[features])[:,1]) + (0.3*gbm.predict(xgb.DMatrix(test[features])))+(0.35*predskeras) #+ (0.20*fb_preds) #is better
test_probs = (.28*clfA.predict_proba(test[features])[:,1])+(0.23*rf.predict_proba(test[features])[:,1]) + (0.25*gbm.predict(xgb.DMatrix(test[features])))+(0.24*predskeras) #+ (0.18*fb_preds) #is better
