...
test_probs_rf = rf.predict_proba(test[features])[:,1]
test_probs_gbm = gbm.predict(xgb.DMatrix(test[features]))
test_probs = (0.24*test_probs_rf) + (0.3*test_probs_gbm)+(0.26*predskeras) + (0.20*fb_preds) #is better
...
submission = pd.DataFrame({"id": test["id"], "prediction": test_probs, "pred_rf": test_probs_rf, "pred_gbm":test_probs_gbm, "pred_keras":predskeras, "pred_fb":fb_preds})
submission.to_csv("rf_xgboost_keras_flatness_solo_v5b.csv", index=False)