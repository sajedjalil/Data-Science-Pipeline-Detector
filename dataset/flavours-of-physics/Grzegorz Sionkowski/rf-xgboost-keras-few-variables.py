...
def get_training_data():
    filter_out = ['id', 'min_ANNmuon', 'FlightDistanceError', 'production', 'mass', 'signal', 'SPDhits', 'IP', 'IPSig', 'isolationc']
...
features = list(set(train.columns) - {'id', 'min_ANNmuon', 'production', 'mass', 'signal', 'SPDhits', 'IP', 'IPSig', 'isolationc'})
 

print("Train a Random Forest model")
rf = RandomForestClassifier(n_estimators=350, n_jobs=-1, criterion="entropy", random_state=1)
...
test_probs = (0.35*rf.predict_proba(test[features])[:,1]) + (0.40*gbm.predict(xgb.DMatrix(test[features])))+(0.25*predskeras)
