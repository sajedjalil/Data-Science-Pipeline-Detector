...
correlation_probs = 0.38*rf.predict_proba(check_correlation[features])[:,1] + 0.42*gbm.predict(xgb.DMatrix(check_correlation[features])) + 0.2*rf1.predict_proba(check_correlation[features])[:,1]
...
test_probs = 0.38*rf.predict_proba(test[features])[:,1] + 0.42*gbm.predict(xgb.DMatrix(test[features])) + 0.2*rf.predict_proba(test[features])[:,1] 
submission = pd.DataFrame({"id": test["id"], "prediction": test_probs})
submission.to_csv("ub_rf_xgboost_submission_3.csv", index=False)
