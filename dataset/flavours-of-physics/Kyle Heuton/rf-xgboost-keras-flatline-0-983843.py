...
# train model
model.fit(X_train, y_train, batch_size=128, nb_epoch=100, validation_data=(X_eval, y_eval), verbose=2, show_accuracy=True)
...
test_probs = (0.5*rf.predict_proba(test[features])[:,1]) + (0.3*gbm.predict(xgb.DMatrix(test[features])))+(0.15*predskeras) + (0.05*fb_preds)
