...
if __name__ == "__main__":
	...
	# Initialize to use Isomap with 2 components (reduces data to 2 dimensions).
	mapper = KeplerMapper(cluster_algorithm=cluster.DBSCAN(eps=0.3, min_samples=15), 
							 reducer = manifold.Isomap(), nr_cubes=30, overlap_perc=0.5, 
							 link_local=False, verbose=1)
	...
	# Create the graph
	complex = mapper.map(data, dimension_index=[0], dimension_name="Isomap(2) 1D")

	# Create the visualizations (lowered the graph_gravity for a nicer graph-look.)

	# Tooltips with image data for every cluster member
	mapper.visualize(complex, "results.html", "Isomap on all '2'-digits in MNIST", graph_charge=-130, graph_gravity=0.10, custom_tooltips=tooltip_s)
