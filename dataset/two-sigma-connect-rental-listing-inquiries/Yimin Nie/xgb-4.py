...
def text_clean(sent):
    ...
    cleaned = ' '.join([w for w in sent.split() if w not in stops])
    cleaned = ' '.join([w for w,tag in pos_tag(cleaned.split()) if (tag == 'NN' or tag == 'JJ' or tag == 'JJR' or tag == 'JJS')])
...
train_df['description'] = train_df['description'].apply(lambda x: text_clean(x))
test_df['description'] = test_df['description'].apply(lambda x: text_clean(x))
