...
#####################################
def runXGB(params, train_X, train_y, test_X, test_y=None, feature_names=None, seed_val=0, num_rounds=1000):
...
features_to_use.extend(["room_sum", "price_per_room", "half_bathrooms"])
####################################################################
...
####Parameters:
params1 = {'objective': 'multi:softprob',
...
for dev_index, val_index in kf.split(range(train_X.shape[0])):
        ...
        preds, model = runXGB(params2,dev_X, dev_y, val_X, val_y)
        cv_scores.append(log_loss(val_y, preds))
        print(cv_scores)
        break
    
print("Preparing XGBoost....")
preds, model = runXGB(params2,train_X, train_y, test_X, num_rounds=500)
out_df = pd.DataFrame(preds)
out_df.columns = ["high", "medium", "low"]
out_df["listing_id"] = test_df.listing_id.values
out_df.to_csv("XGBoost_extended_c.csv", index=False)