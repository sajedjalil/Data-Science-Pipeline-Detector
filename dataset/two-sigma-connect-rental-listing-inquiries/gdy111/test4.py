...
features_to_use.extend(["price_t","num_photos", "num_features", "num_description_words", 
                        "created_year", "created_month", "created_day", "created_hour",'Hardwood Floors','Elevator',
                        "listing_id",'room_dif','room_sum','price_t1','low_features'])
...
display_address={}
street_address={}
for i in train_df[['manager_id','interest_level','display_address',"street_address"]].values:
    if i[0] not in manager_id.keys():
        manager_id[i[0]]=0
    manager_id[i[0]]+=1  
    if i[2] not in display_address.keys():
        display_address[i[2]]=0
    display_address[i[2]]+=1
    if i[3] not in street_address.keys():
        street_address[i[3]]=0
    street_address[i[3]]+=1    
a=[]
b=[]
c=[]
for i in train_df[['manager_id','display_address',"street_address"]].values:
    a.append(manager_id[i[0]])
    b.append(display_address[i[1]])
    c.append(street_address[i[2]])
train_df['manager_sum']=a
train_df['display_sum']=b
train_df['street_sum']=c
features_to_use.append('manager_sum')
features_to_use.append('display_sum')
features_to_use.append('street_sum')
a=[]
b=[]
c=[]

for i in test_df[['manager_id','display_address',"street_address"]].values:
    if i[0] in manager_id.keys():
        a.append(manager_id[i[0]])
    else:
        a.append(0)
    if i[1] in display_address.keys():
        b.append(display_address[i[1]])
    else:
        b.append(0)
    if i[2] in street_address.keys():
        c.append(street_address[i[2]])
    else:
        c.append(0)
test_df['manager_sum']=a
test_df['display_sum']=b
test_df['street_sum']=c                       
print (test_df['manager_sum'].head())
