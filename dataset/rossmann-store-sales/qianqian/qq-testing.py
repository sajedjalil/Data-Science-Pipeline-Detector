...
X_train, X_test = cross_validation.train_test_split(train, test_size=0.1)
#X_train, X_test = train.head(len(train) - val_size), train.tail(val_size)
dtrain = xgb.DMatrix(X_train[features], np.log(X_train["Sales"] + 1))
dvalid = xgb.DMatrix(X_test[features], np.log(X_test["Sales"] + 1))
dtest = xgb.DMatrix(test[features])
watchlist = [(dvalid, 'eval')]
gbm = xgb.train(params, dtrain, num_trees, evals=watchlist, early_stopping_rounds=250, feval=rmspe_xg, verbose_eval=True)
ntree_limit = gbm.best_iteration

pd.to_pickle(gbm,'others_bst.pkl')

print("Validating")
train_probs = gbm.predict(xgb.DMatrix(X_test[features]),ntree_limit=ntree_limit)
...
test_probs = gbm.predict(xgb.DMatrix(test[features]),ntree_limit=ntree_limit)
