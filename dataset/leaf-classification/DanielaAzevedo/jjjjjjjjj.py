...
def filtering(method_PP, method_F):
    ...
    if method_F == "univariance":
        filt_kb_method = SelectKBest(f_classif, k=171)
...
def algorithm(method_A, OneVsRest, OneVsOne, randomized):
    ...
    if method_A == "neural_networks":
        ...
        model = Sequential()
        model.add(Dense(991, input_dim=171, init='normal')) # number of features of the data +1 node for the bias term.
...
def model_builder(method_PP, method_F, method_A, OneVsRest, OneVsOne, randomized, calibrated):
    ...
    if calibrated:
        ...
        model.fit(X_train, y_traincat, batch_size=128,nb_epoch=160)
