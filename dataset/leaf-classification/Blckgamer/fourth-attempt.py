...
def algorithm(method_A, OneVsRest, OneVsOne, randomized):
    ...
    if method_A == "neural_networks":
        ...
        model = Sequential()
        model.add(Dense(991, input_dim=179, init='uniform')) # number of features of the data +1 node for the bias term.
        model.add(Activation('relu'))
        model.add(Dropout(0.3))
        model.add(Dense(759, init='uniform')) #In sum, for most problems, one could probably get decent performance (even without a second optimization step) by setting the hidden layer configuration using just two rules: (i) number of hidden layers equals one; and (ii) the number of neurons in that layer is the mean of the neurons in the input and output layers.
        model.add(Activation('relu'))
        model.add(Dropout(0.5))
        model.add(Dense(99, init='uniform'))# If the NN is a classifier, then it also has a single node unless softmax is used in which case the output layer has one node per class label in your model.
...
def model_builder(method_PP, method_F, method_A, OneVsRest, OneVsOne, randomized, calibrated):
    ...
    if calibrated:
        ...
        model.fit(X_train, y_traincat,batch_size=192,nb_epoch=100)
