...
def main(debug=False):
    ...
    with timer("Save train and test files"):
        train_feats = [f for f in train_df.columns if f not in FEATS_EXCLUDED]
        test_feats = [f for f in test_df.columns if f not in FEATS_EXCLUDED]

        train_df[train_feats].to_csv('train_df_noindex.csv', index=False)
        test_df[test_feats].to_csv('test_df_noindex.csv', index=False)
        train_df[train_feats].to_csv('train_df.csv')
        test_df[test_feats].to_csv('test_df.csv')
