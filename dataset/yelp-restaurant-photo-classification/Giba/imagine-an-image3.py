...
X_train = pd.concat( (X_train.groupby(['business_id'], as_index=False).nth(0),X_train.groupby(['business_id'], as_index=False).nth(1)), axis=0 , ignore_index=True)
X_train = pd.merge(X_train, train_attr, how='left', on='business_id')
X_train = df_all = pd.concat((X_train.groupby(['labels'], as_index=False).nth(0), X_train.groupby(['labels'], as_index=False).last()), axis=0, ignore_index=True)
