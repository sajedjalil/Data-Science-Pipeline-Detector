...
def prepare_data(datapath, val_start_day, train_columns, test_columns, th=5):
    ...
    #remove extinct places
    data_y1h1 = df_train.loc[(df_train.time>=0)&(df_train.time<=262800)]
    data_y1h2 = df_train.loc[(df_train.time>262800)&(df_train.time<=525600)]
    data_y2h1 = df_train.loc[(df_train.time>525600)&(df_train.time<=788400)]
    
    place_y1h1 = set(data_y1h1.place_id)
    place_y1h2 = set(data_y1h2.place_id)
    place_y2h1 = set(data_y2h1.place_id)
    
    del data_y1h1,data_y1h2,data_y2h1
    gc.collect()
    
    place_kill = place_y1h1.difference(place_y1h2.union(place_y2h1))
    df_train = df_train.loc[~df_train.place_id.isin(place_kill)]
