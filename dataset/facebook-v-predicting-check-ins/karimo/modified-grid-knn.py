...
df_train['hour'] = ((df_train['hour']%24+1))*1
...
df_train['weekday'] = (df_train['weekday']%7+1)*1
#df_train['month'] = (df_train['month']%12+1)*2.1
df_train['accuracy'] = np.log10(df_train['accuracy'])*1
...
df_test['hour'] = ((df_test['hour']%24+1))*1
df_test['weekday'] = (df_test['weekday']%7+1)*1
#df_test['month'] = (df_test['month']%12+1)*2.1
df_test['accuracy'] = np.log10(df_test['accuracy'])*1
...
def process_one_cell(df_cell_train, df_cell_test):
    ...
    mask = (place_counts[df_cell_train.place_id.values] >= 4).values
    ...
    df_cell_train.loc[:,'x'] *= 200.0
    df_cell_train.loc[:,'y'] *= 300.0
    df_cell_test.loc[:,'x'] *= 200.0
    df_cell_test.loc[:,'y'] *= 300.0
    ...
    clf = KNeighborsClassifier(n_neighbors=np.floor(np.sqrt(y.size)).astype(int), 
                            weights=calculate_distance,metric='minkowski',n_jobs=2)
    clf.fit(X, y)
