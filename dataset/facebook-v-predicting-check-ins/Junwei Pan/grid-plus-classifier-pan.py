...
def process_one_cell(df_train, df_test, grid_id, th):
    ...
    predictors = df_cell_train.drop(['place_id', 'grid_cell'], axis = 1).columns.values

    #Training Classifier
    #clf = SGDClassifier(loss='modified_huber', n_iter=1, random_state=0, n_jobs=-1) 
    clf = RandomForestClassifier(n_estimators = 10, min_weight_fraction_leaf = 0.001)
    clf.fit(X, y)
    importances = clf.feature_importances_
    lstFeaImp= [[predictors[i], importances[i]] for i in range(len(predictors))]
    sortLstFeaImp = sorted(lstFeaImp, key = lambda x: x[1], reverse = True)
    for i in range(len(predictors)):
        print ("%d\t%s\t%f" % (i, sortLstFeaImp[i][0], sortLstFeaImp[i][1]))
    timeEclipse(timeS, 'Totally')
...
def timeEclipse(timeS, description):
    timeEclipsed = time.time() - timeS
    print ('%s took %f s' % (description, timeEclipsed))
...
if __name__ == '__main__':
    ...
    df_train = pd.read_csv('../input/train.csv', dtype={'x':np.float32, 
                                               ...
                                               'place_id':np.int}, 
                                               index_col = 0)
    timeEclipse(timeS, 'Load Train Data')
    ...
    df_submission  = process_grid(df_train, df_test, df_sub, th, 
                                  n_cell_x * n_cell_y) 
    timeEclipse(timeS, 'Process Grid')
    #creating the submission
    print('Generating submission file ...')
    df_submission.to_csv("sub.csv", index=True)
    timeEclipse(timeS, 'Total')
