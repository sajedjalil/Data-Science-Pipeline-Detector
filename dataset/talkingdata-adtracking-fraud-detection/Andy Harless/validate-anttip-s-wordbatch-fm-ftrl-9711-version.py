...
TRAIN_SIZE = 122071523 # Rows to use for training
...
#for df_c in pd.read_csv('../input/train.csv', engine='c', chunksize=batchsize,
						sep=",", nrows=TRAIN_SIZE, dtype=dtypes):
	rcount += batchsize
	#cpuStats()
...
test_preds = []
rcount = 0
for df_c in pd.read_csv('../input/validation-data-as-pandas-df/validation.csv', engine='c', chunksize=batchsize,
...
click_ids = list(range(len(test_preds)))
y_test = pd.read_csv( '../input/validation-data-as-pandas-df/validation.csv', usecols=['is_attributed'])
print(  "\n\nFULL VALIDATION SCORE:    ", 
        roc_auc_score( np.array(y_test), np.array(test_preds) )  )
print(  "PUBLIC VALIDATION SCORE:  ", 
        roc_auc_score( np.array(y_test)[:PUBLIC_CUTOFF], np.array(test_preds)[:PUBLIC_CUTOFF] )  )
...
df_sub.to_csv("wordbatch_fm_ftrl_val.csv", index=False, float_format='%.9f')