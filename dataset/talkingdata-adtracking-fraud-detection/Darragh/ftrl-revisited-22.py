...
holdday = '2017-11-09'   # data after date N (exclusive) are used as validation
...
for e in range(epoch):
    ...
    for t, x, y, date, _ in data(train, D):  # data is a generator
        ...
        if t%1000000 == 0:
            print("Train Processed Rows : %sM ; Seconds running %s "%( int(t/1e+6), '%0.0f'%(time.time()-start_time)))
...
with open(submission, 'w') as outfile:
    ...
    for t, x, y, date, click_id in data(test, D):
        ...
        outfile.write('%s,%s\n' % (click_id, str(p)))
        if t%1000000 == 0:
            print("Test Processed Rows : %sM ; Seconds running %s "%( int(t/1e+6), '%0.0f'%(time.time()-start_time)))
