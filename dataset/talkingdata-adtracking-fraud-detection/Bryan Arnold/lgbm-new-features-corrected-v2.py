...
#
# MODEL DEF'N (note: takes param's given farther down in the script as input)
#
...
#
# uses 40 mil rows after the first 79.9 some-odd mil rows
#
print('load train...')
train_df = pd.read_csv(path+"train.csv", skiprows=range(1,79903891), nrows=40000000,dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
print('load test...')
...
train_df['day'] = pd.to_datetime(train_df.click_time).dt.day.astype('uint8') # does not use .dt.dayofweek!
...
params = {
    ...
    'scale_pos_weight':406.11 # because training data is extremely unbalanced 
}
