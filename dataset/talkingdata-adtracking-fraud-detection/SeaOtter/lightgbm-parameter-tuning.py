"""
Note: There may be something wrong . because i am beginner :D
I made parameter tuning with Md Asraful Kabir code(https://www.kaggle.com/asraful70/talkingdata-added-new-features-in-lightgbm).
Sometimes it does not work after stopping suddenly.
"""
...
"""
debug = 0
FILENO = 8 #To distinguish the output file name.
...
###############################################################################################################################    
    ...
    def objective(values):
        ...
        xgtrain = lgb.Dataset(train_df[feature_set].values, label=y_train,feature_name=feature_set,
                           categorical_feature=categorical)
        xgvalid = lgb.Dataset(val_df[feature_set].values, label=y_val,feature_name=feature_set,
                          categorical_feature=categorical)
        ...
        auc = -roc_auc_score(y_val, model_lgb.predict(val_df[model_lgb.feature_name()]))
        ...
        return  auc
    res_gp = gp_minimize(objective, space, n_calls=20, random_state=0, n_random_starts=10)
