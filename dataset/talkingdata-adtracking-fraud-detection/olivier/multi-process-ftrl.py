...
class FTRLProximal(object):
    ...
    def multi_fit(self, data, target, epochs=1, eval_x=None, eval_y=None):
        ...
        for e_ in range(epochs):
            ...
            processes = [
                Process(target=self.multi_train2,
            ...
            if eval_x is not None:
                ...
                oof_v = p.map(functools.partial(self.multi_predict2, z_=np.array(z_), n_=np.array(n_)),
    ...
    def multi_train2(self, y_data, z_, n_, lock_, loss_):
        ...
        for count, x in enumerate(data_[idx]):
            ...
            p = self.train2(x, y[count], z_, n_, lock_)
            # Compute cumulated loss
            loss_.value += self._logloss(p, y[count])
            # print(loss_.value)
        return loss_.value

    def train2(self, x, y, z_, n_, lock_):
        # Compute prediction - does not need a lock
        p, w = self._predict2(x, z_, n_)

        # Update weights
        # gradient under mse, remember all x are equal to 1 once hashed
        g = p - y
    ...
    def multi_predict(self, df_dat):
        # Compute validation losses
        p = Pool(self.cpus)
        # Shared memory is not supported by map and z_ and n_ should be read only here
        # So we create np.arrays form shared memory objects
        oof_v = p.map(functools.partial(self.multi_predict2, 
                                        z_=self.z, 
                                        n_=self.n),
    ...
    def _predict(self, x):
        ...
        for i in self._indices(x):
            ...
            # this allows us for not storing the complete w
            # if sign * z[i] <= reg_alpha:
            if (abs(self.z[i]) <= self.reg_alpha):  # or (self.filter[i] <= self.min_occ):
                # w[i] vanishes due to reg_alpha regularization
                w[i] = 0.
            else:
                # apply prediction time reg_alpha, reg_lambda regularization to z and get w
    ...
    def multi_predict2(self, np_dat, z_, n_):
        preds = np.zeros(len(np_dat))
        for i, row in enumerate(np_dat):
            preds[i], w_ = self._predict2(row, z_, n_)
        return preds
...
def main():
    # train_file, trn_nb_samples, chunksize = "../input/train.csv", 184903890, 5000000
    ...
    for epoch in range(nb_epochs):
        ...
        model = FTRLProximal(
            ...
            beta=1.,  # Smoothing parameter for adaptive learning
    ...
    for i_c, df in enumerate(pd.read_csv(test_file, chunksize=chunksize, iterator=True, dtype=dtypes)):
        ...
        if sub_preds is None:
            sub_preds = model.multi_predict(df[features])  # .values)
            sub_ids = df["click_id"].values
        else:
            sub_preds = np.hstack((sub_preds, model.multi_predict(df[features])))  # .values)))
