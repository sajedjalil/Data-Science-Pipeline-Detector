...
#ORIGIN_IMAGE_SIZE = [192, 192] #归一化图片大小测试
ORIGIN_IMAGE_SIZE = [331, 331]
if not IS_VALIDATE_FLAGS:
    ORIGIN_IMAGE_SIZE = [512, 512]
CROP_RATE = 0.9
...
MOREIMAGES_PATH = MOREIMAGES_PATH_SELECT[ORIGIN_IMAGE_SIZE[0]]

OXFORD_FILES = tf.io.gfile.glob(MORE_IMAGES_GCS_DS_PATH + '/oxford_102_no_test' + MOREIMAGES_PATH + '/*.tfrec')
OPENIMAGE_FILES = tf.io.gfile.glob(MORE_IMAGES_GCS_DS_PATH + '/openimage_no_test' + MOREIMAGES_PATH + '/*.tfrec')
INATURAL_FILES = tf.io.gfile.glob(MORE_IMAGES_GCS_DS_PATH + '/inaturalist_no_test' + MOREIMAGES_PATH + '/*.tfrec')
# 获取训练、验证、测试数据
...
if IS_ADD_EXTERN:
    TRAINING_FILENAMES += INATURAL_FILES
