...
def extract_features(data):
    ...
    for i, row in data.iterrows():
        ...
        if len(title) > 0:
            #print query, title
            data.set_value(i, "query_tokens_in_title", len(query.intersection(title))*1.0/len(query.union(title)))
        if len(description) > 0:
            data.set_value(i, "query_tokens_in_description", len(query.intersection(description))*1.0/len(query.union(description)))
