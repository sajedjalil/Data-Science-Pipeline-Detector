...
if __name__ == '__main__':
    ...
    ('svm', SVC(C=12.0, kernel='rbf', degree=3, gamma=0.0, coef0=0.0, shrinking=True, probability=False, tol=0.001, cache_size=200, class_weight=None, verbose=False, max_iter=-1, random_state=None))])
    ...
    for i in range(len(preds)):
        x = 2.0 * math.pow(1.2 * math.pow((int(t_labels[i])), -1.0) + 0.8 * math.pow(preds[i], -1.0), -1.0)
        x = round(x)
        p3.append(int(x))
        
    # Create your first submission file
    submission = pd.DataFrame({"id": idx, "prediction": p3})
    submission2 = pd.DataFrame({"id":idx,"prediction":preds})
    submission3 = pd.DataFrame({"id":idx,"prediction":t_labels})
    submission2.to_csv("model1.csv",index = False)
    submission3.to_csv("model2.csv",index = False)
    submission.to_csv("ensemble_svc_different_preproc1.csv", index=False)
