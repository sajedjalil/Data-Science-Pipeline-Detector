...
if __name__ == '__main__':
    ...
    def vectorize(train, tfv_query=None):
        ...
        if tfv_query is None:
            tfv_query = TfidfVectorizer(min_df=2,  max_features=None,   
                    strip_accents='unicode', analyzer='word',token_pattern=r'\w{1,}',
                    ngram_range=(1, 3), use_idf=1,smooth_idf=0,sublinear_tf=1,
    ...
    clf = Pipeline([('v',TfidfVectorizer(min_df=2, max_df=200, max_features=None, strip_accents='unicode', analyzer='word', token_pattern=r'\w{1,}', ngram_range=(1, 3), use_idf=True, smooth_idf=True, sublinear_tf=True, stop_words = 'english')), 
    ('svd', TruncatedSVD(n_components=350, algorithm='randomized', n_iter=6, random_state=12345, tol=0.0)), 
    ('scl', StandardScaler(copy=True, with_mean=True, with_std=True)), 
    ('svm', SVC(C=10.0, kernel='rbf', degree=3, gamma=0.0, coef0=0.0, shrinking=True, probability=False, tol=0.002, cache_size=200, class_weight=None, verbose=False, max_iter=-1, random_state=None))])
    ...
    submission.to_csv("a_little_change.csv", index=False)
