...
for tq in t_queries:
    ...
    v = TfidfVectorizer(use_idf=False,min_df=0,ngram_range=(1,6),lowercase=True,sublinear_tf=False,stop_words=ML_STOP_WORDS)
    clf.fit(v.fit_transform(df1_s[1]), df1_s[2])
    t_labels_nb = clf.predict(v.transform(df2_s[1]))
    #SDG
    clf = Pipeline([('v',TfidfVectorizer(max_features=None, ngram_range=(1, 6), use_idf=False, smooth_idf=False, sublinear_tf=False, stop_words = ML_STOP_WORDS)), ('sdg', SGDClassifier(alpha=0.0001, average=False, class_weight=None, epsilon=0.1, eta0=0.0, fit_intercept=True, l1_ratio=0.15, learning_rate='optimal', loss='hinge', n_iter=5, n_jobs=1, penalty='l2', power_t=0.5, random_state=None, shuffle=True, verbose=0, warm_start=False))])
    ...
    #Decision Tree
    clf = Pipeline([('v',TfidfVectorizer(max_features=None, ngram_range=(1, 6),  use_idf=False, smooth_idf=False, sublinear_tf=False, stop_words = ML_STOP_WORDS)), ('dtc', DecisionTreeClassifier(random_state=0))])
    clf.fit(df1_s[1], df1_s[2])
    t_labels_dtc = clf.predict(df2_s[1])
    #KNeighbors
    clf = Pipeline([('v',TfidfVectorizer(max_features=None, ngram_range=(1, 6),  use_idf=False, smooth_idf=False, sublinear_tf=False, stop_words = ML_STOP_WORDS)), ('kn', KNeighborsClassifier(n_neighbors=3))])
    clf.fit(df1_s[1], df1_s[2])
