...
with open("../input/train.csv",encoding="ascii", errors="surrogateescape") as f:
    ...
    for line in d2:
        ...
        s=(" ").join(["q"+ z for z in line["query"].split(" ")])  + " " + line["product_title"]
        s=re.sub(r'<([^>]+)>', ' ', s)
...
t_labels=[]
t_labels1=[]
stopw=['to', 'in', 'and', 'any', 'can', 'has', 'with', 'features', 'your', 'black', 'easy', 'from', 'style', 'all', 'you', 'design', 'use', 'will', 'made', 'as', 'it', 'up', 'is', 'are', 'for', 'that', 'of', 'set', 'an', 'be', 'by', 'on', 'the', 'or', 'this', 'one']
for st in stopw:
    if st in d:
        del d[st]
with open("../input/test.csv",encoding="ascii", errors="surrogateescape") as f:
    ...
    for line in d1:
        ...
        s=(" ").join(["q_"+ z for z in line["query"].split(" ")])  + " " + line["product_title"]
        s=re.sub(r'<([^>]+)>', ' ', s)
        s=s.replace('\r',' ').replace('\n',' ').replace('\t',' ').lower()
        s=re.sub(r'[^a-z0-9]',' ', s)
        w=list(set(s.split(" ")))
...
h.append("]);tree = new google.visualization.TreeMap(document.getElementById('chart_div'));tree.draw(data, {});}</script></head><body><div id='chart_div' style='width: 1000px; height: 500px;'></div></body></html>")
