...
# Feature Engineering
# Helper Functions
def drop_cheaters_and_outliers(df):
    ...
    df.drop(['killsWithoutMoving'], axis=1, inplace=True)
    return df
...
class NNModel():
  ...
  def _train(self, x_train, y_train, epochs):
    filepath="/content/drive/My Drive/pubg/epochs:{epoch:03d}-mse:{mae:.3f}.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='mae', verbose=1, 
...
class AdvancedNN(NNModel):
    def __init__(self, input_size, reload=False, path=None):
        if reload:
            ...
            self.early_stopping = EarlyStopping(monitor='mae', mode='min', patience=15, verbose=1)
...
def feature_engineering(is_train=True):
    ...
    if is_train:
        df = drop_cheaters_and_outliers(df)
    df = get_playersJoined(df)
    df = get_killsNorm(df)
...
model1._summarize()
model1._compile()
model1._train(x_train, y_train, 100)
