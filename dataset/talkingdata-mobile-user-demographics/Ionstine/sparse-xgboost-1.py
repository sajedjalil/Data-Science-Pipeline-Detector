...
gatrain['row'] = [i/gatrain.shape[0] for i in range(gatrain.shape[0])]
gatest['row'] = [i/gatest.shape[0] for i in range(gatest.shape[0])]

gatrain['row_i'] = 1
gatest['row_i'] = 1

Xtr_row = csr_matrix((gatrain.row,(gatrain.trainrow,gatrain.row_i)))
Xte_row = csr_matrix((gatest.row,(gatest.testrow,gatest.row_i)))
...
Xtrain = hstack((Xtr_brand, Xtr_model,Xtr_row), format='csr')
Xtest =  hstack((Xte_brand, Xte_model, Xte_row), format='csr')
...
clf = xgb.train(params, d_train, 5000, watchlist, early_stopping_rounds=25)

pred = clf.predict(xgb.DMatrix(Xtest))

pred = pd.DataFrame(pred, index = gatest.index, columns=targetencoder.classes_)
pred.head()
pred.to_csv('sparse_xgb1.csv', index=True)
