...
print("# Num of Features: ", train_sp.shape[1])
...
def baseline_model():
    # create model
    model = Sequential()
    model.add(Dense(10, input_dim=train_sp.shape[1], init='normal'))
    model.add(PReLU())
...
fit= model.fit_generator(generator=batch_generator(train_sp, Y, 800, True),
                        nb_epoch=30,samples_per_epoch=train_sp.shape[0], verbose=2
                        )
scores_val = model.predict_generator(generator=batch_generatorp(test_sp, 800, False), val_samples=test_sp.shape[0])


# evaluate the model
#print('logloss CV {}'.format(log_loss(Y, d_b_train.values)))
result = pd.DataFrame(scores_val , columns=lable_group.classes_)
result["device_id"] = device_id
print(result.head(1))
result = result.set_index("device_id")

result.to_csv('sub_bagofapps7_keras_10_pt2_50_pt2_30epoch_prelu_softmax_test_all.csv', index=True, index_label='device_id')

#print("# Final prediction")
#fit= model.fit_generator(generator=batch_generator(train_sp, Y, 800, True),
#                         nb_epoch=30,samples_per_epoch=69984, verbose=2
#                         )
...
#print(result.head(1))
#result = result.set_index("device_id")
#result.to_csv('sub_bagofapps7_keras_50_pt2_10_pt2_3epoch_prelu_softmax_test.csv', index=True, index_label='device_id')
print("Done")
