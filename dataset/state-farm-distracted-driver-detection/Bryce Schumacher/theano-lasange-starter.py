...
def ZFTurboNet(input_var=None):
    ...
    l_conv = ConvLayer(l_in, num_filters=10, filter_size=2, pad=1, nonlinearity=rectify)
    #l_convb = ConvLayer(l_conv, num_filters=8, filter_size=2, pad=1, nonlinearity=rectify)
    #l_pool = MaxPool2DLayer(l_conv, pool_size=2) # feature maps 16x16

    #l_dropout1 = DropoutLayer(l_pool, p=0.25)
    #l_hidden = DenseLayer(l_conv, num_units=2, nonlinearity=rectify)
    #l_dropout2 = DropoutLayer(l_hidden, p=0.01)

    l_out = DenseLayer(l_conv, num_units=10, nonlinearity=softmax)
