...
def load_train(img_rows, img_cols, color_type=1):
    ...
    for j in range(10):
        ...
        path = os.path.join('..', 'input', 'train', 'c' + str(j), '*.jpg') #generic regex name
        files = glob.glob(path)  #generate the list of the all image names
        for fl in files:
            flbase = os.path.basename(fl)#only image name
...
paths = ["X_train","Y_train","train_index","X_valid","Y_valid","test_index"]


file = open(paths[0],"wb")
...
file = open(paths[2],"wb")
pickle.dump(train_index,file)
file.close()

file = open(paths[3],"wb")
pickle.dump(train_index,file)
file.close()

file = open(paths[4],"wb")
pickle.dump(Y_valid,file)
file.close()

file = open(paths[5],"wb")
