...
def create_model_v1(img_rows, img_cols, color_type=1):
    ...
    nb_conv = 2
    model = Sequential()
    model.add(Convolution2D(nb_filters, kernel_size=(nb_conv,nb_conv),strides=(1,1),border_mode='valid',data_format="channels_first",input_shape=(color_type, img_rows, img_cols)))
    model.add(Activation('relu'))
    model.add(Convolution2D(nb_filters, kernel_size=(nb_conv, nb_conv),data_format="channels_first"))
    model.add(Activation('relu'))
...
def run_single():
    ...
    model = create_model_v1(img_rows, img_cols, 1)
