...
xgb2_params['nrounds']=362
print(xgb2_params)

xgb2 = XgbWrapper(seed=SEED, params=xgb2_params)
xgb2_oof_train, xgb2_oof_test = get_oof(xgb2)
print("XGB-CV: {}".format(mean_absolute_error(np.exp(y_train)-shift, np.exp(xgb2_oof_train)-shift )))
xgb2_test = pd.DataFrame(np.exp(xgb2_oof_test)-shift, columns=[TARGET])
