...
models_1 = [
    XGBRegressor(seed = 0, nthread = -1, colsample_bytree = 0.7, subsample = 0.7, learning_rate = 0.09, max_depth = 5, min_child_weight = 1, n_estimators = 200),
    ExtraTreesRegressor(random_state = 0, n_jobs = -1, n_estimators = 100, max_features = 0.5, max_depth = 7, min_samples_leaf = 2),
    RandomForestRegressor(random_state = 0, n_jobs = -1, n_estimators = 100, max_features = 0.5, max_depth = 5, min_samples_leaf = 2),
    ]

print('\nStarting stacking on dense data...\n')

# Get stacking features
S_train_1, S_test_1 = stacking(models_1, X_train, y_train, X_test, n_folds = 4, shuffle = True, 
...
print('\nStarting stacking on sparse data...\n')

# Get stacking features
S_train_2, S_test_2 = stacking(models_2, X_train_sparse, y_train, X_test_sparse, n_folds = 4, shuffle = True, 
    transform_target = np.log, transform_pred = np.exp, verbose = 2)
    
# 2-nd level model
model = XGBRegressor(seed = 0, colsample_bytree = 0.8, subsample = 0.5, learning_rate = 0.01, max_depth = 4, min_child_weight = 1, n_estimators = 900)
model = model.fit(np.c_[S_train_2], np.log(y_train))
subm_df.iloc[:, 1] = np.exp(model.predict(np.c_[S_test_2]))
