...
def xgboost_pred(train,labels,test):
	...
	xgtrain = xgb.DMatrix(train[2*offset:,:], label=labels[2*offset:])
	xgval = xgb.DMatrix(train[:2*offset,:], label=labels[:2*offset])
	...
	preds = preds1*2.5 + preds2*3.75 + preds3*3.75
	return preds
