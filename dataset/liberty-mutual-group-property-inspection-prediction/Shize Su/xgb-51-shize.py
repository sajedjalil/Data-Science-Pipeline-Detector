...
#preds = 2.5*preds1 + 7.0*preds2 +0.4*(preds_et1*0.5+preds_et2*1)#+0.3*(pred_gbr1*0.5+pred_gbr2*1.5)  #+preds3+preds4+preds5+preds6+preds7+preds8
...
preds.to_csv('xgboost_benchmark_57_et1.csv')
...
preds = pd.DataFrame({"Id": test_ind, "Hazard": preds})
preds = preds.set_index('Id')
preds.to_csv('xgboost_benchmark_57_et2.csv')
