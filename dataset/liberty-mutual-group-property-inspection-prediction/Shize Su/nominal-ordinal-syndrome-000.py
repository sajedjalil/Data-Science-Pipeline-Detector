...
def xgboost_pred(train,labels,test):
	...
	model = xgb.train(plst, xgtrain, num_rounds, watchlist, early_stopping_rounds=60)
...
preds.to_csv('xgboost_benchmark_kk-57preds1_dep8.csv')
...
preds = pd.DataFrame({"Id": test_ind, "Hazard": preds})
preds = preds.set_index('Id')
preds.to_csv('xgboost_benchmark_kk-43preds2_dep8.csv')


preds = 0.57 * preds1 + 0.43 * preds2

#generate solution
preds = pd.DataFrame({"Id": test_ind, "Hazard": preds})
preds = preds.set_index('Id')
preds.to_csv('xgboost_benchmark_kk-57-43_dep8.csv')