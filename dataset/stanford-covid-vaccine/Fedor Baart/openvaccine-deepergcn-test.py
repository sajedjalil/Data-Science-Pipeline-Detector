...
class MyData(Data):
    def __init__(self, x=None, edge_index=None, edge_attr=None, y=None, y_error=None,
                 pos=None, norm=None, face=None, weight=None, **kwargs):
        super(MyData, self).__init__(x=x, edge_index=edge_index,
                                     edge_attr=edge_attr, y=y, y_error=y_error, pos=pos,
...
def build_data(df, is_train):
    ...
    for i in range(len(df)):
        ...
        if is_train:
            ...
            reactivity_error = df.loc[i, 'reactivity_error']
            deg_Mg_pH10_error = df.loc[i, 'deg_error_Mg_pH10']
            deg_Mg_50C_error = df.loc[i, 'deg_error_Mg_50C']
            deg_pH10_error = df.loc[i, 'deg_error_pH10']
            deg_50C_error = df.loc[i, 'deg_error_50C']
            ...
            for j in range(seq_length):
                if j < seq_scored:
                    ...
                    targets_error.append([0, 0, 0, 0, 0])
        ...
        if is_train:
            data.append(MyData(x=node_features, edge_index=edge_index,
                               ...
                               y_error=torch.tensor(targets_error, dtype=torch.float)
                              ))
        else:
            data.append(MyData(x=node_features, edge_index=edge_index,
                               edge_attr=edge_features,
...
def criterion(prds, tgts, weight=None):
    if weight is None:
        return (torch.sqrt(torch.nn.MSELoss()(prds[:,0], tgts[:,0])) +
                ...
                torch.sqrt(torch.nn.MSELoss()(prds[:,4], tgts[:,4]))) / 5
    else:
        return (torch.sqrt(weighted_mse_loss(prds[:,0], tgts[:,0], weight)) +
                ...
                torch.sqrt(weighted_mse_loss(prds[:,4], tgts[:,4], weight))) / 5
...
if __name__ == '__main__':
    ...
    print('Training')
    all_ys = torch.zeros((0, 5)).to(device).detach()
    all_outs = torch.zeros((0, 5)).to(device).detach()
    best_model_states = []
    ...
    for fold, ((clean_train_idx, clean_valid_idx),
        ...
        weight_reactivity = 0.9
        weight_error = 1 - weight_reactivity 
        ...
        for epoch in range(epochs):
            ...
            for data in tqdm(loader_train):
                ...
                y = data.y[mask]
                y_error = data.y_error[mask]
                
                
                out1, out2 = model(data)
                loss1 = criterion(out1[mask], y, weight)
                loss2 = criterion(out2[mask], torch.log(y_error)) # TODO: add weight here also?
                loss = weight_reactivity * loss1 + weight_error * loss2
                loss.backward()
            ...
            valid_loss = 0.0
            nb = 0
            ys = torch.zeros((0, 5)).to(device).detach()
            outs = torch.zeros((0, 5)).to(device).detach()
            for data in tqdm(loader_valid_clean):
                data = data.to(device)
                mask = data.train_mask

                out1, out2 = model(data)
                out1 = out1[mask].detach()
                out2 = out2[mask].detach()
                y = data.y[mask].detach()
                y_error = data.y_error[mask].detach()
                loss1 = criterion(out1, y).detach()
                loss2 = criterion(out2, y_error).detach()
                ...
                outs = torch.cat((outs, out1), dim=0)
    ...
    preds = torch.zeros((len(id_seqpos), 5)).to(device).detach()
