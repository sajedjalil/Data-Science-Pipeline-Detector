...
def interpolateFun0(x):
    ...
    # Handle cases where there are no or only one values
    ...
    if value_locs[0]:
        ...
        #out[0:end] = np.interp(range(0, end), [0, end], [0.1, g[end]])
    ...
    if end < (len(g) - 1):
        ...
        #out[beg+1:] = np.interp(range(beg+1, end), [beg+1, end-1], [g[beg], .1])
