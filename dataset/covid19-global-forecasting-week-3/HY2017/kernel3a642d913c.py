...
def flow_fit(df):
    ...
    day = CC.shape[0]
    na0 = CC[0]
    para0 = [4*na0+1,0.2,0.8,10,10,na0]
    
    daily_m, R7_m = s_com_model(para0,day-1)
    dd0 = np.abs(CC.diff()[1:].values)
    ddm = daily_m.Na.diff()[1:].values
    error_check0 = 10*np.square(np.log(daily_m.Na.values+1)-np.log(CC+1)).sum()+10*np.square(np.log(R7_m.Na.values+1)-np.log(R7+1)).sum()+0.5*np.abs((dd0-ddm+1)/(dd0+ddm+1)).sum()
    ...
    for ii in range(0,3):
        ...
        for i in range(0,fiti):
            ...
            error_check = 10*np.square(np.log(daily_m_fit.Na.values+1)-np.log(CC+1)).sum()+10*np.square(np.log(R7_m_fit.Na.values+1)-np.log(R7+1)).sum()+0.5*np.abs((dd0-ddi+1)/(dd0+ddi+1)).sum()
    ...
    print('Errors : '+str(error_check0)+' e0= '+str(e0)+' e1= '+str(e1)+' e2= '+str(e2))
