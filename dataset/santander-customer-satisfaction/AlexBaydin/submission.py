...
df_train.rename(columns={370: "TARGET", 0 : "ID"},inplace=True) 
...
remove2 = random.sample(remove2, len(df_train) -4000 - len(remove3))
remove3 = random.sample(remove3, len(remove3) - 3007)
remove2 = remove2 +remove3
df_train_orig = df_train
df_train = df_train_orig.drop(df_train_orig.index[remove2])
...
# classifier
clf = xgb.XGBClassifier(missing=np.nan, max_depth=45,
