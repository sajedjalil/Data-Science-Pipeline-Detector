...
import pandas as pd
df1=pd.read_csv('../inpit/train.csv')
dfff=df1
...
def testing(df_need,df_need_not,need_,need_not_,data):#df is conditional probability,data is the test data
    ...
    for i in range(len(data)):
        ...
        p1=sum(np.array(data_need_not_.iloc[i])*np.array(df_need_not.iloc[1]))+sum(sum(np.array(df_need.iloc[3:])*need_Vocab))
        p0=sum(np.array(data_need_not_.iloc[i])*np.array(df_need_not.iloc[0]))+sum(sum(np.array(df_need.iloc[0:3])*need_Vocab))
        if p1>p0:
            return_target.append(1)
        else:
            return_target.append(0)
    return return_target
target=testing(df_need_,df_need_not_,need_,need_not_,dfff)
err=0
for i in range(len(target)):
    if target[i]!=dfff['TARGET'].iloc[i]:
        err=err+1
print(err)
