...
def testing(df_need,df_need_not,need_,need_not_,data,pClass1,df1):#df is conditional probability,data is the test data
    ...
    return_target=[]
    def countpw(data,i):
        pw=np.zeros(len(data.iloc[0]))
        for j in range(1,len(data.iloc[0])):
            pw[j]=list(data.iloc[i]).count(data[data.columns[j]].iloc[i])/len(data)
            print('j',j)
        return pw
    return_target=[]
    for i in range(len(data)):
        ...
        for k in range(3):
            col=np.where(np.array(data.iloc[i])==k,columns)
            datak=data.values[:,np.array(data.iloc[i])==k]
            pw[np.array(data.iloc[i])==k]=np.where()
        '''
        pw=countpw(data,i)
        need_Vocab=count012(np.array(data_need_.iloc[i]))
        p1=np.prod(np.array(data_need_not_.iloc[i])*np.array(df_need_not.iloc[1]))*np.prod(np.array(df_need.iloc[3:])*need_Vocab)*pClass1/np.prod(pw)
        '''
...
print(ddd)
ret=pd.DataFrame(np.zeros((len(ddd),2)),columns=['ID','ATRGET'])
