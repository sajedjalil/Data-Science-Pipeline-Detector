...
for model in clfs:
    ...
    print('model ',indice,': AUC=',metrics.roc_auc_score(Ytarget,preds[indice]))
...
bestscore=metrics.roc_auc_score(Ytarget.values,blend_cv)
            ...
            for j in range(0,len(preds),1):
                if j!=i:
                    if poids[j]+step<=1:
                        ...
                        actualscore=metrics.roc_auc_score(Ytarget.values,blend_cv)
                        #if better, keep it
...
print("optimal blend AUC: ", bestscore)
...
submission.TARGET=blend_to_submit
submission.to_csv('simpleblend.csv', index=False)

plt.figure(indice)
submission.TARGET.hist(bins=30)
