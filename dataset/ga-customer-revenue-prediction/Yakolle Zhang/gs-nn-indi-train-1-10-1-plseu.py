...
lr_patience = 10 if indi_type_id > 1 or seg_func.__name__ != plseu.__name__ else 15
stop_patience = 15 if indi_type_id > 1 or seg_func.__name__ != plseu.__name__ else 25
epochs = 100
batch_size = 1024
...
def learn(data):
    ...
    params = {'get_output': get_simple_linear_output, 'compile_func': compile_default_mse_output,
              'cat_in_dims': list(np.array(cat_in_dims)*2), 'cat_out_dims': list(np.array(cat_out_dims)*2), 
              'seg_out_dims': list(np.array(seg_out_dims)*2),
