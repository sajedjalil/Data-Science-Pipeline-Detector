...
seg_func_r = plseu
hidden_activation_r = plseu
seg_func_c = make_plseu(0.65)
hidden_activation_c = make_plseu(0.65)
...
def learn(data):
    ...
    params = {'get_output': get_simple_linear_output, 'compile_func': compile_default_mse_output,
              ...
              'hidden_units': hidden_units, 'seg_func': seg_func_r, 'hidden_activation': hidden_activation_r}
    if indi_type_id > 1:
        params.update({'get_output': get_simple_sigmoid_output, 'compile_func': compile_default_bce_output,
                       'seg_x_val_range': (0, 1), 'fm_activation': 'softsign', 'seg_func': seg_func_c,
                       'hidden_activation': hidden_activation_c})
        measure_func = auc
        measure_name = measure_func.__name__
        min_delta = 2e-4
