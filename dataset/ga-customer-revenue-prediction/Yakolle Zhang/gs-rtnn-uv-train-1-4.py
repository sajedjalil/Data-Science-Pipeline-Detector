...
def read_res_model_weight(rtnn_model, round_num, tv_round):
    model_dir = f'../input/gs-nn-uv-train-{oof_seed}-{pred_type_id}-{round_num}'
    model = read_weights(rtnn_model, os.path.join(model_dir, f'rtnn_{oof_seed}_{pred_type_id}_{tv_round}'))
    return model
...
def learn(data):
    ...
    params = {'get_output': get_simple_linear_output, 'cat_in_dims': cat_in_dims, 'cat_out_dims': cat_out_dims,
              'seg_out_dims': seg_out_dims, 'seg_x_val_range': (0, scale_max_val), 'use_fm': use_fm,
              'seg_flag': seg_flag, 'add_seg_src': add_seg_src, 'seg_num_flag': seg_num_flag, 'fm_dim': fm_dim,
              'fm_activation': None, 'hidden_units': hidden_units, 'seg_func': seg_func,
              'hidden_activation': hidden_activation}

    c = tv_num
    while not os.path.exists(f'rtsnn_{oof_seed}_{pred_type_id}_{c}_p') and c > 0:
        c -= 1
    c += 1
    ...
    for i in range(c, tv_num + 1):
        ...
        model_id = f'rtsnn_{oof_seed}_{pred_type_id}_{i}'
        ...
        tnn_models = []
        cat_input = Input(shape=[tx['cats'].shape[1]], name='cats')
        seg_input = Input(shape=[tx['segs'].shape[1]], name='segs')
        num_input = Input(shape=[tx['nums'].shape[1]], name='nums')
        for j in range(1, num_round + 1):
            inputs = [cat_input, seg_input, num_input]
            output = get_tnn_block(0, cat_input=cat_input, seg_input=seg_input, num_input=num_input, **params)
            if j > 1:
                lp_input = Input(shape=[1], name='lp')
                inputs = inputs + [lp_input]
                output = Add(name='output')([output, Lambda(lambda x: res_shrinkage * x, name='shlp')(lp_input)])
            model = Model(inputs, output)
            model.compile(loss='mse', optimizer=Adam(lr=1e-3))
            model = read_res_model_weight(model, j, i)
            tnn_models.append(model)
        model = merge(tnn_models, res_shrinkage, rs_learnable=True)
        # model.summary()
        validation((tx, ty, vx, vy, ox, oy, ts_x, model))
        for j in range(num_round):
            for k in range(num_round):
                if k == j:
                    unfreeze_block(model, k)
                else:
                    freeze_block(model, k)
            model.compile(loss=model.loss, optimizer=Adam(lr=5e-4), metrics=model.metrics)
...
def validation(data, model_id=None, _batch_size=10000):
...
def run():
    # reboot_dir = f'../input/gs-rtnn-uv-train-{oof_seed}-{pred_type_id}'
    ...
    sys.stderr = sys.stdout = open(os.path.join('rtnn_uv_train_log'), 'w')
