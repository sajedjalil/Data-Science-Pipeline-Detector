...
################ Train classifiers ###########################################
    ...
    for i in range(6):
        ...
        svc.fit(X_train[::downsample,:], y[::downsample,i])
        scores[:,i] = svc.predict(X_test)
        # scores[:,i] = (rf.predict_proba(X_test)[:,1]*0.2 + 
                        # lda.predict_proba(X_test)[:,1]*0.6 + 
                        # lr.predict_proba(X_test)[:,1]*0.2)
