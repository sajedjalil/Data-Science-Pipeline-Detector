...
for subject in subjects:
    ...
    labels = raw._data[32:]
    feattr = pd.DataFrame(feattr.T)
    feattr = pd.concat([feattr, pd.DataFrame(labels.T)],axis=1)
    feattr.to_csv(str(subject)+'train.csv')
    ...
    featte = pd.DataFrame(featte.T,index=ids)
    featte.to_csv(str(subject)+'test.csv')
    break
