...
col.remove('id')
for i in col:
    b1[i] = (2 * fast[i]  + 2 * gruglo[i] + grucnn[i] * 5 + ave[i] + supbl[i] * 2 + best[i] * 4 +  wordbtch[i] * 2 + lgbm[i] * 2 + tidy[i] + bilst[i] * 4 + oofs[i] * 6 + corrbl[i] * 4) /  22
    
b1.to_csv('submission109.csv', index = False)