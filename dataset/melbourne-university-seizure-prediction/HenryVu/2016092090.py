...
def create_simple_csv():
    ...
    for fl in files:
        ...
        for f in sorted(list(tables.columns.values)):
            #print(tables[f])
            print("Feature %s" %f)
            print("Mean method {}".format(tables[f].mean()))
            #print("Median %f" %tables[f].median())
            #print("Std %f" %tables[f].std())
            print("Sum %f" %tables[f].sum())
            #print("Min %f" %tables[f].min())
            #print("Max %f" %tables[f].max())
            print("Len %d" %len(tables[f]))
            print("Mean calculation {}".format((tables[f].sum()/len(tables[f]))))
            #print(tables[f].sum()/tables[f].mean())
            #out.write("," + str(mean))
            return
