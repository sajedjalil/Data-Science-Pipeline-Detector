...
def create_kmeans_train(patient_id, n_clusters, n, preic, nskp):

    out = open("kmeans_train_" + str(patient_id) + "_" + str(n) + str(preic) + ".csv", "w")
    out.write("Id,sequence_id,patient_id")
    for i in range(60):
        out.write(",cl_" + str(i))
    out.write(",file_size,result\n")
    ...
    for fl in files:
        if os.path.basename(fl) in nskp:
            files_.append(fl)
    for fl in files_[20*(n-1)+10:20*n]:        
        total += 1
        # print('Go for ' + fl)
        id_str = os.path.basename(fl)[:-4]
        arr = id_str.split("_")
        ...
        for f in sorted(list(tables.columns.values)):
            ...
            signal_compressed = np.choose(labels, values)
            signal_compressed.shape = X.shape

            vmin = ft.min()
            vmax = ft.max()
            mean = signal_compressed.mean()
            std = signal_compressed.std()
            median = np.median(signal_compressed)
            out_str += "," + str(mean) + "," + str(std) + "," + str(median) +  str(vmin) + "," + str(vmax)
        out_str += "," + str(os.path.getsize(fl)) + "," + str(result) + "\n"
        if total % 6 == 0:
...
def create_kmeans_test(patient_id, n_clusters, n):

    out = open("kmeans_test_" + str(patient_id) + "_" + str(n) + ".csv", "w")
    out.write("Id,sequence_id,patient_id")
    for i in range(60):
        out.write(",cl_" + str(i))
    out.write(",file_size,result\n")
