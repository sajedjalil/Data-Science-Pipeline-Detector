# ---- tbc Private LB ------
# Enhanced version of ZFTurbo 's Seizure Boost, changes:
#  - features are:  time domian skewness +  frequency domian skewness
#  - xgboost: eta = 0.1, max_depth = 3, subsample = 0.8, colsample_bytree = 0.8, early_stopping_rounds = 100
#  - remove unsafe sample
#
# coding: utf-8
...
# Read unsafe
unsafe_df = pd.read_csv('../input/train_and_test_data_labels_safe.csv')
unsafe_file = unsafe_df[unsafe_df.safe == 0].image.values
...
def create_simple_csv_train(patient_id):
    ...
    for i in range(16):
        out.write(",time_skew_" + str(i))
    for i in range(16):
        out.write(",freq_skew_" + str(i))
    ...
    for fl in files:
        ...
        if os.path.basename(fl) in unsafe_file:
            print('Skip unsafe {} '.format(fl))
            if total % 6 == 0:
                sequence_id += 1
            continue
        ...
        for f in sorted(list(tables.columns.values)):
            time_domain_skew = skew(tables[f])
            out_str += "," + str(time_domain_skew)
        # freq skew
        for f in sorted(list(tables.columns.values)):
            freq_domain = fft(tables[f], axis=0)
            freq_domain_abs = np.abs(freq_domain)
            freq_domain_skew = skew(freq_domain_abs)
            out_str += "," + str(freq_domain_skew)
