...
def create_simple_csv():
    ...
    for row in [0, 1, 2, 3, -4, -3, -2, -1]:
        for i in range(16):
            out.write(",loc_" + str(row) + "_" + str(i))
    ...
    for fl in files:
        ...
        for row in [0, 1, 2, 3, -4, -3, -2, -1]:
            for f in sorted(list(tables.columns.values)):
                meannz = tables[f].iloc[row]
                out.write("," + str(meannz))
...
#    test_prediction = gbm.predict(xgb.DMatrix(test[features].as_matrix()), ntree_limit=gbm.best_iteration+1)
    test_prediction = gbm.predict(xgb.DMatrix(test[features].as_matrix()))
