...
def create_single_model():
    ...
    train_csv_table = pd.read_csv('../input/stage2_labels.csv')
...
def create_submission(model):
    sample_subm = pd.read_csv("../input/stage2_sample_submission.csv")
    ids = sample_subm['id'].values
    for id in ids:
        print('Predict for patient {}'.format(id))
        files = glob.glob("../input/stage2/{}/*.dcm".format(id))
        image_list = []
        for f in files:
            image = load_and_normalize_dicom(f, conf['image_shape'][0], conf['image_shape'][1])
            image_list.append([image])
