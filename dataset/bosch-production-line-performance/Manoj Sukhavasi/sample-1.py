...
trainGb = xgb.train(xgb_params,dtrain,num_boost_round=6)
from sklearn.metrics import matthews_corrcoef

dT = xgb.DMatrix(train)
ypre = trainGb.predict(dT)

topscore = 0;
bestI = 0;
for i in range(0,1,100):
    yP = ypre>i
    score = matthews_corrcoef(y,yP)
    if(score > topscore):
        topscore = score
        bestI = i

print ("Best i "+ str(i) + " Top score" + str(topscore) )
dtest = xgb.DMatrix(test)
ypred = trainGb.predict(dtest)
ytest = ypred > bestI
