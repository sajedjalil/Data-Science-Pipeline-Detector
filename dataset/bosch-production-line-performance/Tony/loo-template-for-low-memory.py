...
def GrabData():
    ...
    cols = [['Id',
            ...
            ['Id',
             ...
             'L3_S36_F3930', 'L3_S47_F4168',
             'L1_S24_F1303', 'L1_S24_F1316',
             'Response']]
    traindata = None
    testdata = None
    for i, f in enumerate(trainfiles):
        ...
        for i, chunk in enumerate(pd.read_csv(directory + f,
            ...
            print(i)
            if subset is None:
                if f == 'train_date.csv' or f == 'test_date.csv' :
                    chunkStats = pd.DataFrame()
                    chunkStats['Id'] = chunk.Id
                    chunkStats['DateCount'] = chunk.count(1)
                    #chunkStats['DateRange'] = chunk.max(1) - chunk.min(1)
                    #chunkStats['DateMean'] = chunk.mean(1)
                    chunkStats['DateMedian'] = chunk.median(1)
                    #chunkStats['DateF'] = chunkStats['DateRange']/chunkStats['DateCount']
                    chunk = chunkStats
                    del chunkStats
                subset = chunk.copy()
            else:
