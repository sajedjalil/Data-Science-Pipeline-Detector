...
def Train():
    ...
    thresholds = np.linspace(0.01, 1.99, 50)
    mcc = np.array([matthews_corrcoef(train.Response, trainpredictions/folds>thr) for thr in thresholds])
    plt.plot(thresholds, mcc)
    best_threshold = thresholds[mcc.argmax()]
    print(mcc.max()),best_threshold
