...
train_test = train_test.sort_values(by=['index']).drop(['index', 'Response', 'StartTime'], axis=1)
print(train_test[:ntrain].head())
print(train_test[ntrain:].head())
train_test[:ntrain].to_csv("Farons_train_features.csv.gz",index=False, compression="gzip")
train_test[ntrain:].to_csv("Farons_test_features.csv.gz", index=False, compression="gzip")
print("Done.....")
