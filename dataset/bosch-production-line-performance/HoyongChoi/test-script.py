...
def GrabData():
    ...
    #          'Response']]
    cols = []
    STATIONS = ['S24', 'S30', 'S32', 'S33', 'S34']
    for i, f in enumerate(trainfiles):
        data = pd.read_csv(directory + f, nrows=100000)
        if f is 'train_numeric.csv':
            data_cols = data.drop(['Id', 'Response'], axis=1).count().reset_index().sort_values(by=0, ascending=False)
        else:
            data_cols = data.drop('Id', axis=1).count().reset_index().sort_values(by=0, ascending=False)
        data_cols['station'] = data_cols['index'].apply(lambda s: s.split('_')[1])
        data_cols = data_cols[data_cols['station'].isin(STATIONS)]
        data_cols = data_cols.drop_duplicates('station', keep='first')['index'].tolist()
        if f is 'train_numeric.csv':
            data_cols.insert(len(data_cols), 'Response')
        data_cols.insert(0, 'Id')
