...
def resultsToCsv(filename, ids, y):
    ...
    if count1 > 0:
        df[['Id', 'Response']].to_csv("{0}-{1}.csv".format(filename, count1), index=False)
...
def Train():
    ...
    #resultsToCsv('xgbsubmission08', test.Id.values, (testpredictions/folds > .08).astype(int))
    #resultsToCsv('xgbsubmission09', test.Id.values, (testpredictions/folds > .09).astype(int))
    #resultsToCsv('xgbsubmission10', test.Id.values, (testpredictions/folds > .1).astype(int))
    resultsToCsv('xgbsubmission20', test.Id.values, (testpredictions/folds > .2).astype(int))
    resultsToCsv('xgbsubmission25', test.Id.values, (testpredictions/folds > .25).astype(int))
    resultsToCsv('xgbsubmission30', test.Id.values, (testpredictions/folds > .3).astype(int))
