...
scores = [0, 0.793, 0.80121, 0.79745, 0.7947, 0.7959, 0.81177, 0.7959] # public leaderboard scores
...
number1 = df['project_is_approved1'].clip(almost_zero,almost_one).apply(logit) * weights[1] # Any answers really close to zero or really close to one will be eliminated
...
df[['id', 'project_is_approved']].to_csv("ensembling_submission.csv", index=False)