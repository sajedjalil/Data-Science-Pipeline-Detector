...
# "The Choice is Yours"
df1 = pd.read_csv("../input/nlp-and-stacking-starter-dpcnn-lgb-lb0-80/submission.csv").rename(columns={'project_is_approved': 'project_is_approved1'}) # 0.793
...
#df3 = pd.read_csv("../input/the-all-in-one-model/submission.csv").rename(columns={'project_is_approved': 'project_is_approved3'}) # 0.78583

# LightGBM and TFIDF
df4 = pd.read_csv("../input/lightgbm-and-tf-idf-starter/submission.csv").rename(columns={'project_is_approved': 'project_is_approved4'}) # 0.79516
...
scores = [0, 0.8, 0.80795, 0.79516, 0.7959, 0.81177, 0.7959] # public leaderboard scores

weights = [0, 0, 0, 0, 0, 0, 0]

power = 68 # 68 > 65~720 > 64 > 72 > 96 > 8 > 4

weights[1] = scores[1] ** power
weights[2] = scores[2] ** power
weights[3] = scores[3] ** power
weights[4] = scores[4] ** power
weights[5] = scores[5] ** power
weights[6] = scores[6] ** power
#weights[7] = scores[7] ** power
...
number3 = df['project_is_approved7'].clip(almost_zero,almost_one).apply(logit) * weights[3]
...
df['project_is_approved'] = ( number1 + number2 + number3 + number4 + number5 + number6 ) / ( totalweight )
